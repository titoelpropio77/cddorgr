...
function actualizar_venta($ven_id = 0) {
    ...
    $sql_ventas = "select * from venta where ven_urb_id=5";    
...
function actualizar_usuarios(){
    $conec=new ADO();
    $pagos=  FUNCIONES::objetos_bd_sql("select * from venta_pago where vpag_importado=1 and vpag_monto>0 and vpag_fecha_pago>='2015-01-01' and vpag_fecha_pago<='2015-12-31' and vpag_usu_import='' limit 10000");
    $nro_proc=0;
    for ($i = 0; $i < $pagos->get_num_registros(); $i++) {
        $pag = $pagos->get_objeto();
        $sql_sel_venta="select ven_id, ven_urb_id from venta where ven_id=$pag->vpag_ven_id ";
        $venta= FUNCIONES::objeto_bd_sql($sql_sel_venta);
        $sql_sel_cli="select int_id, int_codigo from interno where int_id='$pag->vpag_int_id'";
        $cliente= FUNCIONES::objeto_bd_sql($sql_sel_cli);
        $sql_sel="select * from recibos where py='$venta->ven_urb_id' and numero_recibo='$pag->vpag_recibo' and codigo_c='$cliente->int_codigo'";
        $recibo = _db::objeto_sql($sql_sel);
        $_af=  explode(' ', $recibo->fecha);
        $_fecha=$_af[0];
        $_remonto=$recibo->monto*1+$recibo->formularios*1;
        $_pagmonto=$pag->vpag_monto*1;
        $dif_monto=  abs($_remonto-$_pagmonto);
        
//        if(!$recibo || $_fecha!=$pag->vpag_fecha_pago){
        if(!$recibo ){
//            echo "-- * $_remonto-$_pagmonto=$dif_monto<br>";
            $sql_sel="select * from recibos where py='$venta->ven_urb_id' and numero_recibo_ini='$pag->vpag_recibo' and codigo_c='$cliente->int_codigo'";
            $recibo = _db::objeto_sql($sql_sel);
        }
...
//            echo "<br>";
    ...
    echo "Total Registros: <b>$nro_proc</b>";
}
...
//    $and_filtro = " and ven_id >=7001 and ven_id <=8000";
    $urb_ids=_urbs;
    $and_filtro = " and ven_id=9441 and ven_urb_id in ($urb_ids)";
...
//    return;
    ...
    $insert = "insert into venta(
                    ...
                    ven_costo,ven_costo_cub,ven_costo_pag,ven_numero,ven_promotor,ven_ubicacion,ven_suc_id
                ) values";
    $sql_insert = $insert;
    $reg = 0;
...
//            }
            ...
            //                    ven_observacion,ven_codigo,ven_vdo_id,ven_comision,ven_concepto,
            //                    ven_tipo_pago,ven_fecha_cre,ven_monto_pagar,ven_form,ven_intercambio_ids,ven_intercambio_montos,
            //                    ven_costo,ven_costo_cub,ven_costo_pag,ven_numero,ven_promotor,ven_ubicacion
            //                ) values";
            $sql_insert.="('$interno->int_id','0','$lote->lot_id','$tobj->PY','$ven_fecha','$moneda','0',
                            ...
                            '$costo','$costo_cub','0','$tobj->Numero_Adj','$tpromotor->Nombre','$tmp_venta->ciudad','1'
                            )";
            $num++;
            $reg++;
