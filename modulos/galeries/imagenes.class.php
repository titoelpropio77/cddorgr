...
class IMAGENES extends BUSQUEDA 
{
		...
		for($i=0;$i<$num;$i++)
			{
				...
				echo '<tr>';
					...
					echo "<td>";
						?>
						<center><img src="imagenes/foto/chica/<?php echo $objeto->gfo_archivo; ?>" border="0" width="100"></center>
						<?php
					echo "&nbsp;</td>";
					echo "<td>";
		...
		function ejecutar_script(id,gfo){
				...
				$.prompt(txt,{ 
					...
					callback: function(v,m,f){
						...
						if(v){
								location.href='gestor.php?mod=galeries&acc=ELIMINAR&tarea=FOTOS&id='+id+'&gfo='+gfo;
						}
	...
	function insertar_tcp()
	{
		...
		$sql="insert into galeria_fotodos(gfo_archivo,gfo_descripcion,gfo_gal_id) values 
	...
	{	
		 ...
		 $upload_class->upload_dir = "imagenes/foto/"; 

		 $upload_class->upload_log_dir = "imagenes/foto/upload_logs/"; 
		...
		{		    
			...
			{ 
					...
					$mifile='imagenes/foto/'.$upload_class->file_name;
					...
					if ($handle->uploaded) 
					{
						...
						$handle->process('imagenes/foto/chica/');
	...
	function eliminar_tcp()
	{
		...
		if(trim($mi)<>"")
		{
			$mifile="imagenes/foto/$mi";
		
			@unlink($mifile);
			
			$mifile2="imagenes/foto/chica/$mi";
		...
		$sql="delete from galeria_fotodos where gfo_id='".$_GET['gfo']."'";
	...
	function nombre_imagen($id)
	{
		...
		$sql="select gfo_archivo from galeria_fotodos where gfo_id='".$id."'";
