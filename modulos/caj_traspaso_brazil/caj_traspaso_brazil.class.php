...
class caj_traspaso_brazil extends BUSQUEDA {

    var $formulario;
    var $mensaje;
    var $usu;

    function caj_traspaso_brazil() {  //permisos
        $this->ele_id = 192;
        $this->busqueda();
        ...
        $this->modulo = 'caj_traspaso_brazil';

        $this->formulario = new FORMULARIO();

        $this->formulario->set_titulo('TRASPASO CAJA BRAZIL');
    ...
    function dibujar_listado() {
        $sql = "select * from caj_traspaso_brazil where 1 ";
        $this->set_sql($sql, '');
        $this->set_opciones();
        $this->dibujar();
    }
    ...
    function mostrar_busqueda() {
        ...
        for ($i = 0; $i < $this->numero; $i++) {
            ...
            echo '<tr>';
            echo "<td>";
                echo FUNCIONES::get_fecha_latina($objeto->tbr_fecha);
            echo "</td>";
            echo "<td>";
                echo $objeto->tbr_observacion;
            echo "</td>";
            echo "<td>";
                echo FUNCIONES::atributo_bd_sql("select cue_descripcion as campo from con_cuenta where cue_id='$objeto->tbr_cue_ori'");
            echo "</td>";
            echo "<td>";
                echo FUNCIONES::atributo_bd_sql("select cue_descripcion as campo from con_cuenta where cue_id='$objeto->tbr_cue_des'");
            echo "</td>";
            echo "<td>";
                echo $objeto->tbr_usu_cre;
            echo "</td>";
            echo "<td>";
            echo $this->get_opciones($objeto->tbr_id);
            echo "</td>";
            echo "</tr>";
    ...
    function cargar_datos() {
        ...
        $sql = "select * from caj_traspaso_brazil 
				where tbr_id = '" . $_GET['id'] . "'";

        $conec->ejecutar($sql);

        $objeto = $conec->get_objeto();

        $_POST['tbr_id'] = $objeto->tbr_id;

        $_POST['tbr_nombre'] = $objeto->tbr_nombre;
        $_POST['tbr_descripcion'] = $objeto->tbr_descripcion;
    }
    ...
    function datos() {
        if ($_POST) {
            ...
            $valores[$num]["valor"] = $_POST['tbr_nombre'];
            $valores[$num]["tipo"] = "todo";
            $valores[$num]["requerido"] = true;
            $num++;
            $valores[$num]["etiqueta"] = "Descripcion";
            $valores[$num]["valor"] = $_POST['tbr_descripcion'];
            $valores[$num]["tipo"] = "todo";
            $valores[$num]["requerido"] = true;
            $num++;
    ...
    function formulario_params() {
        ...
        <div id="Contenedor_NuevaSentencia">
            <form id="frm_caj_traspaso_brazil" name="frm_caj_traspaso_brazil" action="<?php echo $url; ?>" method="POST" enctype="multipart/form-data">  
...
//                            var marcados=$('.checkdetalle[type=checkbox]:checked"');
    ...
    function emergente() {
        ...
        <script>
            ...
            {
                opener.document.frm_caj_traspaso_brazil.tbr_int_id.value = id;
                opener.document.frm_caj_traspaso_brazil.tbr_nombre_persona.value = valor;
                window.close();
            }
        </script>
        <br><center><form name="form" id="form" method="POST" action="gestor.php?mod=caj_traspaso_brazil&tarea=AGREGAR&acc=Emergente">
                <table align="center">
                    <tr>
        ...
        ?>
            ...
            function insertar_tcp() {
                ...
                $conec = new ADO();
                $sql = "insert into caj_traspaso_brazil (tbr_nombre,tbr_descripcion,tbr_eliminado)
                    values ('" . $_POST['tbr_nombre'] . "','" . $_POST['tbr_descripcion'] . "','No')";
                //echo $sql.'<br>';
                $conec->ejecutar($sql);
            ...
            function modificar_tcp() {
                $conec = new ADO();
                $sql = "update caj_traspaso_brazil set 
                        tbr_nombre='" . $_POST['tbr_nombre'] . "',
                        tbr_descripcion='" . $_POST['tbr_descripcion'] . "'
                        where tbr_id='" . $_GET['id'] . "'";
                //echo $sql;	
                $conec->ejecutar($sql);
            ...
            function formulario_confirmar_eliminacion() {
                ...
                $this->formulario->ventana_confirmacion($mensaje, $this->link . "?mod=$this->modulo", 'tbr_id');
            }

            function eliminar_tcp() {
//                $cantidad = FUNCIONES::atributo_bd("ad_venta", "usu_tbr_id='" . $_POST['tbr_id'] . "'", 'count(*)');
////            echo$cantidad."<br>";
//                if ($cantidad == 0) {
////                echo "elimino";
////                return;
