...
//        echo '</pre>';
    ...
    function mostrar_reporte() {
        ...
        ?></br><?php echo strtoupper($this->nombre_uv()); ?></center></h3></strong></p></td>
					...
					and (ven_lot_ids is null or ven_lot_ids = '')
					$cad
                                        union 
                                        select vlot_lot_id as lot_id,vlot_superficie as ven_superficie,vlot_ven_id as ven_id,vlot_valor as ven_monto,vlot_valor as ven_valor,vlot_metro as ven_metro,ven_tipo,ven_decuento,
                                            ven_moneda,ven_cuota_inicial,ven_vdo_id,ven_fecha,ven_plazo,ven_observacion,
                                            int_nombre,int_apellido,uv_nombre,man_nro,
                                            lot_nro,cast(man_nro as SIGNED) as manzano ,cast(lot_nro as SIGNED) as lote,
                                            zon_precio,zon_moneda		
                                        FROM 
                                        venta_lote inner join
					venta on(vlot_ven_id=ven_id)
					inner join interno on (ven_int_id=int_id)
					inner join lote on (vlot_lot_id=lot_id)
					inner join zona on(lot_zon_id=zon_id)
					inner join uv on (lot_uv_id=uv_id)
					inner join manzano on (lot_man_id=man_id)
					inner join urbanizacion on (man_urb_id=urb_id)
...
//                                                    echo FUNCIONES::atributo_bd_sql("select concat(int_nombre,' ',int_apellido) as campo from venta,interno where ven_lot_id='$objeto->lot_id' and ven_estado in ('Pendiente','Pagado') and ven_int_id=int_id");
                                                    $comprador = FUNCIONES::atributo_bd_sql("select concat(int_nombre,' ',int_apellido) as campo from venta,interno where ven_lot_id='$objeto->lot_id' and ven_estado in ('Pendiente','Pagado') and ven_int_id=int_id");
                                                    if ($comprador == '') {
                                                        $sql_cons = "select concat(int_nombre,' ',int_apellido) as campo from venta_lote 
                                                        inner join venta on(vlot_ven_id=ven_id)
                                                        inner join interno on(ven_int_id=int_id)
                                                        where vlot_lot_id='$objeto->lot_id' and ven_estado in ('Pendiente','Pagado')";
                                                        
                                                        $comprador = FUNCIONES::atributo_bd_sql($sql_cons);
                                                    }
                                                    echo ($comprador != '') ? $comprador: 'No encontrado';
    ...
    function cargar_tabla_venta_lote(){
