...
{	
    if (validCode()) {								      
	  ...
	  var code = fragCode[0] + $("#codigo_pg").val() + fragCode[1];
	  $.ajax({
...
var validCode = function(){
        var str_frag_code0=fragCode[0];
        if(fragCode[0]===undefined){
            str_frag_code0="";
        }
        var str_frag_code1=fragCode[1];
        if(fragCode[1]===undefined){
            str_frag_code1="";
        }
	var code = str_frag_code0 + $("#codigo_pg").val() + str_frag_code1;
	//var code = $("#codigo_pg").val();
	var formate = $("#formatoCodigo").val();
	if ($("#codigo_pg").val() == "") {
	   $.prompt('El campo codigo es requerido.', {opacity: 0.8});
	   return false;
	}
	if ($("#cuenta_pg").val() == "") {
	...
	if(formate.split(".").length != code.split(".").length){
	   $.prompt('Codigo invalido. Formato requerido: ' + formate, {opacity: 0.8});
	   return false;
	}
        console.log(code);
