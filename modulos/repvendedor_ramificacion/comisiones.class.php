...
//        $obj = FUNCIONES::objeto_bd_sql("select * from venta where ven_id='$data->venta'");
        ...
        $sql = "insert into " . COMISION::get_tabla() . "(com_ven_id,com_vdo_id,
                ...
                com_tipo,com_porcentaje,com_pdo_id,com_fecha_mod,com_usu_mod)
                values('{$venta->ven_id}','$data->vendedor',
                '$data->monto','$data->moneda',
                '$estado','$data->fecha',
                '$data->usuario','$data->glosa',
                '$data->tipo','$data->porcentaje','$data->periodo','$data->fecha_mod','$data->usuario_mod');";
        FUNCIONES::eco($sql);
    ...
    public static function actualizar_comisiones($data) {
        ...
        $sql_anul = "update comision set com_estado='Anulado' where com_pdo_id=$data->periodo 
            and com_estado='Pendiente' and com_tipo in ('BIR','BVI','BRA','FED')";
        FUNCIONES::bd_query($sql_anul);
        FUNCIONES::bd_query("delete from comision where com_pdo_id=$data->periodo 
            and com_estado='Anulado' and com_tipo in ('BIR','BVI','BRA','FED')");
...
//                            echo "esta al dia";
    ...
    public static function insertar_comisiones_cobro($data) {
        ...
        for ($i = 0; $i < 7; $i++) {
            ...
            $vendedor = FUNCIONES::objeto_bd_sql("select * from vendedor where vdo_id='$vendedor->vdo_vendedor_id'");
        }
    }
