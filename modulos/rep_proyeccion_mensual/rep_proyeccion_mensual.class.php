...
class REP_PROYECCION_MENSUAL extends BUSQUEDA {

    var $formulario;
    var $mensaje;

    function REP_PROYECCION_MENSUAL() {
        $this->ele_id = 212;
        $this->busqueda();
        $this->coneccion = new ADO();
        $this->link = 'gestor.php';
        $this->modulo = 'rep_proyeccion_mensual';
        $this->formulario = new FORMULARIO();
        $this->formulario->set_titulo('PROYECCION MENSUAL');
    ...
    function obtener_resultados() {
        ...
        if ($_POST['ven_inicio'] <> "") {
            $filtro.=" and ven_fecha>= '" . FUNCIONES::get_fecha_mysql($_POST['ven_inicio']) . "' ";
            $info[]=array('label'=>'Fecha Inicio Ventas','valor'=> $_POST['ven_inicio']);
        } 
        if ($_POST['ven_fin'] <> "") {
            $filtro.=" and ven_fecha <='" . FUNCIONES::get_fecha_mysql($_POST['ven_fin']) . "' ";
            $info[]=array('label'=>'Fecha Fin Ventas','valor'=> $_POST['ven_fin']);
        }
        ...
        $sql = "select year(ind_fecha_programada) as anio , month(ind_fecha_programada) as mes,ind_moneda as moneda,sum(ind_monto) as monto,sum(ind_monto_pagado) as monto_pagado,count(*) as cantidad 
                from interno_deuda ,venta
                where 
                ind_tabla='venta' and ind_estado='Pendiente' $filtro
                and ven_id=ind_tabla_id
                group by year(ind_fecha_programada) , month(ind_fecha_programada), ind_moneda
                order by anio asc,mes asc, ind_estado asc, ind_moneda asc";
//        echo "$sql;<br>";
...
//        $fecha_act=date('Y-m-d');
        ...
        for ($i = 0; $i < $num; $i++) {
            ...
            if($monto>0){
                ...
                $this->sumar_monto($anio, $mes, $mon,$monto,$cantidad,$tca, $proyecciones);
            }
            $conec->siguiente();
        }
//        echo "<pre>";
...
//            $total->total_2+=$total_2;
            ...
            $result[]=array(
                $nro,$proy->anio,$proy->mes,round($proy->monto_1,2),round($proy->monto_2,2),$total_1,$total_2
            );
            $total->monto_1+=$proy->monto_1;
            $total->monto_2+=$proy->monto_2;
            $total->total_1+=$total_1;
    ...
    function sumar_monto($anio, $mes, $mon, $monto, $cantidad, $tca, &$proyecciones){
        $form_pago=7;
//        $head=array('#','A&ntilde;o','Mes','Monto Bs.','Monto $us','Convertido a Bs.','Convertido a $us');
...
//                                    $('#cja_cue_id').trigger('chosen:updated');
                            ...
                            <fieldset>
                            <legend>Filtros Para Ventas:</legend>
                                <div id="ContenedorDiv">
                                    <div class="Etiqueta" >Fecha Inicio</div>
                                    <div id="CajaInput">
                                        <input class="caja_texto" name="ven_inicio" id="ven_inicio" size="12" value="<?php echo date('d/m/Y'); ?>" type="text">
                                    </div>		
                                </div>
                                <div id="ContenedorDiv">
                                    <div class="Etiqueta" >Fecha Fin</div>
                                    <div id="CajaInput">
                                        <input class="caja_texto" name="ven_fin" id="ven_fin" size="12" value="<?php echo date('d/m/Y'); ?>" type="text">
                                    </div>		
                                </div>
                            </fieldset>
                            
                            <fieldset>
                            <legend>Filtros Para Cuotas:</legend>
                            <div id="ContenedorDiv">
                                <?php
                                $mes=  date('m');
                                $anio=  date('Y');
                ...
                <div>
                    <script>
                        jQuery(function($) {
                            ...
                            $("#fin").mask("99/99/9999");
                            $("#ven_inicio").mask("99/99/9999");
                            $("#ven_fin").mask("99/99/9999");
                        });
                    </script>
                    <?php
                }
