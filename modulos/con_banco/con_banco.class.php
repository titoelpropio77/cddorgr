<?php
class con_banco extends BUSQUEDA 
{
	var $formulario;
	var $mensaje;
	var $usu;
		...
		for($i=0;$i<$this->numero;$i++)
			{
				...
				echo '<tr>';
					...
					echo "<td>";
						echo $objeto->ban_nombre;
						echo "&nbsp;";
					echo "</td>";
					
					echo "<td>";
                                                echo $objeto->ban_descripcion;
						echo "&nbsp;";
					echo "</td>";
					echo "<td>";
	...
	function datos()
	{
		if($_POST)
		{
			...
			$valores[$num]["valor"]=$_POST['ban_nombre'];
			$valores[$num]["tipo"]="todo";
			$valores[$num]["requerido"]=true;
			$num++;
			$valores[$num]["etiqueta"]="Descripcion";
			$valores[$num]["valor"]=$_POST['ban_descripcion'];
			$valores[$num]["tipo"]="todo";
			$valores[$num]["requerido"]=true;
			$num++;
...
//		}
	...
	{            
            $cantidad=  FUNCIONES::atributo_bd("con_comprobante", "cmp_ban_id='".$_POST['ban_id']."' and cmp_eliminado='No'", 'count(*)');
//            echo$cantidad."<br>";
            if($cantidad==0){
//                echo "elimino";
//                return;
                $conec= new ADO();		
                $sql="update con_banco set ban_eliminado='Si' where ban_id='".$_POST['ban_id']."'";			 
                $conec->ejecutar($sql);			                
        ...
        function total_comisionado($con_banco){
            $sql = "select sum(com_monto)as total from comision where com_estado='Pendiente' and com_ban_id='".$con_banco."'";
            $conec = new ADO();
            $conec->ejecutar($sql);
            $num = $conec->get_num_registros();
            $total = 0;
        ...
        function comision_pagada($con_banco){
            //$sql = "select sum(com_monto)as total from comision where com_estado='Pagado' and com_ban_id='".$con_banco."'";
            $sql = "select sum(pve_monto)as total from pago_con_bancoes where pve_estado='Activo' and pve_ban_id='".$con_banco."'";
            $conec = new ADO();
            $conec->ejecutar($sql);
            $num = $conec->get_num_registros();
            $total = 0;
        ...
        function formulario_pago_comision() {
                    ...
                    function validar_campos_descuento() {
                        ...
                        if (comdes_motivo === '' || comdes_monto === '') {
                            ...
                            if ((total_comisionado - total_descuento) >= parseFloat(comdes_monto)) {
                                location.href = 'gestor.php?mod=con_banco&tarea=CUENTAS&acc=descuentos&acc2=add&comdes_motivo=' + comdes_motivo + '&comdes_monto=' + comdes_monto + '&id=<?php echo $_GET['id']; ?>&com=<?php echo $_GET['com']; ?>&ven=<?php echo $_GET['ven']; ?>';
                            } else {
                ...
                <div id="Contenedor_NuevaSentencia">
                    <form id="frm_con_banco" name="frm_con_banco" action="<?php echo $url.'&tarea=PAGOS COMISIONES&acc=pagar&id='.$_GET['id']; ?>" method="POST" enctype="multipart/form-data">  
                        <div id="FormSent">
...
//                                        else
                            ...
                            <div id="ContenedorDiv">
                                <div id="CajaBotones">
                                    <center>
                                        ...
                                        <input type="reset" class="boton" name="" value="Cancelar">
                                        <input type="button" class="boton" name="" value="Volver" onclick="javascript:location.href = 'gestor.php?mod=con_banco&tarea=ACCEDER';">                                            
                                    </center>
                                </div>
                            </div>
                        </div>
        ...
        function ver_comprobante_pago_comision($pve_id){
            $sql = "select pve_ban_id,pve_monto,pve_moneda,pve_fecha,pve_glosa,int_nombre,int_apellido 
                    from pago_con_bancoes 
                    inner join con_banco on(ban_id=pve_ban_id)
                    inner join interno on(int_id=ban_int_id)
                    where pve_id ='".$pve_id."'";
            $conec= new ADO();
            $conec->ejecutar($sql);
            ...
            $extra2="'</center></body></html>'"; 
                    ...
                    echo '	<table align=right border=0><tr><td><a href="javascript:var c = window.open('.$page.','.$extpage.','.$features.');
                            ...
                            </a></td><td><img src="images/back.png" align="right" width="20" border="0"  title="VOLVER" onclick="javascript:location.href=\'gestor.php?mod=con_banco&tarea=ACCEDER\';"></td></tr></table>
                            ';
            $conversor = new convertir();
            ?>
			...
			<table style="font-size:12px;" width="100%"  cellpadding="5" cellspacing="0" >
				<tr>
				   ...
				   <tr>
				    <td colspan="2">
				    <strong>con_banco: </strong> <?php echo $objeto->int_nombre.' '.$objeto->int_apellido;?> <br/><br/>
        ...
        function listar_pagos_comisiones() {
                ...
                <script>
                    function anular_pago_comision(id) {
                        ...
                        $.prompt(txt, {
                            ...
                            callback: function(v, m, f) {
                                ...
                                if (v) {
                                    location.href = 'gestor.php?mod=con_banco&tarea=PAGOS COMISIONES&acc=anular&pag_id=' + id;
                                }
                    ...
                    function ver_comprobante_pago_comision(id){
                        location.href = 'gestor.php?mod=con_banco&tarea=PAGOS COMISIONES&acc=ver&pag_id=' + id;
                    }
                </script>
                <?php
                $sql = "select * from pago_con_bancoes where pve_ban_id='" . $_GET['id'] . "'";
                $conec = new ADO();
                $conec->ejecutar($sql);
                $num = $conec->get_num_registros();
                $conv = new convertir();
        ...
        function pagar_monto_comision($ban_id){
            ...
            if($num > 0){
                ...
                $sql = "select int_nombre,int_apellido,int_id,ban_can_id from interno inner join con_banco on (ban_int_id=int_id) where ban_id='".$ban_id."'";                        
                $conec->ejecutar($sql);            
                $obj = $conec->get_objeto();
                $interesado = $obj->int_nombre . " " .$obj->int_apellido;
                $ca_con_banco=$obj->ban_can_id;
                $moneda = '2';                
                $monto = $_POST['comision_a_pagar'];
                $glosa =$_POST['pag_glosa'] ;//'Pago de Comision - con_banco: ' . $interesado . " - Monto: " . $monto;
                $pag_fecha=  FUNCIONES::get_fecha_mysql($_POST['pag_fecha']);
                
                $sql_insert = "insert into pago_con_bancoes(pve_fecha,pve_hora,
                                                            pve_usu_id,pve_ban_id,
                                                            pve_monto,pve_moneda,
                                                            pve_estado,pve_glosa)
                                                    values('".date('Y-m-d')."','".date('H:i')."','".
                                                            $this->usu->get_id()."','".$ban_id."','".
                ...
                ///generar comprobante..
                //$ca_con_banco=  FUNCIONES::atributo_bd("con_banco", "ban_id=".$ban_id, "ban_can_id");
                ...
                $comprobante->tabla = "pago_con_bancoes";
                $comprobante->tabla_id = $llave;
                
                $comprobante->detalles=array(
                            array("ca"=>$ca_con_banco,"cc"=>"","cf"=>"","cuen"=>FUNCIONES::get_cuenta($ges_id,'5.1.1.01.005'),
                            ...
                            array("ca"=>$ca_con_banco,"cc"=>"","cf"=>"","cuen"=>FUNCIONES::get_cuenta($ges_id,'2.1.1.02.002'),
                                "glosa"=>$glosa,"debe"=>$monto,"haber"=>0
                                ),
                            array("ca"=>$ca_con_banco,"cc"=>"","cf"=>"","cuen"=>FUNCIONES::get_cuenta($ges_id,'2.1.1.02.003'),
                                "glosa"=>$glosa,"debe"=>0,"haber"=>$monto
                                )
        ...
        function anular_monto_comision($pag_id){
            $pago=  FUNCIONES::objeto_bd_sql("select * from pago_con_bancoes where pve_id='$pag_id'");
...
//            echo "anulando el pago de comision: " . $pag_id;
            //$sql = "update comprobante set cmp_estado=2 where cmp_tabla='pago_con_bancoes' and cmp_tabla_id='".$pag_id."'";
            $conec = new ADO();
//            $conec->ejecutar($sql);
            $sql2 = "update pago_con_bancoes set pve_estado='Anulado' where pve_id='".$pag_id."'";
            $conec->ejecutar($sql2);
            include_once 'clases/registrar_comprobantes.class.php';
            COMPROBANTES::anular_comprobante('pago_con_bancoes', $pag_id);
	...
	function cambiar()
	{
		...
		$sql="update comision set com_ban_id='".$_POST['con_banco']."' where com_id='".$_POST['comision']."'";
		...
		$sql="update venta set ven_ban_id='".$_POST['con_banco']."' where ven_id='".$venta."'";
			
		$conec->ejecutar($sql);	
		
		$this->formulario->mensaje('Correcto','La comisión fue asignada a otro con_banco.');
	}
	...
	function frm_cambiar()
	{
		?>
		<div id="Contenedor_NuevaSentencia">
			<form id="frm_con_banco" name="frm_con_banco" action="gestor.php?mod=con_banco&tarea=CUENTAS&id=<?php echo $_GET['id']; ?>&acc=cambiar" method="POST" enctype="multipart/form-data">  
				<div id="FormSent">
				  
					<div class="Subtitulo">Seleccione el con_banco al que asignara la comisión</div>
						<div id="ContenedorSeleccion">
							<!--Inicio-->
							<div id="ContenedorDiv">
							   <div class="Etiqueta" >con_banco</div>
								<div id="CajaInput">
									<select style="width:200px;" name="con_banco" class="caja_texto">
										   <?php 		
											$fun=NEW FUNCIONES;		
											$fun->combo("select ban_id as id,concat(int_nombre,' ',int_apellido) as nombre from con_banco inner join interno on (ban_int_id=int_id) where ban_estado='Habilitado'",$_POST['con_banco']);				
											?>
								   </select>
						...
						<div id="ContenedorDiv">
						   <div id="CajaBotones">
								<center>
									...
									<input type="button" class="boton" name="" value="Volver" onclick="javascript:location.href='gestor.php?mod=con_banco&tarea=CUENTAS&id=<?php echo $_GET['id']; ?>';">
	...
	function listado_cuentas()
	{
		...
		function pagar_comision(id){
				...
				$.prompt(txt,{ 
					...
					callback: function(v,m,f){
						...
						if(v){
								location.href='gestor.php?mod=con_banco&tarea=CUENTAS&acc=comision&id='+id;
						}
		...
		function cambiar_comision(id){
			var txt = 'Estas seguro que cambiara la comisión a otro con_banco?';
			...
			$.prompt(txt,{ 
				...
				callback: function(v,m,f){
					...
					if(v){
							location.href='gestor.php?mod=con_banco&tarea=CUENTAS&acc=frm_cambiar&id=<?php echo $_GET['id']; ?>&com='+id;
					}
        ...
        $sql="select * from con_banco
		inner join interno on (int_id=ban_int_id) where ban_id=".$_GET['id'];
        ...
        <table style="font-size:12px;" width="100%"  cellpadding="5" cellspacing="0" >
        	<tr>
       		...
       		<tr>
        		<td colspan="2">
                	<br />
        			<strong>con_banco: </strong> <?php echo $objeto->int_nombre.' '.$objeto->int_apellido;?>
        		</td>
        		<td align="right">
        		</td>
      		</tr> 
		...
		inner join con_banco on (com_ban_id=ban_id)
		inner join interno on (ban_int_id=int_id)
		where
		com_estado='Pendiente' and com_ban_id='".$_GET['id']."' and ven_estado!='Anulado'
		order by ven_id asc";
...
//					echo "</td>";
		...
		FROM 
		comision inner join venta on (com_estado='Pagado' and com_ban_id='".$_GET['id']."' and com_ven_id=ven_id)
		inner join lote on (ven_lot_id=lot_id) 
		inner join manzano on (lot_man_id=man_id)
		inner join urbanizacion on (man_urb_id=urb_id)
		inner join con_banco on (com_ban_id=ban_id)
...
<!--						<center>
						<table>
							<tr>
								<td><a class="linkOpciones" href="gestor.php?mod=con_banco&tarea=CUENTAS&acc=imprimir&id=<?php // echo $objeto->com_id; ?>">
...
//					echo "</td>";
	...
	function pagar_comision()
	{
		...
		if($nume > 0)
		{
			...
			/*
			$sql="SELECT com_ven_id,com_monto,com_moneda,int_nombre,int_apellido,urb_nombre,man_nro,lot_nro,ban_cco_id,ban_cue_id,ban_int_id 
			FROM 
			...
			$cmp_id = $comp->ingresar_comprobante(date('Y-m-d'),$this->tc,$objeto->com_moneda,'',$objeto->ban_int_id,$this->usu->get_id(),'2','1','comision',$_GET['id']);			   
			...
			if($objeto->com_moneda=='1')
			{
				...
				$comp->ingresar_detalle($cmp_id,$mde,$objeto->ban_cue_id,$cco,"Comisión por la venta del terreno: Urb:$objeto->urb_nombre - Mza:$objeto->man_nro - Lote:$objeto->lot_nro");
			...
			else
			{
				...
				$comp->ingresar_detalle($cmp_id,0,$objeto->ban_cue_id,$cco,"Comisión por la venta del terreno: Urb:$objeto->urb_nombre - Mza:$objeto->man_nro - Lote:$objeto->lot_nro",$mde);
	...
	{		
		...
		$sql="SELECT com_ban_id,com_monto,com_moneda,com_fecha_pag,int_nombre,int_apellido,urb_nombre,man_nro,lot_nro,ban_cco_id,ban_cue_id,ban_int_id 
		FROM 
		...
		$extra2="'</center></body></html>'"; 
			...
			echo '	<table align=right border=0><tr><td><a href="javascript:var c = window.open('.$page.','.$extpage.','.$features.');
				  ...
				  ">
				<img src="images/printer.png" align="right" width="20" border="0" title="IMPRIMIR">
				</a></td><td><img src="images/back.png" align="right" width="20" border="0"  title="VOLVER" onclick="javascript:location.href=\'gestor.php?mod=con_banco&tarea=CUENTAS&id='.$objeto->com_ban_id.'\';"></td></tr></table>
				';
		$conversor = new convertir();
