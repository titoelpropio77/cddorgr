...
class con_comprobante extends BUSQUEDA {
    ...
    function CON_COMPROBANTE() {
        ...
        $this->arreglo_campos[7]["nombre"] = "cmp_tabla";
        $this->arreglo_campos[7]["texto"] = "Origen";
        $this->arreglo_campos[7]["tipo"] = "comboarray";
        $this->arreglo_campos[7]["valores"] = "venta_pago,cv_divisa,reserva_pago,comision,venta,con_traspaso,con_compra,reserva_anulacion,venta_retencion,venta_activacion,venta_fusion,extra_pago,venta_cambio_lote,venta_devolucion,reserva_devolucion,pago_vendedores,venta_ajuste_saldos:Venta Pago,C/V Divisa,Reserva Pago,Comision,Venta,Traspaso,Compra,Reserva Anulacion,Venta Retencion,Venta Activacion,Fusion de Ventas,Pago Extra,Venta Cambio Lote,Venta Devolucion,Reserva Devolucion,Pago Vendedores,Ajuste de Saldos Migraciones";
        
        $this->arreglo_campos[8]["nombre"] = "cmp_tabla_id";
        $this->arreglo_campos[8]["texto"] = "Registro ID";
        $this->arreglo_campos[8]["tipo"] = "numero";
    ...
    function dibujar_listado() {
		...
		if ($_POST) {
			$sql = "SELECT cmp_id, tco_descripcion, mon_id, mon_titulo, mon_Simbolo, cmp_nro, cmp_nro_documento, cmp_fecha,
                ges_descripcion, pdo_id, pdo_descripcion, cmp_glosa, cmp_referido, cmp_tabla,cmp_usu_cre,cmp_tabla_id
			...
			$sql = "SELECT cmp_id, tco_descripcion, mon_id, mon_titulo, mon_Simbolo, cmp_nro, cmp_nro_documento, cmp_fecha,
                ges_descripcion, pdo_id, pdo_descripcion, cmp_glosa, cmp_referido, cmp_tabla,cmp_usu_cre,cmp_tabla_id
                FROM con_comprobante cmp, con_periodo p, con_gestion g, con_moneda m, con_tipo_comprobante tc
                where cmp.cmp_tco_id= tc.tco_id and cmp.cmp_mon_id=m.mon_id and cmp.cmp_ges_id=g.ges_id and cmp.cmp_peri_id=pdo_id and cmp.cmp_eliminado='No'
				and left(cmp.cmp_fecha_cre,10)=CURRENT_DATE()";
		}
...
//        $conversor = new convertir();
        $atablas=array(
			...
			'venta_ajuste_saldos' => 'Ajuste de Saldos',
...
//        echo $sql;
    ...
    function datos() {
        if ($_POST) {
            $fecha_cmp=  FUNCIONES::get_fecha_mysql($_POST[comp_fecha]);
            $sql_sel = "select * from con_periodo where pdo_ges_id=$_SESSION[ges_id] and '$fecha_cmp'>=pdo_fecha_inicio and '$fecha_cmp'<=pdo_fecha_fin and pdo_eliminado='No' and pdo_estado='Abierto'";
            $periodo = FUNCIONES::objeto_bd_sql($sql_sel);
            $msj='';
            
            if($periodo){
                $_POST['cmp_peri_id']=$periodo->pdo_id;
            }else{
                $msj="<li>No existe un Periodo Activo en esta Gestion</li>";
                $sw=false;
            }
...
//            $num++;
            ...
            if ($val->validar($valores) && $sw) {
                ...
                $_j = 1;
                while ($_i <= $numfilas) {
                    if(isset($_POST['detalle_'.$_j])){
                        $fila=  json_decode($_POST['detalle_'.$_j]);
                        
                        $_POST['cuen'.$_j] = $fila->cuen;
                        $_POST['cuen_nombre'.$_j] = utf8_decode(base64_decode($fila->cuen_nombre));
                        $_POST['debe'.$_j] = $fila->debe;
                        $_POST['haber'.$_j] = $fila->haber;
                        $_POST['glosa'.$_j] = utf8_decode(base64_decode($fila->glosa));
                        $_POST['ca'.$_j] = $fila->ca;
                        $_POST['ca_nombre'.$_j] = utf8_decode(base64_decode($fila->ca_nombre));
                        $_POST['cf'.$_j] = $fila->cf;
                        $_POST['cf_nombre'.$_j] = utf8_decode(base64_decode($fila->cf_nombre));
                        $_POST['cc'.$_j] = $fila->cc;
                        $_POST['cc_nombre'.$_j] = utf8_decode(base64_decode($fila->cc_nombre));
                        $_POST['une'.$_j] = $fila->une;
                    }
                    $_i++;
                    $_j++;
                }
...
//                }
        ...
        <script>
            ...
            $('#btn-submit').click(function() {
                ...
                $("#generado_libro").children().remove();
                ...
                $("#cont_filas").val(inputContar);
                var detalles=$("#cmp_detalle tbody").children();
                $("#generado_detalle").children().remove();
                for(var i=0;i<detalles.size();i++){
                    var det=detalles[i];
                    var _id=$(det).attr(("data-id"));
                    
                    var fila={};
                    fila.cuen=$(det).find('#cuen'+_id).val();
                    fila.cuen_nombre=Base64.encode($(det).find('#cuen_nombre'+_id).val());
                    fila.debe=$(det).find('#debe'+_id).val();
                    fila.haber=$(det).find('#haber'+_id).val();
                    fila.glosa=Base64.encode($(det).find('#glosa'+_id).val());
                    fila.ca=$(det).find('#ca'+_id).val();
                    fila.ca_nombre=Base64.encode($(det).find('#ca_nombre'+_id).val());
                    fila.cf=$(det).find('#cf'+_id).val();
                    fila.cf_nombre=Base64.encode($(det).find('#cf_nombre'+_id).val());
                    fila.cc=$(det).find('#cc'+_id).val();
                    fila.cc_nombre=Base64.encode($(det).find('#cc_nombre'+_id).val());
                    fila.une=$(det).find('#une'+_id+' option:selected').val();
                    
                    $(det).find('#cuen'+_id).removeAttr('name');
                    $(det).find('#cuen_nombre'+_id).removeAttr('name');
                    $(det).find('#debe'+_id).removeAttr('name');
                    $(det).find('#haber'+_id).removeAttr('name');
                    $(det).find('#glosa'+_id).removeAttr('name');
                    $(det).find('#ca'+_id).removeAttr('name');
                    $(det).find('#ca_nombre'+_id).removeAttr('name');
                    $(det).find('#cf'+_id).removeAttr('name');
                    $(det).find('#cf_nombre'+_id).removeAttr('name');
                    $(det).find('#cc'+_id).removeAttr('name');
                    $(det).find('#cc_nombre'+_id).removeAttr('name');
                    $(det).find('#une'+_id).removeAttr('name');
                    
                    var str_fila=JSON.stringify(fila);
                    console.log(str_fila);
                    var hidden='<input type="hidden" name="detalle_'+_id+'" '+"value='"+str_fila+"'>";
                    $("#generado_detalle").append(hidden);
                }
//                return false;
...
//                var as_json = new bsn.AutoSuggest(inputVisible, options);
    ...
    function formulario_detalle(){
        ...
        <div hidden="" id="generado_detalle"></div>
        <div hidden="" id="generado_libro"></div>
        <?php
            $ges_id=$_SESSION['ges_id'];
...
//        $cmp_peri_id = $_POST['cmp_peri_id'];
        ...
        $cmp_ges_id = FUNCIONES::gestion_id($cmp_fecha);
        $cmp_peri_id = FUNCIONES::obtener_periodo($cmp_fecha);
...
//            echo "con j: ".$_j."<br>";
            if(isset($_POST['detalle_'.$_j])){
                $cde_secuencia = $_i;
                $str_detalle=$_POST['detalle_'.$_j];
                $str_detalle=str_replace('\"', '"', $str_detalle);
//                echo "-- $str_detalle<br>";  
                
                $fila=  json_decode($str_detalle);
                
                $cde_can_id = $fila->ca*1; //(isset($_POST['ca' . $_j]) && $_POST['ca' . $_j] != "") ? $_POST['ca' . $_j] : 0;
                $cde_cco_id = $fila->cc*1; //(isset($_POST['cc' . $_j]) && $_POST['cc' . $_j] != "") ? $_POST['cc' . $_j] : 0;
                $cde_cfl_id = $fila->cf*1; //(isset($_POST['cf' . $_j]) && $_POST['cf' . $_j] != "") ? $_POST['cf' . $_j] : 0;
                $cde_cue_id = $fila->cuen*1; //(isset($_POST['cuen' . $_j]) && $_POST['cuen' . $_j] != "") ? $_POST['cuen' . $_j] : 0;
                $cde_glosa =utf8_decode(base64_decode($fila->glosa)); //$_POST['glosa' . $_j];
                $debe = $fila->debe*1; //$_POST['debe' . $_j];
                $haber = $fila->haber*1; //$_POST['haber' . $_j];
                $une_id  = $fila->une*1;//$_POST['une' . $_j];
                $valor = 0;
                if ($debe >0) {
...
//        _PRINT::pre($objeto);
		...
		if ($_SESSION[id] == 'admin') {
			if (isset($_GET[set_moneda])) {
				$mon_id = $_GET[mon_ver];
			}
		}
//        echo "-- $objeto->mon_id -- <br>";
//        $mon_id=$objeto->mon_id;
        ...
        if(!isset($_GET['info'])){
            echo '<table align=right border=0>
                ...
                '<td>
                <img src="images/back.png" align="right" width="20" border="0"  title="VOLVER" onclick="javascript:location.href=\'gestor.php?mod='.$_GET[mod].'&tarea=ACCEDER\';">
                </td></tr></table>';
        }else{
            $editar=""; 
            if($objeto->cmp_tabla==""){
...
//                echo $activos_disp;
        ...
        <div id="contenido_reporte" style="clear:both; ">
            ...
            <style>
                ...
                .tab_detalle thead, .tab_detalle tbody{
                    ...
                    <table class="tab_detalle">
                        <thead >
                            <tr >
                                ...
                                <th width="15%" ><div>C&Oacute;DIGO</div></th>
                                <th width="55%" ><div>CUENTA</div></th>
                                <th width="15%" ><div>DEBE</div></th>
                                <th width="15%" ><div>HABER</div></th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php                            
