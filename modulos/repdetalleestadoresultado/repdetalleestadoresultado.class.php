...
class REPDETALLEESTADORESULTADO extends BUSQUEDA 
{
	var $formulario;
	var $mensaje;
	
	function REPDETALLEESTADORESULTADO()
	{
		$this->coneccion= new ADO();
		
		$this->link='gestor.php';
		
		$this->modulo='repdetalleestadoresultado';
		
		$this->formulario = new FORMULARIO();
		
		$this->formulario->set_titulo('DETALLE DE CUENTAS');
	}
	...
	function formulario()
	{
	 ...
	 <?
		...
		?>
						...
						<form id="frm_sentencia" name="frm_sentencia" action="gestor.php?mod=repdetalleestadoresultado" method="POST" enctype="multipart/form-data">  
								...
								<span class="flechas1">(DD/MM/AAAA)</span>
										   ...
										   <div id="CajaInput">
											...
											<div id="ContenedorDiv">
											   ...
											   <div id="CajaInput">
											   <select name="int_id" id="int_id" class="caja_texto">
											   <option value="">Seleccione</option>
											   <?php 		
												$fun=NEW FUNCIONES;		
	...
	{	
	    ...
	    $conec= new ADO();
	    $sql = "SELECT cmp_id, CONCAT(int_apellido,' ',int_nombre) as persona, cco_id, cco_descripcion,cmp_interesado,cmp_mon_id,cmp_tc,cmp_fecha,ABS(cde_monto) as monto,cde_glosa,ABS(cde_monto_sus) as montosus
				...
				WHERE  cmp_estado = 1 and cde_cue_id =$cue_id".$aux.$centro.$fecha."
				order by cmp_fecha
				"; 
		//echo $sql;
	    $conec->ejecutar($sql);		
	...
	{		
		...
		if (isset($_POST['tco_id']) && !empty($_POST['tco_id']))
		{
		    $sql = "SELECT cue_id, cue_numero, cue_descripcion, cue_padre_id,cue_tcu_id
					FROM cuenta
					WHERE cue_tcu_id = ".$_POST['tco_id']."  and cue_nivel = 3 and cue_id in (".$cuenta.") 
					ORDER BY cue_numero"; 
		}
		else
		{
		    $sql =  "SELECT cue_id, cue_numero, cue_descripcion, cue_padre_id,cue_tcu_id
					FROM cuenta
					WHERE cue_tcu_id <= 2 and cue_nivel = 3 and cue_id in (".$cuenta.")
					ORDER BY cue_numero"; 
