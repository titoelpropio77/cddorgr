<?php
class CON_REP_CMP_RESULTADO extends BUSQUEDA {
    var $formulario;
    var $mensaje;

    function CON_REP_CMP_RESULTADO() {
        $this->coneccion = new ADO();
        $this->link = 'gestor.php';
        $this->modulo = 'con_rep_cmp_resultado';
        $this->formulario = new FORMULARIO();
        $this->formulario->set_titulo('RESULTADO DE CUENTAS TITULO');
        ...
        if (!($_POST['formu'] == 'ok')) {
            ...
            <script>
                $("#frm_sentencia").live('submit', function() {
                    $(".error").remove();
                    var cuenta_id=$('#cuenta_id option:selected').val()*1;
                    if(!(cuenta_id>0)){
                        $.prompt('Seleccione la Cuenta Titulo');
                        return false;
                    }
...
//                    definir_filtro();
//                    complete_cuenta_cc();
                    cargar_cuentas_titulo();
                    $("#id_centroc").val('');
                    $("#nombre_centroc").val('');
                });

                function cargar_cuentas_titulo(){
                    var ges_id=$("#gestion option:selected").val();
                    $.get('AjaxRequest.php',{peticion:'ctitulos',ges_id:ges_id},function (resp){
                        var lista=JSON.parse(resp);
                        $('#cuenta_id option').remove();
                        var options='<option value=""></option>';
                        for(var i=0;i<lista.length;i++){
                            options+='<option value="'+lista[i].id+'">'+lista[i].nombre+'</option>';
                        }
                        $('#cuenta_id').append(options);
                    });
                }
            ...
            function barra_de_impresion() {
			  ...
			  <p>";
                ...
                echo '	<table align=right border=0><tr>
                      ...
                      </a></td><td><a><img src="images/excel.png" align="right" border="0" title="EXPORTAR EXCEL" id="importar_excel"></a></td><td><img src="images/back.png" align="right" width="20" border="0"  title="VOLVER" onclick="javascript:location.href=\'gestor.php?mod=' . $this->modulo . '\';"></td></tr></table><br><br>
		';
            }
...
//                            $sql = "SELECT * FROM con_cuenta WHERE cue_eliminado='No' and cue_ges_id='$ges_id' and cue_codigo='$form->cuenta';";
                            $sql = "SELECT * FROM con_cuenta WHERE cue_eliminado='No' and cue_id='$_POST[cuenta_id]';";
