...
class RESERVA extends BUSQUEDA {
    ...
    function RESERVA() {
        ...
        $this->arreglo_campos[$num]["texto"] = "Urbanizaciï¿½n";
        $this->arreglo_campos[$num]["tipo"] = "cadena";
        $this->arreglo_campos[$num]["tamanio"] = 40;
        $num++;
        ...
        $this->arreglo_campos[$num]["valores"] = "Habilitado,Deshabilitado,Expirado,Concretado:Habilitado,Deshabilitado,Expirado,Concretado";
        $num++;
...
//            $this->arreglo_opciones[$nun]["script"] = "ok";
    ...
    function dibujar_listado() {
        ...
        $sql = "SELECT 
			res_id,res_int_id,res_vdo_id,res_lot_id,res_fecha,res_hora,res_monto_referencial,res_moneda,res_estado,res_usu_id,int_nombre,int_apellido,urb_nombre,man_nro,lot_nro,lot_tipo,uv_nombre 
			FROM 
			reserva_terreno 
...
<!--            <th>Monto Referencial</th>
    ...
    function mostrar_busqueda() {
        ...
        for ($i = 0; $i < $this->numero; $i++) {
            ...
            echo "</td>";
//            echo "<td>";
//            echo $objeto->res_monto_referencial;
//            echo "</td>";
            
//            echo "<td>";
//            echo number_format($this->obtener_total_anticipos_reserva($objeto->res_id),2,'.',',');
//            echo "</td>";
...
//                $sql = "insert into reserva_terreno (res_int_id,res_vdo_id,res_lot_id,res_fecha,res_hora,res_anticipo,res_moneda,res_estado,res_usu_id,res_plazo_fecha) values ('" . $_POST['ven_int_id'] . "','" . $_POST['vendedor'] . "','" . $id_lote . "','" . date('Y-m-d') . "','" . date('H:i') . "','" . $_POST['ven_anticipo'] . "','" . $_POST['ven_moneda'] . "','Habilitado','" . $this->usu->get_id() . "','" . $_POST['fecha_plazo_final'] . "')";
                $sql = "insert into reserva_terreno (res_int_id,res_vdo_id,res_lot_id,
                                            ...
                                            values ('" . $_POST['ven_int_id'] . "','" . $_POST['vendedor'] . "','" . $id_lote . "',
                                                    ...
                                                    '$_POST[vendedor2]','$_POST[ven_urb_id]')";
...
//if (strlen($num) > 14) die("El n?mero introducido es demasiado grande"); 
    ...
    function nota_reserva($res_id){
        ...
        $conec = new ADO();
            $sql = "select 
				res_fecha,res_usu_id,res_moneda,res_plazo_fecha,res_plazo_hora,res_monto_referencial,res_vdo_id,(zon_precio*lot_superficie)as monto,res_int_id,res_estado,int_nombre,int_apellido,
				urb_nombre,man_nro,lot_nro,zon_precio,lot_superficie,zon_nombre,uv_nombre		
				from 
				reserva_terreno 
				inner join interno on(res_id = '$res_id' and res_int_id=int_id)
...
//                    $this->ver_comprobante($llave);
    ...
    //            $costo=  FUNCIONES::ad_parametro('par_cambio_titular');
                ...
                $this->mensaje = "Se realizó el cambio de Propietario";
                            ...
                            for ($i = 0; $i < $num; $i++) {
                                ...
                                ?>
                                    <tr class="busqueda_campos">
                                        ...
                                        <td >
                                            <?php
                                                if ($objeto->rph_cop_id <> 0) {echo '<b>Propietario:</b> ' . FUNCIONES::interno_nombre($objeto->rph_int_id) . ' - <b>Copropietario:</b> ' . FUNCIONES::interno_nombre($objeto->rph_cop_id);
                                                } else {echo '<b>Propietario:</b> ' . FUNCIONES::interno_nombre($objeto->rph_int_id);
                                                } 
                                            ?>
                                        </td>
                                        <td align="left">
                                            ...
                                            } else { echo '<b>Propietario:</b> ' . FUNCIONES::interno_nombre($objeto->rph_int_id_nuevo); 
                                            }
                                            ?>
                                        </td>
                        ...
                        <div id="ContenedorDiv">
                            ...
                            <div id="CajaInput">
                                <select name="vendedor_id" id="vendedor_id">
                                    ...
                                    <?php
                                    $fun=new FUNCIONES();
                                    $fun->combo("select vdo_id as id,concat(int_nombre,' ',int_apellido) as nombre
                                               from vendedor 
                                               inner join interno on (vdo_int_id=int_id) 
                                               where vdo_estado='Habilitado'", $reserva->res_vdo_id);
                                    ?>
                                </select>
