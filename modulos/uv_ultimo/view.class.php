...
class VIEWPLANO{
	...
	function plano_config($urb_id){
		$sql = "SELECT pla_urb_id,pla_tab_plano,pla_panel_left,pla_zoom,pla_marker,pla_img_width,pla_img_height,pla_imagen,urb_id,urb_nombre FROM urbanizacion INNER JOIN urbanizacion_plano_config ON (pla_urb_id=urb_id) WHERE urb_id='".$urb_id."'";
		$result = mysql_query($sql);
		$num = mysql_num_rows($result);
		if ($num > 0) 
		{
	...
	function plano()
	{
		...
		$datos->data = array(
			...
			'viewHtml' => array(
				'breadcrumb' => '<ul class="breadcrumb"><li><a href="#"data-cfm-map-link="'.$this->config->urb_id.'">'.$this->config->urb_nombre.'</a></li></ul>',
				'legend' => ''
			),
			'leyenda' => array(
				array(
	...
	{
		$arr = array();
		$sql = "SELECT man_id,man_nro, man_img_x, man_img_y, CAST(man_nro as signed) as numero  FROM manzano WHERE 	man_urb_id='".$_GET['u']."' and man_img_x !='' && man_img_y !='' ORDER BY numero ASC"; //ORDER BY numero ASC
		$result = mysql_query($sql);
		$num = mysql_num_rows($result);
		if ($num > 0) 
		{
	...
	function loteInfo()
	{
		...
		$sql="
			select lot_nro,lot_superficie,lot_estado,man_nro,urb_nombre, zon_precio, uv_nombre, zon_moneda,zon_nombre
			from lote 
			inner join manzano on (lot_man_id=man_id)
		...
		$html = "<div class='loteInfo'>";
			...
			$html .= "<b>Superficie: </b> ".$objeto->lot_superficie."<br/>";
			
			if($this->acceso)
			{
				$html .= "<b>Precio: </b> ".number_format($precio, 2, ".", ",")." ".$sufijo."<br/>";
			}
