...
class VENTA extends BUSQUEDA {
	...
	function VENTA(){
		...
		$num++;
                ...
                $this->arreglo_campos[$num]["tamanio"] = 40;
		$num++;
		$this->arreglo_campos[$num]["nombre"]="urb_nombre";
        ...
        function concretar_venta_por_reserva() {
            ...
            if ($reserva->res_estado == 'Habilitado') {
                ...
                $mensaje = 'No puede realizar la Venta, por que la Reserva que pertenece al Terreno seleccionado ya que no se encuentra en estado <b>Habilitado</b>.';
                $this->formulario->ventana_volver($mensaje, 'gestor.php.?mod=reserva&tarea=ACCEDER');
            }
        }
    ...
    //        echo "la reserva es: " . $reserva;
            ...
            $sql = "SELECT 		
                            res_id,res_int_id,res_vdo_id,res_lot_id,res_fecha,res_hora,res_estado,res_usu_id,int_nombre,int_apellido,urb_id,urb_nombre,man_id,man_nro,lot_id,lot_nro,lot_tipo,uv_id,uv_nombre,
...
//                FUNCIONES::print_pre($comprobante);
	...
	function dibujar_listado(){
			...
			ven_monto,ven_id,ven_fecha,ven_tipo,ven_moneda,ven_estado,
                        ...
                        ven_cuota_inicial,ven_res_anticipo,ven_monto,ven_numero,ven_devuelto,ven_monto_intercambio,ven_bloqueado
			FROM 
			venta 
		...
		for($i=0;$i<$this->numero;$i++)
                {				
                        ...
                        if($objeto->ven_estado=='Anulado'){
                            $operaciones[]="RETENER";$operaciones[]="AUTORIZACION";$operaciones[]="PAGOS";$operaciones[]="FUSION";
                            $operaciones[]="CAMBIAR PROPIETARIO";$operaciones[]="DEVOLVER";$operaciones[]="VER DEVOLUCION";
                            $operaciones[]="CAMBIAR LOTE";$operaciones[]="REFORMULAR";$operaciones[]="SEGUIMIENTO";
                            $operaciones[]="EXTRACTOS";$operaciones[]="ANULAR";$operaciones[]="FECHA VALOR";
                            $operaciones[]="ACTIVAR";
...
//                                echo "&nbsp;</td>";
                                echo "<td>";
                                    echo "<span style='padding:1px 3px ;color:#fff; background-color:{$a_est[$objeto->ven_estado]}'>$objeto->ven_estado</span>";
                                echo "&nbsp;</td>";
                                        ...
                                        <div id="ContenedorDiv">
                                            ...
                                            <div id="CajaInput">
                                                ...
                                                if($tipo!="reserva"){
                                                    ...
                                                    $sql="select urb_id as id,urb_nombre as nombre, urb_interes_anual from urbanizacion where urb_ventas_internas='Si' and urb_id=".$_POST['ven_urb_id'];
                                                    $urbanizacion=FUNCIONES::objeto_bd_sql($sql);
                                                    ?>
                                                        <select style="width:200px;" name="ven_urb_id" id="ven_urb_id" class="caja_texto" >
        ...
        function insertar_tcp() {
            ...
            if (!$loteven &&( ($lote->lot_estado == 'Disponible') || ($frm_reserva && $lote->lot_estado = 'Reservado')) ){
                ...
                if ($_POST['ven_tipo'] == "Contado" || $_POST['ven_tipo'] == "Credito") {
                    ...
                    $monto_efectivo=$monto-$monto_intercambio-$ven_anticipo;
...
//        function insertar_comision($ven_id,$tipo ,$monto,$moneda, $fecha,$urb, &$conec=null){
        function insertar_comision($data, &$conec=null){
            ...
            if($par->vendedor){
                ...
                $referido = FUNCIONES::atributo_bd_sql("select concat(int_nombre,' ',int_apellido) as campo from vendedor, interno where vdo_int_id=int_id and vdo_id='$_POST[vendedor]'");
//                $urb=  FUNCIONES::objeto_bd_sql("select * from urbanizacion where urb_id=$urb_id");
//                if($urb->urb_tipo=='Interno'){
                    include_once 'clases/modelo_comprobantes.class.php';
                    include_once 'clases/registrar_comprobantes.class.php';
		...
		?>
			...
			<table border='0' style="font-size:12px;" width="100%"  cellpadding="5" cellspacing="0" >
				<tr>
				   ...
				   <tr>
				    <td colspan="2">
					<strong>Nro. de Venta: </strong> <?php echo $venta->ven_id;?> <br>
					...
					<strong>Moneda: </strong> <?php $moneda=$venta->ven_moneda; if($moneda=="1") echo "Bolivianos"; else echo "Dolares"; ?> <br>
					<?php
					...
					<?php if($venta->ven_observacion<>'') { ?>
                                            <strong>Observaci&oacute;n: </strong> <?php echo $venta->ven_observacion;?> <br>
					<?php } ?>					
				...
				<tbody>
                                    <?php if (!$venta->ven_lot_ids){?>
                                        <tr>
                                            ...
                                            <?php } ?>
                                            <?php if($venta->ven_tipo=='Credito'){?>
                                                <td><?php echo number_format($venta->ven_monto_efectivo, 2); ?></td>
...
//                                            echo $venta->ven_lot_ids.'<br>';
	...
	function barra_opciones($venta, $tarea="",$print=false){
					...
					for($i=0;$i<$num;$i++)
					{
						...
						echo '<tr>';
								...
								echo "&nbsp;</td>";
                                                                ...
                                                                echo "&nbsp;</td>";
								echo "<td>";
									if($objeto->ind_fecha_pago<> '0000-00-00' && $objeto->ind_fecha_pago!='') echo $conversor->get_fecha_latina($objeto->ind_fecha_pago);
								echo "&nbsp;</td>";
...
//		echo $tipo_venta. ' - '.$tipo_lote.' - '.$urb_id;
		if($tipo_venta=='Contado' && $tipo_lote=='Lote' )
			include('contratos/'.$urb_id.'_lote_contado.php');
		if($tipo_venta=='Credito' && $tipo_lote=='Lote' )
			include('contratos/'.$urb_id.'_lote_credito.php');
		if($tipo_venta=='Contado' && $tipo_lote=='Vivienda' )
			include('contratos/'.$urb_id.'_vivienda_contado.php');
		if($tipo_venta=='Credito' && $tipo_lote=='Vivienda' )
			include('contratos/'.$urb_id.'_vivienda_credito.php');
	}
...
////////////////////////////////-------------- MODULO DESBLOQUEO CUOTAS   ----------------------/////////////////////////////////////
                                        ...
                                        <div class="Etiqueta" ><span class="flechas1">*</span>Persona</div>
                                            <div id="CajaInput">
                                                <?php if ($personas <> 0) { ?>
                                                    ...
                                                    <?php } else { echo 'No se le asigno ninguna personas, para poder cargar las personas.'; } ?>
                                            </div>
                                    </div>
                                    <div id="ContenedorDiv">
                                        ...
                                        <div id="CajaInput">
                                            ...
                                            if ($personas <> 0) {
                                                ...
                                                <a style="float:left; margin:0 0 0 7px;float:right;display:inline;" href="#" onClick="reset_interno();">
                                                    <img src="images/borrar.png" border="0" title="BORRAR" alt="BORRAR"></a>
                                                <?php
                                            } else { echo 'No se le asigno ning?na personas, para poder cargar las personas.'; }
                                            ?>
                                        </div>
                                    </div>
                                    <div id="ContenedorDiv">
                            ...
                            for ($i = 0; $i < $num; $i++) {
                                ...
                                ?>
                                    <tr class="busqueda_campos">
                                        ...
                                        <td >
                                            <?php
                                                if ($objeto->vph_cop_id <> 0) {echo '<b>Propietario:</b> ' . FUNCIONES::interno_nombre($objeto->vph_int_id) . ' - <b>Copropietario:</b> ' . FUNCIONES::interno_nombre($objeto->vph_cop_id);
                                                } else {echo '<b>Propietario:</b> ' . FUNCIONES::interno_nombre($objeto->vph_int_id);
                                                } 
                                            ?>
                                        </td>
                                        <td align="left">
                                            ...
                                            } else { echo '<b>Propietario:</b> ' . FUNCIONES::interno_nombre($objeto->vph_int_id_nuevo); 
                                            }
                                            ?>
                                        </td>
    ...
    function modificar_firma_contrato(){
        ...
        $sql="update venta set ven_fecha_firma='$fecha' where ven_id='$_GET[id]'";
