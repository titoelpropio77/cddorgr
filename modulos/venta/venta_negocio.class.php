...
class VENTA_NEGOCIO extends VENTA {

    function VENTA_NEGOCIO() {
        parent::__construct();
    }
    ...
    function cambiar_lote() {
        ...
        if ($c_lotes->get_num_registros() > 0) {
            $this->formulario->ventana_volver('Existe un cambio pendiente a ser Pagado', $this->link . '?mod=' . $this->modulo, '', 'Error');
            return;
        }
...
//        $tipo=  FUNCIONES::objeto_bd_sql("select * from extra_pago_tipo where ept_id=$extra_pago_id");//ing_cambio_ubi
        ...
        $hoy = date('Y-m-d');
        $ahora = date('H:i');
        $nota = "Bloqueo temporal por cambio de lote.";
        $id_lote = $_POST[new_lot_id];
        /**/
        $sql_ins = "insert into bloquear_terreno (
            bloq_lot_id,bloq_fecha,bloq_hora,bloq_estado,
            bloq_usu_id,bloq_int_id,bloq_nota,bloq_vdo_id
            ) values (
            '$id_lote','$hoy','$ahora','Habilitado',
            '$_SESSION[id]','$venta->ven_int_id','$nota','0'
            )";
        $conec->ejecutar($sql_ins, TRUE, TRUE);
        $bloq_id = ADO::$insert_id;
                ...
                <div id="FormSent" style="width: 100%">
                    ...
                    <div id="ContenedorSeleccion">
                        ...
                        <div id="ContenedorDiv">
                            <div class="Etiqueta">Observacion:</div>
                            <div id="CajaInput">
                                <textarea id="observacion" rows="5" cols="60" name="observacion"><?php echo $objeto->vnot_observacion; ?></textarea>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    function listado_notas($venta) {
        ...
        <?php foreach ($notas as $not) { ?>
                    <tr>
                        ...
                        <td >
							<?php
							if ($_SESSION[id] == $not->vnot_usu_cre) {
							?>
...
//        echo "<br><br>";
    ...
    function eliminar_documento() {
        $vdoc_id = $_GET[vdoc_id];
        if ($vdoc_id) {
            $sql_up = "update venta_documento set vdoc_eliminado='Si' where vdoc_ven_id='$_GET[id]' and vdoc_id='$vdoc_id'";
            $conec = new ADO();
            $conec->ejecutar($sql_up);
    ...
    function guardar_documento() {
        ...
        move_uploaded_file($_FILES['archivo']['tmp_name'], "docs/venta/$nombre_archivo");
        $fecha_cre = date('Y-m-d H:i:s');
        $tdoc_id = $_POST[tdoc_id];
        $sql_insert = "insert into venta_documento(
                vdoc_ven_id,vdoc_tdoc_id,vdoc_archivo,vdoc_descripcion,vdoc_fecha_cre,vdoc_usuario,vdoc_eliminado
            )values(
    ...
    function frm_documento() {
        $this->formulario->dibujar_titulo('SUBIR ARCHIVO');
        ?>
        <form name="frm_sentencia" action="gestor.php?mod=venta&tarea=DOCUMENTOS&id=<?php echo $_GET[id]; ?>&op=add" method="post" enctype="multipart/form-data">
            <br>
            <input type="hidden" id="ticket" name="ticket" value="<?php echo Ticket::pedirTicket(); ?>">
            <div id="ContenedorDiv">
    ...
    function listado_documentos() {
        $this->formulario->dibujar_titulo('DOCUMENTOS DE VENTA');
        ?>
        <style>
            .del_doc{cursor: pointer;}
        </style>
        <div style="text-align: left; margin: 10px;">
            <input type="button" class="boton" value="Agregar" onclick="location.href = 'gestor.php?mod=venta&tarea=DOCUMENTOS&id=<?php echo $_GET[id] ?>&op=add';">
        </div>
        <table class="tablaLista" cellspacing="0" cellpadding="0">
            <thead>
                <tr>
            ...
            <tbody>
        <?php $documentos = FUNCIONES::objetos_bd_sql("select * from venta_documento,ter_tipo_documento where vdoc_tdoc_id=tdoc_id and vdoc_ven_id=$_GET[id] and vdoc_eliminado='No'"); ?>
        <?php for ($i = 0; $i < $documentos->get_num_registros(); $i++) { ?>
            <?php $doc = $documentos->get_objeto(); ?>
                    <tr data-venta="<?php echo $_GET[id]; ?>" data-id="<?php echo $doc->vdoc_id; ?>">
                        <td><?php echo $doc->vdoc_id; ?></td>
                        <td><?php echo $doc->tdoc_nombre; ?></td>
                        <td><?php echo $doc->vdoc_descripcion; ?></td>
                        <td><img src="images/files.png" style="height: 15px; float: left;margin-right: 2px;"><?php echo $doc->vdoc_archivo; ?></td>
                        <td><?php echo FUNCIONES::atributo_bd_sql("select concat(int_nombre,' ',int_apellido) as campo from ad_usuario,interno where usu_per_id=int_id and usu_id='$doc->vdoc_usuario'"); ?></td>
                        <td><a href="docs/venta/<?php echo $doc->vdoc_archivo; ?>" target="_blank">Descargar</a></td>
        ...
        <script>
                                     $('.del_doc').click(function() {
                                         ...
                                         $.prompt(txt, {
                                             ...
                                             callback: function(v, m, f) {
                                                 if (v) {
                                                     location.href = 'gestor.php?mod=venta&tarea=DOCUMENTOS&id=' + venta + '&op=del&vdoc_id=' + doc_id;
                                                 }
                                             }
                                         });
