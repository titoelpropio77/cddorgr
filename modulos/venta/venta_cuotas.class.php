...
class VENTA_CUOTAS extends VENTA {
    ...
    function extractos() {
        ...
        }elseif($acc=='ver_comprobante_contable'){
            $this->ver_comprobante_contable($_GET[cmp_id]);
        }else{
            $this->listado_extractos($venta);
        }
    }
    ...
    function anular_pago($pag_id, $origen = '') {
		...
		$bool2 = COMPROBANTES::anular_comprobante('venta_pago_desc_inc', $pag_id);
		$bool = $bool && $bool2;
        if(!$bool){
        ...
        $venta_producto =  FUNCIONES::objeto_bd_sql("select * from venta_producto 
            where vprod_ven_id='$pag->vpag_ven_id' and vprod_estado='Pendiente'");
        ...
        if($monto_pagado>0){
            ...
            // DESHACER PAGO DE COSTO PRODUCTO            
            $str_costo_prod_ids=trim($pag->vpag_costo_prod_ids);
            if($str_costo_prod_ids && $pag->vpag_costo_prod>0){
                                
                if ($venta->ven_costo_producto > 0) {                    
                    $sql_cuotas = "select * from interno_deuda where ind_id in ($str_costo_prod_ids) and ind_tipo='pcuota' order by ind_id asc";
                } else {
                    if ($venta_producto) {
                        $sql_cuotas = "select idpr_id as ind_id,
                        idpr_num_correlativo as ind_num_correlativo from interno_deuda_producto 
                        where idpr_id in ($str_costo_prod_ids) and idpr_tipo='pcuota' order by ind_id asc";
                    }
                }
                
//                $cuotas=  FUNCIONES::lista_bd_sql("select * from interno_deuda where ind_id in ($str_costo_prod_ids) and ind_tipo='pcuota' order by ind_id asc");
                $cuotas=  FUNCIONES::lista_bd_sql($sql_cuotas);
                $costo_ids = explode(',', trim($pag->vpag_costo_prod_ids));
                $costo_montos = explode(',', trim($pag->vpag_costo_prod_montos));
                if (count($costo_ids) ==  count($cuotas) && count($cuotas)==  count($costo_montos)){
                    $costo_cub=$costos_pag=0;
                    for ($i = 0; $i < count($cuotas); $i++) {
                        $cu=$cuotas[$i];
                        ...
                        if ($venta->ven_costo_producto > 0) {                    
                            $sql="update interno_deuda set 
                                    ind_costo_pagado_producto=ind_costo_pagado_producto-$costo_pagado
                                where ind_id = '$cu->ind_id'
                            ";
                        } else {
                            if ($venta_producto) {
                                $sql="update interno_deuda_producto set 
                                        idpr_costo_pagado=idpr_costo_pagado-$costo_pagado
                                    where idpr_id = '$cu->ind_id'
                                ";
                            }
                        }
...
//                            ";
                    ...
                    if ($venta->ven_costo_producto > 0) {
                        ...
                        set ven_costo_producto_cub=ven_costo_producto_cub-$costo_cub, 
                        ven_costo_producto_pag=ven_costo_producto_pag-$costos_pag 
                        where ven_id=$venta->ven_id";
                    } else {
                        if ($venta_producto) {
                            $sql_up_ven_costo="update venta_producto 
                            set vprod_costo_cub=vprod_costo_cub-$costo_cub, 
                            vprod_costo_pag=vprod_costo_pag-$costos_pag 
                            where vprod_id='$venta_producto->vprod_id'";
                        }                        
                    }
...
//                    where ven_id=$venta->ven_id";
            ...
            $str_cap_prod_ids=trim($pag->vpag_capital_prod_ids);
            if($str_cap_prod_ids){
                $cuotas=  FUNCIONES::lista_bd_sql("select * from interno_deuda_producto 
                where idpr_id in ($str_cap_prod_ids) and idpr_tipo='pcuota' 
                order by idpr_id asc");
                
                $cap_prod_ids = explode(',', trim($pag->vpag_capital_prod_ids));
                $cap_prod_montos = explode(',', trim($pag->vpag_capital_prod_montos));
                
                if (count($cap_prod_ids) ==  count($cuotas) && count($cuotas)==  count($cap_prod_montos)){
                    for ($i = 0; $i < count($cuotas); $i++) {
                        
                        $cu = $cuotas[$i];
                        $cap_prod_pagado = $cap_prod_montos[$i];
                        
                        $sql = "update interno_deuda_producto set 
                        idpr_estado='Pendiente',    
                        idpr_capital_pagado=idpr_capital_pagado-$cap_prod_pagado,
                        idpr_monto_pagado=idpr_interes_pagado+idpr_capital_pagado,
                        idpr_saldo_final=0    
                        where idpr_id = '$cu->idpr_id'
                            ";
                        $conec->ejecutar($sql);
                    }
                }else{
                    $conec->rollback();
                    $mensaje = "Forulario: Cantidad de cuotas de Producto diferente a la cantidad que existe en la base de datos o la cantidad y montos es diferente";
            ...
            $str_int_prod_ids=trim($pag->vpag_interes_prod_ids);
            if($str_int_prod_ids){
                $cuotas=  FUNCIONES::lista_bd_sql("select * from interno_deuda_producto 
                where idpr_id in ($str_int_prod_ids) and idpr_tipo='pcuota' 
                order by idpr_id asc");
                
                $int_prod_ids = explode(',', trim($pag->vpag_interes_prod_ids));
                $int_prod_montos = explode(',', trim($pag->vpag_interes_prod_montos));
                
                if (count($int_prod_ids) ==  count($cuotas) && count($cuotas)==  count($int_prod_montos)){
                    for ($i = 0; $i < count($cuotas); $i++) {
                        
                        $cu = $cuotas[$i];
                        $int_prod_pagado = $int_prod_montos[$i];
                        
                        $sql = "update interno_deuda_producto set 
                        idpr_estado='Pendiente',    
                        idpr_interes_pagado=idpr_interes_pagado-$int_prod_pagado,
                        idpr_monto_pagado=idpr_interes_pagado+idpr_capital_pagado,
                        idpr_saldo_final=0    
                        where idpr_id = '$cu->idpr_id'
                            ";
...
//        $det_body=array($objeto->vpag_interes,$objeto->vpag_capital,$objeto->vpag_form,$objeto->vpag_monto);
        ...
        if($objeto->vpag_capital_prod>0){
            $det_cabecera[]='Capital Prod.';
            $det_body[]=$objeto->vpag_capital_prod;
        }
        ...
        if($objeto->vpag_capital_prod>0){
            $det_cabecera[]='Nro Cuotas Prod.';
            $det_body[]=$this->str_numeros_cuotas_producto($objeto->vpag_capital_prod_ids);
        }
        ...
        $tot_cap_prod_ap = 0;
        if ($venta_producto) {
            $pagado_prod = $this->total_pagado_producto($venta_producto->vprod_id);
            $tot_cap_prod_ap = $pagado_prod->capital;
            $saldo_prod = $venta_producto->vprod_monto_efectivo - $tot_cap_prod_ap;
        }
//        if($venta->ven_urb_id=='4'){
            $det_cabecera[]='Total Aportado';
//            $det_body[]=$pagado->capital + $tot_cap_prod_ap;
            $det_body[]=$pagado->capital;
//        }
        if ($venta_producto) {
            $det_cabecera[]='Total Aportado Prod.';
//            $det_body[] = $pagado_prod->capital + $venta_producto->vprod_anticipo;
            $det_body[] = $pagado_prod->capital;
        }
//        if($venta->ven_urb_id=='4'){
            $det_cabecera[]='Saldo Deudor';
            $det_body[]=$objeto->vpag_saldo_final;
//        }
        ...
        if ($venta_producto) {
            $det_cabecera[]='Saldo Deudor Prod.';
            $det_body[] = $saldo_prod;
        }

        $sql_dp = "select * from debito inner join debito_periodo on (deb_dp_id=dp_id)
            where deb_estado='Activo' and deb_vpag_id='$objeto->vpag_id'";
        $debito_periodo = FUNCIONES::objeto_bd_sql($sql_dp);
        
        $desc_dp = "";
        if ($debito_periodo) {
            $desc_dp .= $debito_periodo->dp_descripcion;
        }
        ...
        $data=array(
            ...
            'nro_recibo'=> $objeto->vpag_recibo,
            'fecha'=> $objeto->vpag_fecha_pago,
            'concepto'=> $desc_dp . " - Pago de la Venta Nro. $venta->ven_id - " . $venta->ven_concepto,
            'has_detalle'=>'1',
            'det_cabecera'=>$det_cabecera,
            'det_body'=>$det_body,
            'nota'=>"El proximo pago en fecha $txt_nfecha, debera pagar un monto de $par->monto (interes: $par->interes, capital:$par->capital, Formulario: $par->form)",
...
//        echo "</pre>";
        ...
        $sql_pago = "insert into venta_pago(
                        ...
                        vpag_val_monto,vpag_capital_prod,vpag_capital_prod_ids,vpag_capital_prod_montos,vpag_vprod_id,
                        vpag_interes_prod,vpag_interes_prod_ids,vpag_interes_prod_montos
                    )values(
                        ...
                        '$val_monto','$cob->vcob_capital_prod','$cob->vcob_capital_prod_ids','$cob->vcob_capital_prod_montos','$cob->vcob_vprod_id',
                        '$cob->vcob_interes_prod','$cob->vcob_interes_prod_ids','$cob->vcob_interes_prod_montos'
                        )";
...
//        echo "</pre>";
        ...
        if ($cob->vcob_capital_prod > 0) {
            $res_prod = $this->pagar_cu_producto($data_pago, $conec);
        }        
        $ucuota=  FUNCIONES::objeto_bd_sql("select * from interno_deuda where ind_tabla='venta' and ind_tabla_id=$venta->ven_id and ind_capital_pagado>0 order by ind_id desc limit 1");
...
//        '$cob->vcob_interes' ,'$cob->vcob_capital','$cob->vcob_mora','$cob->vcob_form','$cob->vcob_monto'
        $data=array(
            ...
            'capital_producto'=>$cob->vcob_capital_prod,
            'costo_producto_separado' => ($cob->vcob_capital_prod > 0) ? $res_prod->costo : 0,
            'form'=>$cob->vcob_form,
...
//                    $interes_dif = $vpag_interes - ($cu->ind_interes * 1);
    ...
    function pagar_cu_capital($data,&$conec=null){
        ...
        $costo_ids=array();
        $costo_montos=array();
        $costo_prod_ids=array();
        $costo_prod_montos=array();
        
        $str_capital_ids = trim($cob->vcob_capital_ids);
        /*AQUI ME QUEDE... CALCULANDO EL SALDO DEL COSTO DE VENTA..*/
        $saldo_venta = $venta->ven_monto_efectivo;
        $saldo_venta_prod = $venta->ven_monto_producto;
        $saldo_costo = $venta->ven_costo - $venta->ven_costo_cub;

        $prorat_lote = ($venta->ven_monto_efectivo / ($saldo_venta + $saldo_venta_prod));
        $prorat_prod = ($venta->ven_monto_producto / ($saldo_venta + $saldo_venta_prod));
        if ($str_capital_ids && $cob->vcob_capital > 0) {
            $cuotas = FUNCIONES::lista_bd_sql("select * from interno_deuda where ind_id in ($str_capital_ids) and ind_tipo='pcuota' order by ind_id asc");
...
//                            echo " ENTRO TERCER IF <br>";
                    ...
                    $conec->ejecutar($sql_ins_idp);
                    // $saldo_costo=$venta->ven_costo-$venta->ven_res_anticipo-$venta->ven_monto_intercambio-$venta->ven_cuota_inicial-$venta->ven_costo_cub;
					// $saldo_costo = $venta->ven_costo - ($venta->ven_res_anticipo + $venta->ven_monto_intercambio);	-- 	COMENTADO: HRP - 13/06/2019
                    $costo_pagado=0;
                    
                    $saldo_costo_producto = $venta->ven_costo_producto;
                    $costo_pagado_producto = 0;
                    
                    if($saldo_costo + $saldo_costo_producto > 0){
                        if($cu->ind_num_correlativo==0){
                            $costo_pagado = $capital_pagado * $prorat_lote;
                            $costo_cub = $costo_pagado;
                            
                            $costo_pagado_producto = $capital_pagado * $prorat_prod;
                            $costo_prod_cub = $costo_pagado_producto;
                        }else{
                            if($cob->vcob_saldo_final<=0 && $i == count($cuotas)-1){
//                                $costo_pagado=$venta->ven_costo-$venta->ven_costo_cub-$venta->ven_costo_pag-$costo;
                                $costo_pagado = $venta->ven_costo - ($venta->ven_costo_cub + $venta->ven_costo_pag + $costo);
                                $costo_pagado_producto = $venta->ven_costo_producto - ($venta->ven_costo_producto_cub + $venta->ven_costo_producto_pag + $costo_producto);
                            }else{
                                $costo_pagado = round(($capital_pagado * $prorat_lote * $saldo_costo) / $saldo_venta, 2);
                                if ($saldo_venta_prod > 0) {
                                    $costo_pagado_producto = round(($capital_pagado * $prorat_prod * $saldo_costo_producto) / $saldo_venta_prod, 2);
                                }
                            }
                            $costo_pag += $costo_pagado;
                            $costo_prod_pag += $costo_pagado_producto;
                        }
                        $costo += $costo_pagado;
                        $costo_producto += $costo_pagado_producto;
                        $costo_ids[] = $cu->ind_id;
                        $costo_montos[] = $costo_pagado;
                        
                        if ($costo_pagado_producto > 0) {
                            $costo_prod_ids[] = $cu->ind_id;
                            $costo_prod_montos[] = $costo_pagado_producto;
                        }
                    }
                    if($costo_pagado>0){
                        $sql_ins_idp = "insert into interno_deuda_pago(
                    ...
                    if($costo_pagado_producto > 0){
                        $sql_ins_idp = "insert into interno_deuda_pago(
                                            ...
                                            '$cu->ind_id','$cu->ind_num_correlativo','$pago_id','$recibo','$pag_codigo','costo_producto',
                                            '$fecha_pago','$costo_pagado_producto','Activo','$fecha_cre','$usuario_id',
                                            '$cob->vcob_usu_cre','$cob->vcob_codigo','$cob->vcob_aut'
                                        )
                            ";
                        $conec->ejecutar($sql_ins_idp);
                    ...
                    $sql = "update interno_deuda set 
                                ...
                                ind_monto_pagado=ind_interes_pagado+ind_capital_pagado,
                                ind_costo_pagado=ind_costo_pagado+$costo_pagado,
                                ind_costo_pagado_producto=ind_costo_pagado_producto+$costo_pagado_producto,    
...
//                    echo $sql.';<br>';
                ...
                $txt_costo_prod_ids=  implode(',', $costo_prod_ids);
                $txt_costo_prod_montos=  implode(',', $costo_prod_montos);
                $sql_up_pag_costo="update venta_pago 
                set vpag_costo='$costo', 
                vpag_costo_prod='$costo_producto',     
                vpag_costo_ids='$txt_costo_ids', 
                vpag_costo_montos='$txt_costo_montos',
                vpag_costo_prod_ids='$txt_costo_prod_ids', 
                vpag_costo_prod_montos='$txt_costo_prod_montos'    
                where vpag_id=$pago_id";
                ...
                $sql_up_ven_costo="update venta 
                set ven_costo_cub=ven_costo_cub+$costo_cub, 
                ven_costo_producto_cub=ven_costo_producto_cub+$costo_prod_cub,   
                ven_costo_pag=ven_costo_pag+$costo_pag, 
                ven_costo_producto_pag=ven_costo_producto_pag+$costo_prod_pag     
                $set_ven_estado
                where ven_id=$venta->ven_id";
                $conec->ejecutar($sql_up_ven_costo);
...
//            return true;
    ...
    function calcular_datos_cobro_casa($ven_prod, $fecha_pago){
        $sql_cuotas_capital="select * from interno_deuda_producto 
        where idpr_tabla='venta_producto' and idpr_tabla_id=$ven_prod->vprod_id and 
        idpr_fecha_programada<='$fecha_pago' and idpr_estado='Pendiente' and
        idpr_capital_pagado < idpr_capital and idpr_tipo='pcuota' order by idpr_id asc
        ";
//        echo "<p>sql_cuotas_capital_prod:$sql_cuotas_capital</p>";
        ...
        $interes_montos = array();
        foreach ($cuotas_capital as $cu) {
            $mcapital = $cu->idpr_capital - $cu->idpr_capital_pagado;            
            $capital += $mcapital;            
            $capital_ids[] = $cu->idpr_id;
            $capital_montos[] = $mcapital;            
            
            
            if ($cu->idpr_interes > 0) {
                $minteres = $cu->idpr_interes - $cu->idpr_interes_pagado;
                $interes += $minteres;
                $interes_ids[] = $cu->idpr_id;
                $interes_montos[] = $minteres;
            }
        }
                        
        $resp = new stdClass();
        $resp->capital = $capital;
        $resp->capital_ids = implode(',', $capital_ids);
        $resp->capital_montos = implode(',', $capital_montos);
        
        if ($interes > 0) {
            $resp->interes = $interes;
            $resp->interes_ids = implode(',', $interes_ids);
            $resp->interes_montos = implode(',', $interes_montos);
        }
...
//        $venta_producto = FUNCIONES::objeto_bd_sql($sql_ven_prod);
        ...
        $vprod_id = NULL;
        if ($venta_producto) {
            $datos_cobro_casa = $this->calcular_datos_cobro_casa($venta_producto, $fecha_pago);
            $capital_prod = $datos_cobro_casa->capital;
            $capital_prod_ids = $datos_cobro_casa->capital_ids;
            $capital_prod_montos = $datos_cobro_casa->capital_montos;
            
            $interes_prod = $datos_cobro_casa->interes;
            $_interes = $interes_prod * 1;
            $interes_prod_ids = $datos_cobro_casa->interes_ids;
            $interes_prod_montos = $datos_cobro_casa->interes_montos;
            
            $monto += round($capital_prod + $_interes, 2);
            $vprod_id = $venta_producto->vprod_id;
        }
        
//        if($interes+$capital>0){
        if(($interes+$capital > 0) || ($capital_prod > 0)){
            $txt_interes_ids=implode(',', $interes_ids);
            $txt_capital_ids=implode(',', $capital_ids);
            $txt_mora_ids=implode(',', $mora_ids);
            $txt_mora_con_ids=implode(',', $mora_con_ids);
            ...
            $sql_cobro = "insert into venta_cobro(
                                ...
                                vcob_fecha_cre,vcob_usu_cre,vcob_aut,
                                vcob_capital_prod,vcob_capital_prod_ids,vcob_capital_prod_montos,vcob_vprod_id,
                                vcob_interes_prod,vcob_interes_prod_ids,vcob_interes_prod_montos
                            )values(
                                '$venta->ven_id','$codigo','$par->fecha_pago','$fecha_valor',
                                '$venta->ven_int_id','$venta->ven_moneda','$saldo_inicial','$dif_dias',
                                ...
                                '$fecha_cre','$usuario_id','0',
                                '$capital_prod','$capital_prod_ids','$capital_prod_montos','$vprod_id',
                                '$interes_prod','$interes_prod_ids','$interes_prod_montos'
                            )";
            //echo $sql_cobro.';<br>';
            $conec->ejecutar($sql_cobro);
            return true;
...
//                        return false;
        ...
        <form id="frm_sentencia" name="frm_sentencia" enctype="multipart/form-data" method="POST" action="gestor.php?mod=venta&tarea=PAGOS&id=<?php echo $venta->ven_id?>&acc=pagar" >
            <div id="FormSent" style="width:80%">
                ...
                <div id="ContenedorSeleccion">
                    ...
                    <?php
                    if ($cobro->vcob_interes_prod > 0) {
                        ?>
                    <div id="ContenedorDiv">
                        <div class="Etiqueta">Interes Producto</div>
                        <div id="CajaInput">
                            <div class="read-input"><?php echo $cobro->vcob_interes_prod;?></div>
                        </div>
                    </div>
                        <?php
                    }
                    ...
                    <?php
                    if ($cobro->vcob_capital_prod > 0) {
                        ?>
                    <div id="ContenedorDiv">
                        <div class="Etiqueta">Capital Producto</div>
                        <div id="CajaInput">
                            <div class="read-input"><?php echo $cobro->vcob_capital_prod;?></div>
                        </div>
                    </div>
                        <?php
                    }
                    ...
                    <div id="ContenedorDiv">
                        ...
                        <div id="CajaInput">
                            <div class="read-input"><?php echo $cobro->vcob_capital+$cobro->vcob_interes+$cobro->vcob_form + $cobro->vcob_capital_prod;?></div>
                        </div>
                    </div>
                    <div id="ContenedorDiv">
    ...
    function listado_extractos($venta){
            ...
            $arr_usuarios_bajos = array('admin2','arturo','elopez','mpedraza','jrivero','mvega','lhurtado');
            $has_permiso_ver_cmp = $has_permiso_ver_cmp || in_array($_SESSION[id], $arr_usuarios_bajos);
            
            $venta_producto = FUNCIONES::obtener_producto($venta->ven_id);
			$saldo_producto = ($venta_producto) ? $venta_producto->vprod_monto_efectivo:0;
							
			if ($_SESSION[id] == 'admin') {
				// FUNCIONES::print_pre($venta_producto);
				// echo "<p style='color:red;'>saldo_producto:$saldo_producto</p>";
			}
            ?>
            <style>
...
//                    var ruta='gestor.php?mod=con_comprobante&info=ok&tarea=VER&id=';
                    var ruta='gestor.php?mod=venta&tarea=EXTRACTOS&info=ok&id=<?php echo $venta->ven_id;?>&acc=ver_comprobante_contable&cmp_id=';
                    window.open(ruta+id,'Comprobante','width=900, height=500, scrollbars=yes');
                    popup = window.open(ruta+id,'Comprobante','width=900, height=500, scrollbars=yes');
                    popup.document.close();
                }
            ...
            <table id="tabla_extracto"  width="100%"  class="tablaReporte" cellpadding="0" cellspacing="0">
                <thead>
                    <tr>
                        <th >#</th>
						<th >Pag. Id</th>
                        ...
                        if ($venta_producto) {
                        ?>
                        <th >Interes<br>Producto</th>
                        <th >Capital<br>Producto</th>
                        <?php
                        }
                        ?>
                        <th >Mora</th>
						...
						if ($_SESSION[id] == 'admin') {
						?>
						<th >Costo por Pago</th>
						<th >Costo Pag. Acum.</th>
						<?php
						}
						?>
                    </tr>	
                    ...
                    <tbody>
                        ...
                        <?php for ($i = 0; $i < $pagos->get_num_registros(); $i++) {?>
                            ...
                            <tr>
                                ...
                                if ($venta_producto) {
                                ?>
                                <td><?php echo number_format($pag->vpag_interes_prod, 2);?></td>
                                <td><?php echo number_format($pag->vpag_capital_prod, 2);?></td>
                                <?php
                                }
                                ?>
                                ...
                                <?php
                                if ($venta_producto) {
                                    $saldo_producto -= $pag->vpag_capital_prod;
                                ?>
                                <td><?php echo number_format($saldo_producto, 2);?></td>
                                <?php                                
                                }
                                ?>
                                ...
                                if ($venta_producto) {
                                ?>
                                <td><?php echo $this->str_numeros_cuotas($pag->vpag_capital_prod_ids,'producto');?></td>
                                <?php
                                }
                                ?>
                                ...
                                $total->interes_producto +=$pag->vpag_interes_prod;
                                $total->capital_producto += $pag->vpag_capital_prod;
...
//                                    echo "<p>vpag_ven_id:". $pag->vpag_venta_id . " - grupo:" . $this->obtener_grupo_id($this->usu->get_id()) . " - i:$i</p>";
								...
								<?php 
								if ($_SESSION[id] == 'admin') {									
									$tot_costo_pag += $pag->vpag_costo;
								?>
								<td ><?php echo number_format($pag->vpag_costo, 2, '.', ',');?></td>
								<td ><?php echo number_format($tot_costo_pag, 2, '.', ',');?></td>
								<?php									
								}
								?>
                            </tr>
                    ...
                    <tfoot>
                        <tr>
                            ...
                            if ($venta_producto) {
                            ?>
                            <td><?php echo number_format($total->interes_producto, 2);?></td>
                            <td><?php echo number_format($total->capital_producto, 2);?></td>
                            <?php
                            }
                            ?>
...
//                    popup.document.close();
    ...
    function ver_comprobante_contable($cmp_id){
        include_once 'modulos/con_comprobante/con_comprobante.class.php';
        $cmp = new con_comprobante();
        $cmp->imprimir_cmp($cmp_id);
    }
    
    function str_numeros_cuotas($str_ind_ids, $origen = '') {
        ...
        if ($origen == '') {
         ...
         $cuotas=  FUNCIONES::lista_bd_sql("select idpr_num_correlativo as ind_num_correlativo,
             idpr_venta_id as ind_venta_id,idpr_capital_desc as ind_capital_desc,
             idpr_capital_inc as ind_capital_inc
             from interno_deuda_producto where idpr_id in ($str_ind_ids) and idpr_tipo='pcuota' ");   
        }
         $li_cu=array();
...
//                $this->cabecera_venta_producto($venta_producto);
                ?>
            <center><h2>SEGUIMIENTO DE CUOTAS</h2>
...
//            }
            ...
            <div class="aTabsCont">
                <div class="aTabsCent" style="width: 100%">
                    <ul class="aTabs">
                        <?php if(TRUE){?>
                            <li><a data-tipo="lote" class="activo" >Plan de Pagos Lote</a></li>
                        <?php }?>
                        <?php if($venta_producto){?>
                            <li><a data-tipo="casa"  >Plan de Pagos Casa</a></li>
                        <?php }?>
            ...
            <script>
                $('.aTabs li a').click(function(){
                    ...
                    $('.box-plan').hide();
                    $('.box-plan-'+tipo).show();
                    $('.aTabs .activo').removeClass('activo');
                    $(this).addClass('activo');
                });
            </script>     
			<script type="text/javascript" src="js/jquery.thfloat-0.7.2.min.js"></script>
            <?php
            $this->listado_pagos_casa($venta);
            ?>
            <div class="box-plan box-plan-lote">    
            <br><br><center><h2>SEGUIMIENTO DE CUOTAS</h2>
            <table id="tabla_seguimiento"  width="100%"  class="tablaReporte" cellpadding="0" cellspacing="0">
                <thead>
                    <tr>		        	
                        <th >Id</th>
                        <th >Usuario</th>
...
//                        $tiene_intereses = FUNCIONES::tiene_intereses($venta->ven_id);
                ...
                <?php for($i=0;$i<$num;$i++){ ?>
                    ...
                    <tr>
						...
						<?php
						if ($_SESSION[id] == 'admin') {
							echo "<td>" . number_format($cu->ind_saldo, 2, '.', ',') . "</td>";
						}
						?>
...
//        return;
        ...
        if (isset($_POST[vprod_id]) && ($_POST[vprod_id]*1 > 0)) {
            $interes_prod = $_POST[interes_prod]*1;
            $capital_prod = $_POST[capital_prod]*1;
            $monto += $interes_prod + $capital_prod;
            
            $capital_prod_ids = $_POST[capital_prod_ids];
            $capital_prod_montos = $_POST[capital_prod_montos];
        }
...
//        $form_montos=$_POST[form_montos];
        ...
        $sql_ins = "insert into venta_cobro(
                        ...
                        '$capital_prod','$capital_prod_ids','$capital_prod_montos','$_POST[vprod_id]',
                        '$interes_prod','',''
                    )";
//        echo $sql_ins.'<br>';
        $conec->ejecutar($sql_ins);
    ...
    function frm_autorizar_cobro($venta) {
        ...
        if(!$upago){
            $upago=new stdClass();
            $upago->vpag_saldo_final=$venta->ven_monto_efectivo + $venta->ven_monto_producto;
...
//        $pagado=  $this->total_pagado($ven_id);
        ...
        $venta_producto = FUNCIONES::obtener_producto($ven_id);
        ?>
        <style>
            .img-boton{
        ...
        <form id="frm_sentencia" name="frm_sentencia" enctype="multipart/form-data" method="POST" action="gestor.php?mod=venta&tarea=AUTORIZACION&id=<?php echo $venta->ven_id?>" >
            ...
            if ($venta_producto) {
                ...
                $total_pagado_prod = $this->total_pagado_producto($venta_producto->vprod_id);                
                $saldo_producto = $venta_producto->vprod_monto - $total_pagado_prod->capital;
                                
            ?>
            <input type="hidden" name="saldo_prod" id="saldo_prod" value="<?php echo $saldo_producto;?>">
            <input type="hidden" name="vprod_id" id="vprod_id" value="<?php echo $venta_producto->vprod_id;?>">
            <?php
            }
            ?>
...
//                                $venta_producto = null;
                                if ($venta_producto) {
                                    ?>
                                <div id="ContenedorDiv">
                                    <div class="Etiqueta">Nro. de Cuotas Producto</div>
                                    <div id="CajaInput">
                                        <input type="text" size="20" name="sol_nro_cuotas_prod" id="sol_nro_cuotas_prod" value="" autocomplete="off">
                                    </div>
                                    <img src="images/enter.png" width="25" class="img-boton" id="enter-cuotas-prod">
                                </div>
                                <div id="ContenedorDiv">
                                    <div class="Etiqueta">Cuotas Producto</div>
                                    <div id="CajaInput">
                                        <table class="tablaReporte" id="list_cuotas_prod" width="100%" cellspacing="0" cellpadding="0">
                                            <thead>
                                                <tr>
                                                    <th>Nro</th>
                                                    <th>Fecha Prog.</th>
                                            ...
                                            <tfoot>
                                                <tr>
                                                    ...
                                                    <td>&nbsp;</td>
                                                    <td>
                                                        <input type="hidden" id="nro_cuota_sig_prod" name="nro_cuota_sig_prod">
                                                        <input type="hidden" id="fecha_cuota_sig_prod" name="fecha_cuota_sig_prod">
                                                        <input type="hidden" id="s_cuota_prod" name="s_cuota_prod">
                                                        <input type="hidden" id="nro_cuotas_prod" name="nro_cuotas_prod">
                                                        <input type="hidden" id="u_capital_prod" name="u_capital_prod">
                                                        <input type="hidden" id="u_saldo_prod" name="u_saldo_prod" value="<?php echo $upago->vpag_saldo_final;?>">
                                                        <input type="hidden" id="nu_capital_prod_ids" name="nu_capital_prod_ids">
                                                        <input type="hidden" id="nu_capital_prod_montos" name="nu_capital_prod_montos">
                                                        <input type="hidden" id="nu_interes_prod_ids" name="nu_interes_prod_ids">
                                                        <input type="hidden" id="nu_interes_prod_montos" name="nu_interes_prod_montos">
                                                        <input type="hidden" id="nu_capital_prod" name="nu_capital_prod">
                                                        <input type="hidden" id="nu_interes_prod" name="nu_interes_prod">
                                                        <span id="txt_nu_capital_prod"></span>
                                                    </td>
                                                    <td>&nbsp;</td>
                                                    <td>&nbsp;</td>
<!--                                                    <td>
                                                    ...
                                                    <td>
                                                        <input type="hidden" id="nu_mora_gen_prod_ids" name="nu_mora_gen_prod_ids">
                                                        <input type="hidden" id="nu_mora_gen_prod_montos" name="nu_mora_gen_prod_montos">
                                                        <input type="hidden" id="nu_mora_gen_prod_dias" name="nu_mora_gen_prod_dias">
                                                        <input type="hidden" id="nu_mora_gen_prod" name="nu_mora_gen_prod">
                                                        <span id="txt_nu_mora_gen_prod"></span>
                                                    </td>
                                                </tr>
                                            </tfoot>
                                        </table>
                                ...
                                if ($venta_producto) {
                                    ?>
                                <div id="ContenedorDiv">
                                    <div class="Etiqueta fwbold">Capital Producto</div>
                                    <div id="CajaInput">
                                        <input type="hidden" name="capital_prod_ids" id="capital_prod_ids" value="">
                                        <input type="hidden" name="capital_prod_montos" id="capital_prod_montos" value="">
                                        <input type="text" class="caja_texto" name="capital_prod" id="capital_prod" value="" autocomplete="off">
<!--                                        <input type="hidden" name="capital_prod" id="capital_prod" value="">
                                        <div class="read-input" id="txt_capital_prod">&nbsp;</div>-->
                                    </div>
                                </div>
                                    <?php
                                }
...
//            if ($venta_producto) {
            ?>
                if ($('#capital_prod').length > 0) {
            cmp_edit("#capital_prod",
                {
                    keyup:function(obj,e){
                        var objcap=get_ini_capital_producto();
                        var capital=$(obj).val()*1;
                        ...
                        $('#capital_prod_ids').val(capital_ids.join(','));
                        $('#capital_prod_montos').val(capital_montos.join(','));
                        calcular_monto_producto();
                    },
                    cancel:function(){
                        var capital=reset_capital_producto();
                        calcular_monto_producto();
                        return capital.toFixed(2);
                    }
                }
...
//                        $('#capital_prod_ids').val('');
//                        $('#capital_prod_montos').val('');
//                        $('#capital_prod').val('0');
//                        $('#txt_capital_prod').text('0');
...
//                        calcular_saldo_final();
            ...
            $('#enter-cuotas-prod').click(function (){
                cargar_cuotas_producto(); 
            });
            ...
            $('#sol_nro_cuotas_prod').keypress(function (e){
                if(e.keyCode===13){
                    cargar_cuotas_producto();
                }
            });
...
//                    $('#txt_capital').text(sum_nu_capital);
            ...
            function cargar_cuotas_producto(){
                var sol_nro_cuotas=$('#sol_nro_cuotas_prod').val()*1;
                var params={};
                params.tarea='vcuotas_prod';
                params.fecha_pago=_FECHA_PAGO;
                params.ven_id=$('#vprod_id').val();
                params.nro_cuotas=sol_nro_cuotas;
                mostrar_ajax_load();
                $.post('ajax.php',params,function (respuesta){
                    ocultar_ajax_load();
...
//                    console.log(respuesta);
                    ...
                    var nu_interes_ids=new Array();
                    var nu_interes_montos=new Array();
                    
                    var sum_nu_capital=0;
                    var sum_nu_interes=0;
                    ...
                    $('#list_cuotas_prod tbody tr').remove();
                    var trows='';
                    var u_capital=0;
                    var lista=resp.listac;
                    for(var i=0;i<lista.length;i++){
                        ...
                        trows+='<tr>';
                        trows+='     <td>';
                        trows+='        <input type="hidden" name="cuotas_prod[]" value="'+fil.id+'">';
                        trows+='        '+fil.nro;
                        trows+='     </td>';
                        trows+='     <td>'+fecha_latina(fil.fecha_prog)+'</td>';
                        trows+='     <td>'+fil.interes+'</td>';
...
//                        trows+='     <td>'+fil.form+'</td>';
                        ...
                        nu_interes_ids.push(fil.id);
                        nu_interes_montos.push(fil.interes);
                        
                        sum_nu_capital+=fil.capital*1;
                        sum_nu_interes+=fil.interes*1;
                    ...
                    $('#nro_cuotas_prod').val(lista.length);
                    $('#u_capital_prod').val(u_capital);
                    var saldo=$('#saldo_prod').val()*1;
                    var u_saldo=saldo-sum_nu_capital;
                    $('#u_saldo_prod').val(u_saldo.toFixed(2));
                    
                    $('#nu_capital_prod_ids').val(nu_capital_ids.join(','));
                    $('#nu_capital_prod_montos').val(nu_capital_montos.join(','));                    
                    $('#nu_capital_prod').val(sum_nu_capital);
                    $('#txt_nu_capital_prod').text(sum_nu_capital);
                    
                    $('#nu_interes_prod_ids').val(nu_interes_ids.join(','));
                    $('#nu_interes_prod_montos').val(nu_interes_montos.join(','));                    
                    $('#nu_interes_prod').val(sum_nu_interes);
//                    $('#txt_nu_capital_prod').text(sum_nu_capital);
...
//                    $('#txt_nu_form').text(sum_nu_form);
                    ...
                    $('#nu_mora_gen_prod_ids').val(nu_mora_gen_ids.join(','));
                    $('#nu_mora_gen_prod_montos').val(nu_mora_gen_montos.join(','));
                    $('#nu_mora_gen_prod_dias').val(nu_mora_gen_dias.join(','));
                    $('#nu_mora_gen_prod').val(sum_nu_mora_gen);
                    $('#txt_nu_mora_gen_prod').text(sum_nu_mora_gen);
                    $('#list_cuotas_prod tbody').append(trows);
...
//                    console.log(s_cuota);
                    $('#s_cuota_prod').val(JSON.stringify(s_cuota));
                    $('#nro_cuota_sig_prod').val(s_cuota.nro);
                    $('#fecha_cuota_sig_prod').val(s_cuota.fecha_prog);
                    
//                    $('#capital').val(sum_nu_capital);
//                    $('#txt_capital').text(sum_nu_capital);
                    
                    reset_capital_producto();
//                    reset_form();
//                    reset_mora();
                    calcular_monto_producto();
                });
            ...
            function get_ini_capital_producto(){
                var capital_ids=trim($('#nu_capital_prod_ids').val());
                var capital_montos=trim($('#nu_capital_prod_montos').val());
                var capital=$('#nu_capital_prod').val()*1;
                return {'capital_ids': capital_ids,'capital_montos': capital_montos,'capital':capital};
            }
            
            function get_ini_interes_producto(){
                var interes_ids=trim($('#nu_interes_prod_ids').val());
                var interes_montos=trim($('#nu_interes_prod_montos').val());
                var interes=$('#nu_interes_prod').val()*1;
                return {'interes_ids': interes_ids,'interes_montos': interes_montos,'interes':interes};
            }
            ...
            function reset_capital_producto(){
                var obcap=get_ini_capital_producto();
                
                $('#capital_prod_ids').val(obcap.capital_ids);
                $('#capital_prod_montos').val(obcap.capital_montos);
                
                $('#capital_prod').val(obcap.capital.toFixed(2));
                $('#txt_capital_prod').text(obcap.capital.toFixed(2));
                $('#capital_prod').trigger('change');
                return obcap.capital;

            }
            
            function reset_interes_producto(){
                var obcap=get_ini_interes_producto();
                
                $('#interes_prod_ids').val(obcap.interes_ids);
                $('#interes_prod_montos').val(obcap.interes_montos);
                
                $('#interes_prod').val(obcap.interes.toFixed(2));
                $('#txt_interes_prod').text(obcap.interes.toFixed(2));
                $('#interes_prod').trigger('change');
                return obcap.interes;
...
//                console.log(monto);
            ...
            function calcular_monto_producto(){
//                var interes=$('#interes').val()*1;
                var interes_prod = 0;
                var capital_prod = $('#capital_prod').val()*1;
                var interes=$('#interes').val()*1;
                var capital=$('#capital').val()*1;
                var form=$('#form').val()*1;
                var envio=$('#envio').val()*1;
                var mora=$('#mora').val()*1;
//                console.log(interes+'+'+capital);
                var monto = interes + capital + form + mora + envio + capital_prod + interes_prod;
...
//                        $('#meses_plazo').val(plazo_res);
            ...
            function enviar_frm_pagos(){
				...
				if (capital < 0) {
					$.prompt('No puede autorizar un cobro con capital negativo(' + capital + ').');
					return false;
				}
...
//        }
        ...
        if($_POST){
            if ($_POST[reformular_plan_lote] == 'si') {
                $resp_ref_lote = $this->guardar_reformulacion($venta);
            }
            
            if ($resp_ref_lote) {
                if (!$resp_ref_lote->ok) {
                    $url_volver = "$this->link?mod=$this->modulo&tarea=$_GET[tarea]&id=$_GET[id]";
                    $this->formulario->ventana_volver($resp_ref_lote->mensaje, $url_volver, '', 'Error');
                    return FALSE;
                }
            }
            
            if (isset($_POST[reformular_plan_casa]) && $_POST[reformular_plan_casa] == 'si') {
                $venta = FUNCIONES::objeto_bd_sql("select * from venta 
                    inner join venta_producto on(ven_id=vprod_ven_id)                     
                    inner join urbanizacion_producto on(vprod_prod_id=uprod_id)
                    where ven_id='$_GET[id]' and vprod_estado='Pendiente'");
                $this->guardar_reformulacion_producto($venta);
            }            
        }else{
    ...
    function guardar_reformulacion_producto($venta){
//        echo "reformulando producto....";
...
//        return false;
        ...
        $saldo = $_POST[saldo_final_prod];
        $saldo_final = $_POST[saldo_final_prod];

        $tipo_plan = $atp[$_POST[def_plan_efectivo_prod]];

        $nro_cuota_inicio = $_POST[nro_cuota_sig_prod];
        $plazo = $_POST[meses_plazo_prod];
        if ($tipo_plan == 'cuota') {
            $plazo += ($nro_cuota_inicio - 1);
        }

        $cuota=$_POST[cuota_mensual_prod];
               
        $rango=$_POST[ven_rango_prod];
        $frecuencia=$_POST[ven_frecuencia_prod];
        
        $fecha_inicio=  $_POST[fecha_venta_prod];
        $fecha_pri_cuota=  FUNCIONES::get_fecha_mysql($_POST[fecha_pri_cuota_prod]);
                
        $descripcion = "Venta de Producto $venta->uprod_nombre - Venta $venta->vprod_ven_id";
        
        $params=array(
            'int_id'=>$venta->ven_int_id,
            'ven_id'=>$venta->ven_id,
            'vprod_id'=>$venta->vprod_id,
            'interes_anual'=>$venta->ven_val_interes,
            'moneda'=>$venta->ven_moneda,
        ...
        $sql_ins_ref="insert into venta_reformulacion (vref_vprod_id,vref_usu,
            vref_fecha,vref_fecha_cre,vref_parametros)values('$venta->vprod_id',
            '$_SESSION[id]','$fecha_ref','$fecha_cre','$ser_params')";
        
        $conec->ejecutar($sql_ins_ref);
        $this->reformular_plan_producto($params,$conec);
        $exito = $conec->commit();
...
//            if ($_POST[reformular_plan_casa] == 'si') {
            if (FALSE) {
                $resp->ok = TRUE;
                $resp->mensaje = $mensaje;
                return $resp;
...
//            if ($_POST[reformular_plan_casa] == 'si') {
    ...
    function reformular_plan_producto($params, $conec = null){
        ...
        $sql_sel_ucu_pen = "select * from interno_deuda_producto where idpr_tabla='venta_producto' 
            and idpr_tabla_id='$par->vprod_id' and idpr_estado='Pendiente' 
            and (idpr_capital_pagado+idpr_interes_pagado)>0 
            and idpr_tipo='pcuota' and idpr_num_correlativo>0 
            order by idpr_id asc limit 1";

        $ucu_pen =  FUNCIONES::objeto_bd_sql($sql_sel_ucu_pen);
//        FUNCIONES::print_pre($ucu_pen);
        if($ucu_pen){
            $_saldo_final = ($ucu_pen->idpr_saldo + $ucu_pen->idpr_capital) - $ucu_pen->idpr_capital_pagado;
            $sql_up = "update interno_deuda_producto set idpr_estado='Pagado' , idpr_saldo_final='$_saldo_final' 
                where idpr_id=$ucu_pen->idpr_id";
            $conec->ejecutar($sql_up);
        }
        $and_filtro='';
        if($par->idpr_id){
            $and_filtro=" and idpr_id>$par->idpr_id";
        }
        
        $sql_del = "update interno_deuda_producto set idpr_estado='Anulado' 
            where idpr_estado='Pendiente' 
            and idpr_tabla='venta_producto' and idpr_tabla_id='$par->vprod_id' 
            and idpr_tipo='pcuota' $and_filtro";
        $conec->ejecutar($sql_del);
...
//            FUNCIONES::print_pre($lista_cuotas);
            ...
            foreach ($lista_cuotas as $fila) {
                ...
                'venta_producto','$par->vprod_id','$fila->nro_cuota','$par->int_id',
                '$hoy','$par->moneda','$fila->interes','$fila->capital',
                '$fila->monto','$fila->saldo','$fila->fecha','Pendiente',
                '$fila->dias','Cuota Nro $fila->nro_cuota - $par->concepto',
                '$user','pcuota','$hoy','0','$par->ven_id'
            )";

                $conec->ejecutar($sql_cuo);
            }
                       
        }else{
            $sql_up_venta = "update venta_producto set vprod_estado='Pagado' where vprod_id=$par->vprod_id";
            $conec->ejecutar($sql_up_venta);
        }
...
//        }
    ...
    function icremento_descuento_venta($params,&$conec=null) {
		...
		/*REFLEJO CONTABLE DE LA VARIACION DE CAPITAL(DESC/INC)*/
...
//            }
    ...
    function pri_cuota_pendiente_producto($venta){
        $pcuota=  FUNCIONES::objeto_bd_sql("select * from interno_deuda_producto where idpr_tabla='venta_producto'
            and idpr_tabla_id='$venta->vprod_id' and idpr_estado='Pendiente' and (idpr_capital_pagado+idpr_interes_pagado)=0 and idpr_num_correlativo>0 order by idpr_id asc limit 1");
        if(!$pcuota){
//            $ucuota=FUNCIONES::objeto_bd_sql("select * from interno_deuda where ind_tabla='venta' and ind_tabla_id='$venta->ven_id' and ind_estado='Pendiente' and ind_capital_pagado>0 and ind_tipo='pcuota' and ind_num_correlativo>0 order by ind_id asc limit 1");
//            if(!$ucuota){
            $ucuota=FUNCIONES::objeto_bd_sql("select * from interno_deuda_producto where idpr_tabla='venta' and idpr_tabla_id='$venta->vprod_id' and idpr_estado='Pagado' order by idpr_id desc limit 1");
            if(!$ucuota){
                $ucuota=new stdClass();
                $ucuota->idpr_num_correlativo=0;
                $ucuota->idpr_fecha_programada=$venta->vprod_fecha;
            }
//            }
            $pcuota=new stdClass();
            $pcuota->idpr_num_correlativo=$ucuota->idpr_num_correlativo+1;
            if($venta->ven_frecuencia=='30_dias'){
                $pcuota->idpr_fecha_programada=  FUNCIONES::sumar_dias(30, $ucuota->idpr_fecha_programada);
            }elseif($venta->ven_frecuencia=='dia_mes'){
                $af=  explode('-', $ucuota->idpr_fecha_programada);
                $pcuota->idpr_fecha_programada =  FUNCIONES::sumar_meses($ucuota->idpr_fecha_programada, $venta->ven_rango, $af[2]);
            }
        }
        return $pcuota;
    }
...
//        $pagado=  $this->total_pagado($ven_id);
        ...
        <form id="frm_sentencia" name="frm_sentencia" enctype="multipart/form-data" method="POST" action="gestor.php?mod=venta&tarea=REFORMULAR&id=<?php echo $venta->ven_id?>" >
            ...
            <input type="hidden" name="reformular_plan_lote" id="reformular_plan_lote" value="si">
            <div id="FormSent" style="width:100%">
                <div id="div_plan_lote" reform-id="reformular_plan_lote" target-id="tg_plan_lote" class="Subtitulo">PLAN DE PAGOS LOTE</div>
                <div id="ContenedorSeleccion" class="tg_plan_lote">
                    <div id="ContenedorDiv">
                        <div class="Etiqueta">Nro. Venta</div>
                        <div id="CajaInput">
                            <div class="read-input"><?php echo $venta->ven_id;?></div>
                ...
                if ($venta_producto) {
                    $sql_vprod = "
                        select ifnull(sum(idpr_capital_pagado),0)as capital,
                        ifnull(sum(idpr_interes_pagado),0)as interes from interno_deuda_producto
                        where idpr_tabla='venta_producto' and idpr_tabla_id='$venta_producto->vprod_id'
                            and idpr_estado='Pagado'";
//                    echo $sql_vprod;
                    $obj_total_pagado = FUNCIONES::objeto_bd_sql($sql_vprod);
                    
                    $sql_plazo = "select ifnull(count(idpr_id),0)as campo 
                    from interno_deuda_producto 
                    where idpr_tabla='venta_producto' and idpr_tabla_id='$venta_producto->vprod_id'
                    and idpr_estado in('Pendiente','Pagado')";
                    
//                    echo "<p>$sql_plazo</p>";
                    
                    $vprod_plazo = FUNCIONES::atributo_bd_sql($sql_plazo);
                    
                    $scuota_prod = $this->pri_cuota_pendiente_producto($venta_producto);
                    
                    $plazo_rest_prod = $vprod_plazo - ($scuota_prod->idpr_num_correlativo - 1);
//                    echo "<p>vprod_plazo:$vprod_plazo - num_corre:$scuota_prod->idpr_num_correlativo</p>";
                ?>
                <input type="hidden" name="nro_cuota_sig_prod" id="nro_cuota_sig_prod" value="<?php echo $scuota_prod->idpr_num_correlativo;?>">
                <input type="hidden" name="fecha_cuota_sig_prod" id="fecha_cuota_sig_prod" value="<?php echo $scuota_prod->idpr_fecha_programada;?>">
                <input type="hidden" name="reformular_plan_casa" id="reformular_plan_casa" value="no">
                <div id="div_plan_casa" reform-id="reformular_plan_casa" target-id="tg_plan_casa" class="Subtitulo">PLAN DE PAGOS CASA</div>
                <div id="ContenedorSeleccion" class="tg_plan_casa">
                    <div id="ContenedorDiv">
                        <div class="Etiqueta">Fecha Venta Casa:</div>
                        <div id="CajaInput">
                            <input type="hidden" id="fecha_venta_prod" name="fecha_venta_prod" value="<?php echo $venta_producto->vprod_fecha?>">
                            <div class="read-input"><?php echo FUNCIONES::get_fecha_latina($venta_producto->vprod_fecha);?></div>
                        </div>
                    </div>
                    <div id="ContenedorDiv">
                        <div class="Etiqueta">Precio Casa:</div>
                        <div id="CajaInput">
                            <div class="read-input"><?php echo $venta_producto->vprod_monto;?></div>
                        </div>
                    </div>
                    <div id="ContenedorDiv">
                        <div class="Etiqueta">Anticipos Casa:</div>
                        <div id="CajaInput">
                            <div class="read-input"><?php echo $venta_producto->vprod_anticipo;?></div>
                        </div>
                    </div>
                    <div id="ContenedorDiv">
                        <div class="Etiqueta">Saldo Financiar Credito Casa:</div>
                        <div id="CajaInput">
                            <div class="read-input"><?php echo $venta_producto->vprod_monto_efectivo;?></div>
                        </div>
                    </div>
                    <div id="ContenedorDiv">
                        <div class="Etiqueta">Capital Pagado:</div>
                        <div id="CajaInput">
                            <div class="read-input"><?php echo $obj_total_pagado->capital;?></div>
                        </div>
                    </div>
                    <div id="ContenedorDiv">
                        <div class="Etiqueta">Saldo:</div>
                        <div id="CajaInput">
                            <input type="hidden" id="saldo_final_prod" name="saldo_final_prod" value="<?php echo ($venta_producto->vprod_monto_efectivo - $obj_total_pagado->capital);?>" />
                            <div class="read-input"><?php echo ($venta_producto->vprod_monto_efectivo - $obj_total_pagado->capital);?></div>
                        </div>
                    </div>
                    ...
                    <div id="box-reformular" style="">
                        ...
                        <div id="ContenedorDiv">
                            <div id="CajaInput">
                                <span style="float: left; margin-top: 2px;">Definir Plan de Pagos por: &nbsp;</span>
                                <select  id="def_plan_efectivo_prod" name="def_plan_efectivo_prod" data-tipo="efectivo" style="width: 100px;">
                                    <option value="mp">Meses Plazo</option>
                                    <option value="cm">Cuota Mensual</option>
                        ...
                        <div id="ContenedorDiv">
                            ...
                            <div id="CajaInput" name="divCuotaMensual" style="margin-right: 5px;">
                                <span style="float: left; margin-top: 2px; margin-right: 5px;">Monto Cuota: </span>
                                <input type="text" name="cuota_mensual_prod" id="cuota_mensual_prod" size="8" value="" onKeyPress="return ValidarNumero(event);" autocomplete="off">
                            </div>
                            <div id="CajaInput">
                                <span style="float: left; margin-top: 2px; margin-right: 5px;" >Nro de Cuotas: </span>
                                <input type="text" name="meses_plazo_prod" id="meses_plazo_prod" size="8" value="<?php echo $plazo_rest_prod;?>" onKeyPress="return ValidarNumero(event);" autocomplete="off">
                            </div>
                            <div id="CajaInput">
                                <span style="float: left; margin-top: 2px; margin-left: 15px; margin-right: 5px;">Fecha de la Primer Cuota: </span>
                                <input class="caja_texto" name="fecha_pri_cuota_prod" id="fecha_pri_cuota_prod" size="12" value="<?php echo FUNCIONES::get_fecha_latina ($scuota_prod->idpr_fecha_programada);?>" type="text">
                                <script>
                                    $("#fecha_pri_cuota_prod").mask("99/99/9999");
                                </script>
                            </div>
                            <div id="CajaInput" name="divCuotaMensual" >
                                <span style="float: left; margin-top: 2px; margin-right: 5px;"> &nbsp;&nbsp; Rango: </span>
                                <select id="ven_rango_prod" name="ven_rango_prod">
                                    <option value="1" <?php echo $venta->ven_rango=='1'?'selected=""':'';?>>Mensual</option>
                                    <option value="2" <?php echo $venta->ven_rango=='2'?'selected=""':'';?>>Bimestral</option>
                                    <option value="3" <?php echo $venta->ven_rango=='3'?'selected=""':'';?>>Trimestral</option>
                                    <option value="4" <?php echo $venta->ven_rango=='4'?'selected=""':'';?>>Cuatrimestral</option>
                            ...
                            <div id="CajaInput" name="divCuotaMensual" >
                                <span style="float: left; margin-top: 2px; margin-right: 5px;"> &nbsp;&nbsp; Frec.: </span>
                                <select id="ven_frecuencia_prod" name="ven_frecuencia_prod">
                                    <option value="30_dias" <?php echo $venta->ven_frecuencia=='30_dias'?'selected=""':'';?>>Cada 30 dias</option>
                                    <option value="dia_mes" <?php echo $venta->ven_frecuencia=='dia_mes'?'selected=""':'';?>>Mantener el dia</option>
                                </select>
                            </div>
                            <div id="CajaInput">
                                <img id="ver_plan_efectivo_prod" src="imagenes/generar.png" style='margin:0px 0px 0px 5px; cursor: pointer' onclick="javascript:ver_plan_pago_prod();">
                            </div>
                ...
                <div id="ContenedorDiv">
                    <div id="CajaBotones">
                        <center>
                            <input class="boton" type="button" onclick="alertar_reformulacion_planes();" value="Guardar" name="">
        ...
        <script>
            $(".Subtitulo").click(function (){
                ...
                var reform_id = $(this).attr('reform-id');
                $('#'+reform_id).val(valor_ref);
...
//            $('#div_plan_lote').trigger('click');
            $('#div_plan_casa').trigger('click');
            ...
            $('#def_plan_efectivo_prod').change(function(){
                var def=$(this).val();
                if(def==='mp'){
                    $('#meses_plazo_prod').parent().show();
                    $('#cuota_mensual_prod').parent().hide();
                    $('#ver_plan_efectivo_prod').show();
                    $('#fecha_pri_cuota_prod').prev('span').text('Fecha Pri Cuota: ');                                        
                }else if(def==='cm'){
                    $('#meses_plazo_prod').parent().show();
                    $('#cuota_mensual_prod').parent().show();                    
                    $('#ver_plan_efectivo_prod').show();                    
                    $('#meses_plazo_prod').prev('span').text('Maximo Plazo: ');
                    $('#cuota_mensual_prod').prev('span').text('Monto Cuota: ');
                    $('#cuota_mensual_prod').prev('span').show();
                    $('#fecha_pri_cuota_prod').prev('span').text('Fecha Pri Cuota: ');
                                        
                }
            });
            
            $('#def_plan_efectivo_prod').trigger('change');
            ...
            function ver_plan_pago_prod(){
                var saldo_financiar=0;
                var ncuotas=0;
                var fecha_pri_cuota=0;
                var monto_cuota=0;
                var def=$('#def_plan_efectivo_prod option:selected').val();
                if(def==='mp'){
                    ncuotas = $('#meses_plazo_prod').val();
                    monto_cuota = '';
                }else if(def==='cm'){
                    var cu_pagadas=$('#nro_cuota_sig_prod').val()*1-1;
                    ncuotas = ($('#meses_plazo_prod').val()*1)+cu_pagadas;
                    monto_cuota = $('#cuota_mensual_prod').val();
                }
                saldo_financiar=$('#saldo_final_prod').val()*1;
                fecha_pri_cuota =$('#fecha_pri_cuota_prod').val();
                
                var fecha_inicio = $('#fecha_venta_prod').val();
                var fp_mysql=fecha_mysql(fecha_pri_cuota);
                var fi_mysql=fecha_inicio;
                if(fi_mysql>fp_mysql){
                    $.prompt('La fecha de la Primera cuota no puede ser menor a la fecha de la Venta del Producto');
                    return;
                }
                
                var rango =$('#ven_rango_prod option:selected').val();
                var frec =$('#ven_frecuencia_prod option:selected').val();
                ...
                if ((ncuotas*1 > 0 || monto_cuota*1 > 0) && saldo_financiar > 0  && fecha_pri_cuota !== '') {
                    ...
                    var par={};
                    par.tarea='plan_pagos';
                    par.origen='plan_pagos_prod';
                    par.saldo_financiar=saldo_financiar;
                    par.monto_total=saldo_financiar;
                    par.meses_plazo=ncuotas;
                    par.cuota_mensual=monto_cuota;
                    par.ven_moneda=moneda;
                    par.nro_inicio=$('#nro_cuota_sig_prod').val();
                    par.fecha_inicio=fecha_inicio;
                    par.fecha_pri_cuota=fecha_pri_cuota;
                    par.interes=interes;
                    par.form=form;
            ...
            function alertar_reformulacion_planes(){
                
                var ref_lote = $('#reformular_plan_lote').val();
                var ref_casa = $('#reformular_plan_casa').val();
                
                if (!(ref_lote == 'si' || ref_casa == 'si')) {
                    $.prompt('Nada para reformular.');
                    return false;
                }
                
                var txt = 'A continuacion se reformularan los siguientes planes de pagos:<br>';
                if (ref_lote == 'si') {
                    txt += 'Plan de Pagos Lote';
                }
                
                if (ref_casa == 'si') {
                    txt += '<br>Plan de Pagos Casa';
                }
                $.prompt(txt,{ 
                    buttons:{Aceptar:true, Cancelar:false},
                    callback: function(v,m,f){						
                        if(v){
                            enviar_frm_reformulacion();
                        }
                    }
                });
            }
            ...
            function enviar_frm_reformulacion(){
                ...
                if ($('#reformular_plan_casa').length > 0) {
                    var res_val_prod = validar_parametros_producto();
                    if (!res_val_prod.ok) {
                        $.prompt(res_val_prod.mensaje);
                        return false;
                    }
                }
...
//                            console.info('OK');
            ...
            function validar_parametros_producto(){
                var res = {};
                var saldo_final=$('#saldo_final_prod').val()*1;
                
                if(saldo_final<=0){
                    
                    res.mensaje = 'El Saldo Final del Producto debe ser Mayor a 0';
                    res.ok = false;
                    return res;
                }
                
                var def=$('#def_plan_efectivo_prod option:selected').val();
                var fecha_pri_cuota =$('#fecha_pri_cuota_prod').val();
                var fecha_inicio =$('#fecha_venta_prod').val();
                var fp_mysql=fecha_mysql(fecha_pri_cuota);
                var fi_mysql=fecha_inicio;
                if(fi_mysql>fp_mysql){
                    res.mensaje = 'La fecha de la Primera cuota no puede ser menor a la fecha de la Venta del Producto';
                    res.ok = false;
                    return res;
                }
                
                if(def==='mp'){
                    var mp=$('#meses_plazo_prod').val()*1;
                    var fpc=$('#fecha_pri_cuota_prod').val();
                    if(!(mp>0 && fpc!=='')){
                                                
                        res.mensaje = 'Revise los datos del credito efectivo:<br> - La meses plazo <br> - Fecha de la primera cuota ';
                        res.ok = false;
                        return res;
                    }   
                }else if(def==='cm'){
                    var cm=$('#cuota_mensual_prod').val()*1;
                    var fpc=$('#fecha_pri_cuota_prod').val();
                    if(!(cm>0 && fpc!=='')){
                                                
                        res.mensaje = 'Revise los datos del credito efectivo:<br> - La cuota Mensual <br> - Fecha de la primera cuota ';
                        res.ok = false;
                        return res;
                    }
                }
    ...
    function total_pagado_producto($vprod_id) {
        $sql_pag = "select sum(idpr_interes_pagado)as interes, sum(idpr_capital_pagado) as capital, sum(idpr_monto_pagado) as monto, 
        sum(idpr_capital_desc) as descuento, sum(idpr_capital_inc) as incremento ,
        sum(idpr_costo_pagado) as costo
        from interno_deuda_producto where idpr_tabla='venta_producto'
        and idpr_tabla_id='$vprod_id' and idpr_estado in('Pendiente','Pagado')";
        $pagado = FUNCIONES::objeto_bd_sql($sql_pag);
        return $pagado;
    }
    
    function pagar_cu_producto($data,&$conec=null){
        if ($conec == null) {
            $conec = new ADO();
        }
        ...
        $recibo = $data->recibo;
        $pag_codigo = $data->pag_codigo;
        $saldo_venta = $vproducto->vprod_monto_efectivo;
        $costo_ids = array();
        $costo_montos = array();
        $costo = 0;
        $costo_pag = 0;
        ...
        $str_interes_ids = trim($cob->vcob_interes_prod_ids);
        if ($str_interes_ids && $cob->vcob_interes_prod > 0) {
            $s_rastro .= "-1";
            $cuotas = FUNCIONES::lista_bd_sql("select * from interno_deuda_producto where idpr_id in ($str_interes_ids) and idpr_tipo='pcuota' order by idpr_id asc");
//            $vpag_interes = $cob->vcob_interes;
            $interes_montos = explode(',', trim($cob->vcob_interes_prod_montos));
            $interes_ids = explode(',', trim($cob->vcob_interes_prod_ids));
            if (count($interes_ids) ==  count($cuotas) && count($cuotas)==  count($interes_montos)){
                $s_rastro .= "-2";
                for ($i = 0; $i < count($cuotas); $i++) {
                    $s_rastro .= "-3.$i";
...
//                    $interes_dif = $vpag_interes - ($cu->ind_interes * 1);
                    ...
                    $sql_ins_idp = "insert into interno_deuda_pago(
                                        ...
                                        '$cu->idpr_id','$cu->idpr_num_correlativo','$pago_id','$recibo','$pag_codigo','interes_producto',
                                        '$fecha_pago','$interes_pagado','Activo','$fecha_cre','$usuario_id',
                                        '$cob->vcob_usu_cre','$cob->vcob_codigo','$cob->vcob_aut'
                                    )
                        ";
                    ...
                    $sql = "update interno_deuda_producto set 
                                idpr_interes_pagado=idpr_interes_pagado+$interes_pagado,
                                idpr_monto_pagado=idpr_capital_pagado+idpr_interes_pagado
                                where idpr_id = '$cu->idpr_id'
                            ";
                    //            echo $sql.';<br>';
                    $conec->ejecutar($sql);
        ...
        echo "<p style='color:white'>$s_rastro</p>";
                        
        $str_capital_ids = trim($cob->vcob_capital_prod_ids);
               
        if ($str_capital_ids && $cob->vcob_capital_prod > 0) {
            $cuotas = FUNCIONES::lista_bd_sql("select * from interno_deuda_producto 
            where idpr_id in ($str_capital_ids) and idpr_tipo='pcuota' order by idpr_id asc");
//            $vpag_capital = $cob->vcob_capital;
            $capital_montos = explode(',', trim($cob->vcob_capital_prod_montos));
            $capital_ids = explode(',', trim($cob->vcob_capital_prod_ids));
            if (count($capital_ids) ==  count($cuotas) && count($cuotas)==  count($capital_montos)){
                $is_reformular=false;
                for ($i = 0; $i < count($cuotas); $i++) {
                    $cu = $cuotas[$i];
//                    $capital_dif = $vpag_capital - ($cu->ind_capital * 1);
                    ...
                    $ind_estado = 'Pendiente';
                    $saldo_final = 0;
                    if ($capital_pagado.'' == $cu->idpr_capital.'') {  
                        $ind_estado = 'Pagado';
                        $saldo_final = $cu->idpr_saldo;
                    } else {
                        if($i == 0 && $i == count($cuotas)-1){// primer y el ultimo pago
//                            echo " ENTRO PRIMER IF <br>";
                            $ind_capital_pagado = $cu->idpr_capital_pagado + $capital_pagado;
                            if ($ind_capital_pagado . '' == $cu->idpr_capital . '') {
                                $ind_estado = 'Pagado';
                                $saldo_final = $cu->idpr_saldo;
                            } elseif ($ind_capital_pagado > $cu->idpr_capital) {
                                $ind_estado = 'Pagado';
                                $capital_dif = $ind_capital_pagado - ($cu->idpr_capital * 1);
                                $saldo_final = $cu->idpr_saldo - $capital_dif;
                                $is_reformular = true;
                                $ant_saldo = $cu->idpr_saldo;
                                $ant_capital = $cu->idpr_capital;
                                $ant_fecha_prog = $cu->idpr_fecha_programada;
                                $ant_ant_cuota = $venta->ven_cuota;
                            }
...
//                            echo " ENTRO TERCER IF <br>";
                            if ($capital_pagado > $cu->idpr_capital) { /// pago por demas
                                $ind_estado = 'Pagado';
                                $capital_dif = $capital_pagado - ($cu->idpr_capital * 1);
                                $saldo_final = $cu->idpr_saldo - $capital_dif;
                                $is_reformular = true;
                    ...
                    $sql_ins_idp = "insert into interno_deuda_pago(
                                        ...
                                        '$cu->idpr_id','$cu->idpr_num_correlativo','$pago_id','$recibo','$pag_codigo','capital_producto',
                                        '$fecha_pago','$capital_pagado','Activo','$fecha_cre','$usuario_id',
                                        '$cob->vcob_usu_cre','$cob->vcob_codigo','$cob->vcob_aut'
                                    )
                        ";
                    ...
                    // $saldo_costo = $vproducto->vprod_costo - ($vproducto->vprod_anticipo + $vproducto->vprod_costo_cub);                    
					$saldo_costo = $vproducto->vprod_costo - ($vproducto->vprod_anticipo);                    
                    $costo_pagado=0;
                    
                    if ($saldo_costo > 0) {
                        if ($saldo_final <= 0 && $i == count($cuotas)-1) {
                            $costo_pagado = $vproducto->vprod_costo - ($vproducto->vprod_costo_cub + $vproducto->vprod_costo_pag + $costo);
                        } else {
                    ...
                    if($costo_pagado>0){
                        $sql_ins_idp = "insert into interno_deuda_pago(
                            ...
                            '$cu->idpr_id','$cu->idpr_num_correlativo','$pago_id','$recibo','$pag_codigo','costo_producto',
                            '$fecha_pago','$costo_pagado','Activo','$fecha_cre','$usuario_id',
                            '$cob->vcob_usu_cre','$cob->vcob_codigo','$cob->vcob_aut'
                        )";
                        $conec->ejecutar($sql_ins_idp);
                    ...
                    $sql = "update interno_deuda_producto set 
                                idpr_estado='$ind_estado',
                                idpr_fecha_pago='$fecha_pago',
                                idpr_capital_pagado=idpr_capital_pagado+$capital_pagado,
                                idpr_costo_pagado=idpr_costo_pagado+$costo_pagado,    
                                idpr_monto_pagado=idpr_interes_pagado+idpr_capital_pagado,                                
                                idpr_saldo_final='$saldo_final'
                                where idpr_id = '$cu->idpr_id'
                            ";
//                    echo $sql.';<br>';
                    $conec->ejecutar($sql);
                ...
                $sql_up_pag_costo="update venta_pago 
                set vpag_costo_prod='$costo',                     
                vpag_costo_prod_ids='$txt_costo_ids', 
                vpag_costo_prod_montos='$txt_costo_montos'    
                where vpag_id=$pago_id";
                $conec->ejecutar($sql_up_pag_costo);
                ///ACTUALIZAR COSTO DE VENTA
                
                $total_pagado_prod = $this->total_pagado_producto($vproducto->vprod_id);                
                $saldo_producto = $vproducto->vprod_monto - $total_pagado_prod->capital;
                
                $set_ven_estado = "";
                if ($saldo_producto <= 0) {
                    $set_ven_estado = ",vprod_estado='Pagado'";
                    
                }
                
                $sql_up_ven_costo = "update venta_producto set 
                vprod_costo_cub=vprod_costo_cub+$costo_cub, 
                vprod_costo_pag=vprod_costo_pag+$costo_pag
                $set_ven_estado
                where vprod_id=$vproducto->vprod_id";
                $conec->ejecutar($sql_up_ven_costo);
        ...
        return (object)array('costo' => $costo);
    }
