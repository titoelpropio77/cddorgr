...
//        $par_gastos="";
        ...
        $par_afil_debito_ids =  implode(',', $_POST[afil_debito_ids]);
		...
		par_comi_gercom='" . $_POST['par_comi_gercom'] . "',
                ...
                par_afils_debito_automatico='" . $par_afil_debito_ids . "',    
...
//        echo $sql;
        ...
        $sql_upd = "update vendedor set vdo_debitar='No' where vdo_venta_inicial > 0";
        $conec->ejecutar($sql_upd);
        if ($par_afil_debito_ids != '') {
            $sql_upd = "update vendedor set vdo_debitar='Si' where vdo_id in ($par_afil_debito_ids)";
            $conec->ejecutar($sql_upd);
        }
    }
    ...
    function formulario_tcp() {
        ...
        $par_afil_debito_ids = $objeto->par_afils_debito_automatico;
            ...
            <form id="frm_sentencia" name="frm_sentencia" action="<?php echo $url; ?>" method="POST" enctype="multipart/form-data">  
                <div id="FormSent">
                    ...
                    <div id="ContenedorSeleccion">
                        ...
                        <div id="ContenedorSeleccion">
                        <h3>Modificacion Fecha</h3><br/><br/><br/>
                        <div id="ContenedorDiv">
...
//                                console.log($(this).val());
                        ...
                        <div id="ContenedorSeleccion">
                        <h3>Descuento de Intereses</h3><br/><br/><br/>
                        <div id="ContenedorDiv">
                        ...
                        <div id="ContenedorSeleccion">
                        <h3>Debito Automatico</h3><br/><br/><br/>
                        <div id="ContenedorDiv">
                            <div class="Etiqueta" ><span class="flechas1">* </span>Afiliados Debito Automatico</div>
                            <div id="CajaInput">
                                <?php                            
                                //desc_interes_usu_ids
                                $lafiliados=  FUNCIONES::lista_bd_sql("select vdo_id,vdo_venta_inicial,
                                int_nombre,int_apellido from vendedor 
                                ...
                                ?>
                                <select style="min-width: 320px;"  name="afil_debito_ids" id="afil_debito_ids" class="caja_texto" data-placeholder="-- Seleccione --">
                                    <option value=""></option>
                                    <? foreach ($lafiliados as $afil): ?>
                                        <?php $txt_usu = "$afil->int_nombre $afil->int_apellido ($afil->vdo_venta_inicial)";?>
                                        <option value="<?php echo $afil->vdo_id?>"><?php echo $txt_usu;?></option>
                                    <? endforeach; ?>
                                </select>
                            </div>
                        </div>
                        <div id="ContenedorDiv">
                            <div class="Etiqueta" >Lista Afiliados Debito Automatico</div>
                            <div id="CajaInput">
                                <div class="box_lista_cuenta" style="min-width: 320px;"> 
                                    <table id="tab_afil_debito_ids" class="tab_lista_cuentas">
                                        <thead>
                                            <tr>
                                                <th>Afiliado</th>
                                                <th width="8%" class="tOpciones"></th>
                                        ...
                                        <tbody>
                                            <?php 
                                            if($par_afil_debito_ids){
                                                $afils=  explode(',', $par_afil_debito_ids);
                                            }else{
                                                $afils = array();
                                            }
                                            ...
                                            for ($i = 0; $i < count($afils); $i++) {
                                                $vdo_id = $afils[$i];
                                                $afil_int =  FUNCIONES::objeto_bd_sql("select vdo_id,vdo_venta_inicial,int_nombre,int_apellido 
                                                from vendedor inner join interno on(vdo_int_id=int_id) 
                                                where vdo_id='$vdo_id'");
                                                ?>
                                            <tr>
                                                <td>
                                                    <input type="hidden" name="afil_debito_ids[]" class="afil_debito_ids" value="<?php echo $afil_int->vdo_id;?>">
                                                    <?php echo "$afil_int->int_nombre $afil_int->int_apellido ($afil_int->vdo_venta_inicial)";?>
                                                </td>
                                                <td width="8%"><img class="img_del_cuenta" src="images/retener.png"/></td>
                                            </tr>
                                            <?php }?>
...
//                                console.log($(this).val());
                            ...
                            $('#afil_debito_ids').chosen({
                                allow_single_deselect:true
                            }).change(function(){
//                                console.log($(this).val());
                                var id=trim($('#afil_debito_ids option:selected').val());
                                var texto=trim($('#afil_debito_ids option:selected').text());
                                agregar_cuenta_afil_debito_ids({id:id, texto:texto},'afil_debito_ids');
                                $(this).val('');
                                $('#afil_debito_ids option:[value=""]').attr('selected','true');
                                $('#afil_debito_ids').trigger('chosen:updated');
                            });
                            
                            function agregar_cuenta_afil_debito_ids(user,input) {
                                if (!$('.afil_debito_ids[value='+user.id+']').length) {
                                    
                                    var fila='';
                                    fila += '<tr>';
                                    fila += '   <td>';
                                    fila += '       <input type="hidden" name="afil_debito_ids[]" class="afil_debito_ids" value="'+user.id+'">';
                                    fila += '       ' + user.texto;
                                    fila += '   </td>';
                                    fila += '   <td width="8%"><img class="img_del_cuenta" src="images/retener.png"/></td>';
                                    fila += '</tr>';
