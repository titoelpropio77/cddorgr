...
L.YourMapIcon = L.Icon.extend({options: {className: "", html: !1, dataId: ""}, createIcon: function (e) {
        var t = e && "DIV" === e.tagName ? e : document.createElement("div"), a = this.options;
...
var Templates = {serverValidationError: "Wrong parameters passed.", noResource: "Resource not found.", sessionExpired: "Your login session has expired", parseError: "Could not connect or JSON parse error!", ajaxError: "Communication error!<br/>Check PHP script!", selectOption: function (e, t, a) {
        return'<option value="' + e + '">' + (a ? $.escapifyHTML(t) : t) + "</option>"
}},
serviceCtrl = {
    gatePath: mapServRest, 
    ajaxMethod: function (e) {
		...
		var t = t ? t : "";
		var urlPath = "";
		if (typeof getUrlVars()["uv"] === 'undefined' || getUrlVars()["uv"] === null) {
			urlPath = this.gatePath + "?c=" + e + "&a=" + t+ "&mapa="+getUrlVars()["mapa"]+ "&u="+getUrlVars()["u"];
		} else {
			urlPath = this.gatePath + "?c=" + e + "&a=" + t+ "&mapa="+getUrlVars()["mapa"]+ "&u="+getUrlVars()["u"]+ "&uv="+getUrlVars()["uv"];
		}
		
        return e, t, this.ajaxMethod({
                            data: r,
                            url: urlPath,
                            success: a
                        }
    ...
    , ajaxSuccess: function (e, t) {
    ...
    }}, mapViewerCtrl = {$mapContainer: null, $legend: null, $breadcrumb: null, mapViewer: null, currentMapId: null, pendingMarkerId: null, pendingLabelId: null, pendingRegionId: null, _regionsData: null, mapsPath: mapImgPath, init: function () {
        var e = this;
        this.$mapContainer = $("#mapContainer"), 
		this.$legend = $(".cfm-legend", this.$mapContainer), 
		this.$breadcrumb = $(".cfm-breadcrumb", this.$mapContainer), 
        ...
        if (4 != e) {
			...
			if (t.leyenda) {
				$("#leyenda").html(""); 
				$("#leyenda").append('<a class="leyendafila leyendafilaActivo" title="Todos" data-class="Todos"><span class="leyendaColumA" style="background-color:#fff;">&nbsp;</span><span class="leyendaColumB"> Todos</span></a>');
				for (var l = 0; l < t.leyenda.length; l++) {
					$("#leyenda").append('<a class="leyendafila" title="'+t.leyenda[l].label+'" data-class="'+t.leyenda[l].cssClass+'"><span class="leyendaColumA" style="background-color:'+t.leyenda[l].color+'">&nbsp;</span><span class="leyendaColumB"> '+t.leyenda[l].label+'</span></a>');
                } 
			}
			...
			var imgRuta = "";
			if (typeof getUrlVars()["uv"] === 'undefined' || getUrlVars()["uv"] === null) {
				imgRuta = this.mapsPath + t.map.id + "/map_{z}_{x}_{y}.jpg";
			} else {
				imgRuta = this.mapsPath + t.map.id + "/"+getUrlVars()["uv"]+"/map_{z}_{x}_{y}.jpg";
			}
			
            for (var a = imgRuta, r = t.map.mapImage.width, i = t.map.mapImage.height, n = 0, o = r, s = i, l = 129; o > l || s > l; )
                o = Math.floor(o / 2), s = Math.floor(s / 2), n++;
            n--;
            var m = this.findInitialZoom(t.map.zoom);
            if (this.clear(), this.mapViewer = L.map("mapViewer", {minZoom: 18 - n, maxZoom: 18, crs: L.CRS.Simple}), -1 == m) {
                var p = this.mapViewer.unproject([0, 0], this.mapViewer.getMaxZoom()), c = this.mapViewer.unproject([r, i], this.mapViewer.getMaxZoom()), d = L.latLngBounds(p, c);
                this.mapViewer.fitBounds(d)
            } else
                this.mapViewer.setView(this.mapViewer.unproject([r / 2, i / 2], this.mapViewer.getMaxZoom()), m);
            {
                L.tileLayer(a, {attribution: r + "x" + i, bounds: L.latLngBounds(this.mapViewer.unproject([1, 1], this.mapViewer.getMaxZoom()), this.mapViewer.unproject([r, i], this.mapViewer.getMaxZoom()))}).addTo(this.mapViewer)
            }
            ...
            }), this.$mapContainer.addClass("map-id-" + this.currentMapId), 
				this.fillMapContainer(t.viewHtml.breadcrumb, t.viewHtml.legend), this._regionsData = t.regions, t.labels) {
                for (var h = 0; h < t.labels.length; h++) {
                    var u = t.labels[h];
                    L.marker(this.mapViewer.unproject([u.x, u.y], this.mapViewer.getMaxZoom()), {icon: L.yourMapIcon({className: "cfm-label cfm-layer-element", html: u.html, dataId: u.id})}).addTo(this.labelsLayer)
                }
                this.labelsLayer.addTo(this.mapViewer)
            }
            if (t.markers) {
                for (var h = 0; h < t.markers.length; h++) {
                    var f = t.markers[h], g = L.marker(this.mapViewer.unproject([f.x, f.y], this.mapViewer.getMaxZoom()), {icon: L.yourMapIcon({className: "cfm-marker cfm-layer-element cfm-marker-" + f.typeCssName, html: f.html, dataId: f.id})});
        ...
        if (this.unselectElement(), e.addClass("cfm-selected"), a) {
            ...
            this.mapViewer.panTo(r.getLatLng(), {animate: !1})
        }
        ...
        for (var t = e.is(".cfm-label") ? this.labelsLayer.getLayers() : this.markersLayer.getLayers(), a = e.data("id"), r = 0; r < t.length; r++)
            if (t[r].options.icon.options.dataId == a)
                return t[r];
        return!1
        ...
        if (this.currentMapId == e) {
            ...
            if (!r) {
                var s = this.$mapContainer.offset(), l = this.$mapContainer.height(), m = this.$mapContainer.width(), p = $("#markerPopover");
                if (p.length)
                    var c = p.offset().top - s.top, d = p.offset().left - s.left, h = p.outerHeight(!0), u = p.outerWidth(!0);
                var f = L.point(0, 0);
                c + h > l && (f.y += h - (l - c)), 0 > d && (f.x += d - 10), d + u > m && (f.x -= m - (d + u) - 10), this.mapViewer.panBy(f, {duration: .5})
            }
        ...
        if (this.currentMapId == e) {
            ...
            return this.mapViewer.setView(this.mapViewer.unproject([r.x, r.y], this.mapViewer.getMaxZoom()), i, {animate: !a}), !0
        }
        return this.pendingRegionId = t, this.showMap(e), !1
    }, markerPopoverContentFunction: function () {
	
		var e = $(".cfm-params", $(this));
		var loteId = $(this).children().attr('data-id');
		
		var html="";
		var resul = ajax_cargar(serviceCtrl.gatePath + "?&c=&a=loteInfo&mapa=" +getUrlVars()["mapa"], "POST", "&id="+loteId, "json");
		resul.success(function (jsondata) {
			html += jsondata.data.info;
        }); 
		
        resul.error(function (jqXHR, textStatus, errorThrown) {
			html +="<b>Error al cargar datos</b>";
        });
		...
		return e.length ? e[0].outerHTML : html 
    }, markerPopoverTitleFunction: function () {
	
        var e = $(this); 
		var nro = $(this).children().attr('data-nro');
		var manz = $(this).children().attr('data-manz');
        return '<div class="table-wrap"><div class="text">Mz:' + manz + ' Lote:' + nro + '</div></div>';
//		return '<div class="table-wrap"><div class="text"> &nbsp; </div></div>';
        ...
        var e = [1, 3, 6, 12, 25, 50, 100], t = e[e.length - (18 - mapViewerCtrl.mapViewer.getZoom() + 1)];
        mapViewerCtrl.mapViewer.attributionControl.setPrefix("Desarrollado por: <a href='http://orangegroup.com.bo/' target='_blank'>Orange Group SRL</a> | Zoom: " + t + "%");
        var a = [];
    ...
    }}, modalCtrl = {$messageModal: null, $longTextModal: null, init: function () {
        ...
        $(".notifications").notify({type: t ? "error" : "success", message: {html: e}}).show()
