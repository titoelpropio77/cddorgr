...
class Plano
{
	...
	function plano_puntos_vista() {
	
		$sqlWhere = "";
		if(isset($_GET['fase'])) {
			$sqlWhere = "pla_urb_id='".$_GET['fase']."' ";
		} else {
			$sqlWhere = "pla_urb_id='".$_GET['id']."' ";
		}
		
		// Cargar configuracion
		$conec = new ADO();
		$sql="SELECT * FROM urbanizacion_plano_config WHERE ".$sqlWhere."; ";
		$conec->ejecutar($sql);
		$num = $conec->get_num_registros();
		if($num > 0) {
			$objetoDatos = $conec->get_objeto();
							 ...
							 callback: function(v,m,f){
							  if(v){
									...
									$.ajax({
										...
										data: "&lot_id="+lot_ID+"&lot_coorX="+lot_coorX+"&lot_coorY="+lot_coorY+"&urb_id="+urb_id+"&man_id="+f.manzano+"&equipamiento="+f.equipamiento+"&uv="+f.uv, 
										success: function (results) {
											var jsondata = JSON.parse(results);
											var accion = jsondata[0].respuesta.accion.toString();
											if(accion == "correcto"){
	...
	function plano_puntos() {
		...
		// Consultar si esta por fases
		$sql="SELECT * FROM urbanizacion_plano_config WHERE pla_fases='".$_GET['id']."'";
		$conec->ejecutar($sql);
		$numFase = $conec->get_num_registros();
		if($numFase > 0) {
		
			if(isset($_GET['fase'])) {
				$this->plano_puntos_vista();
			} else {
			
				?>
				<div class="fila_formulario_cabecera">PLANO DE URBANIZACION POR FASE</div>
				<table class="tablaLista" cellpadding="0" cellspacing="0">
					...
					<tbody>
						...
						for( $f=0; $f<$numFase;  $f++) {
							$objFase = $conec->get_objeto();
							
							$url_publico = "http://" . $_SERVER["SERVER_NAME"]."/modulos/uv/?mapa=&u=".$objFase->pla_fases."&uv=".$objFase->pla_uv_id;
							$url_privado = "http://" . $_SERVER["SERVER_NAME"]."/modulos/uv/?mapa=".$encodeLimpio."&u=".$objFase->pla_fases."&uv=".$objFase->pla_uv_id;
							
							?>  
							<tr>
								<td>Fase <?php echo $objFase->pla_uv_id; ?></td>
								<td><a href="gestor.php?mod=uv&tarea=PUNTOS&id=2&fase=<?php echo $objFase->pla_urb_id; ?>&uv=<?php echo $objFase->pla_uv_id; ?>">Agregar puntos</a></td>
	...
	function lotes_coordenadas_imagen($uv_id)
	{
		$datos = "";
		$sql="SELECT * FROM lote WHERE lot_uv_id='".$uv_id."'";
		$conec2 = new ADO();
		$conec2->ejecutar($sql);
	...
	{
		$datos = "";
		// filtrar por fase o uv
		if(isset($_GET['uv'])){
			$datos = $this->lotes_coordenadas_imagen($_GET['uv']);
		} else {
			// todo los uv de la urbanizacion
			$conec = new ADO();
			...
			for( $i=0; $i<$num;  $i++){
				$objeto = $conec->get_objeto();
				$datos .= $this->lotes_coordenadas_imagen($objeto->uv_id);
				$conec->siguiente();
			}
		}
		return $datos;
