...
class con_apertura_gestion extends BUSQUEDA 
{
	...
	function main(){
            ...
            }elseif($acc == 'gestionar_cmps_saldos_cartera'){
                $this->gestionar_cmps_saldos_cartera();
            } else{
                $this->ver_opciones();
            }
	}
            ...
            foreach ($listado as $fila) {
                if($fila->aprobado){
                    ...
                    if($fila->debe>0 || $fila->haber>0) {
                        ...
                        if ($fila->can_id_ges_post > 0) {
                            $can_id=  $fila->can_id_ges_post;
//                            echo "<p>can_id: $can_id para $fila->codigo.</p>";
                        } else {
//                            echo "<p>can_id = 0 para $fila->codigo.</p>";
                            if($fila->can_codigo != ''){
                                $can_id=  FUNCIONES::get_cuenta_ca($ges_id, $fila->can_codigo);
//                                echo "<p>Calculando can_id: $can_id para $fila->codigo y can_codigo=$fila->can_codigo.</p>";
                            }
                        }
                        
                        $cfl_id = 0;
                        if ($fila->cfl_id_ges_post > 0) {
                            $cfl_id=  $fila->cfl_id_ges_post;
//                            echo "<p>cfl_id: $cfl_id para $fila->codigo.</p>";
                        } else {
//                            echo "<p>cfl_id = 0 para $fila->codigo.</p>";
                            if($fila->cfl_codigo != ''){
                                $cfl_id=  FUNCIONES::get_cuenta_cf($ges_id, $fila->cfl_codigo);
//                                echo "<p>Calculando cfl_id: $cfl_id para $fila->codigo y cfl_codigo=$fila->cfl_codigo.</p>";
                            }
                        }
                        
                        $cue_id = 0;
                        if ($fila->cue_id_ges_post > 0) {
                            $cue_id=  $fila->cue_id_ges_post;
//                            echo "<p>cue_id: $cue_id para $fila->codigo.</p>";
                        } else {
//                            echo "<p>cue_id = 0 para $fila->codigo.</p>";
                            if($fila->codigo != ''){
                                $cue_id = FUNCIONES::get_cuenta($ges_id, $fila->codigo);
//                                echo "<p>Calculando cue_id: $cue_id para $fila->codigo.</p>";
                            }
                        }                                                
...
//            $aprobado = 0;  //LUEGO ELIMINAR ESTA LINEA
...
//                echo"$saldo=$sum_debe-$sum_haber;";
                ...
                $conec = NULL;
                $params = new stdClass();
                $params->insercion_masiva = TRUE;
                $params->limit_ins = 1000;
                COMPROBANTES::modificar_comprobante($comprobante, $conec, TRUE, $params);
                require_once('modulos/con_comprobante/con_comprobante.class.php');
                $con_comprobante = new con_comprobante();
                $cmp=  FUNCIONES::objeto_bd_sql("select * from con_comprobante where cmp_tabla='apertura_gestion' and cmp_apertura=1 and cmp_ges_id='$_SESSION[ges_id]' and cmp_eliminado='No'");
                $con_comprobante->imprimir_cmp($cmp->cmp_id);
...
//                echo"$saldo=$sum_debe-$sum_haber;";
                ...
                $params->insercion_masiva = TRUE;
                $params->limit_ins = 1000;
                COMPROBANTES::registrar_comprobante($comprobante, $conec, TRUE, $params);
                require_once('modulos/con_comprobante/con_comprobante.class.php');
                $con_comprobante = new con_comprobante();
                $cmp=  FUNCIONES::objeto_bd_sql("select * from con_comprobante where cmp_tabla='apertura_gestion' and cmp_apertura=1 and cmp_ges_id='$_SESSION[ges_id]' and cmp_eliminado='No'");
                $con_comprobante->imprimir_cmp($cmp->cmp_id);
...
//            $formula_val_1=array();
            ...
            $list_cue_cartera = $this->get_cuentas_cartera($gestion_id);            
            
            $f_cue_cartera = ($list_cue_cartera != '')? " and cue_id not in ($list_cue_cartera)": '';
            
            $arr_cuentas_post = $this->arr_cod_cuentas_por_gestion($ges_id);
//            $arr_cuentas_ca_post = $this->arr_cod_cuentas_por_gestion($ges_id,'con_cuenta_ca');
//            $arr_cuentas_cf_post = $this->arr_cod_cuentas_por_gestion($ges_id,'con_cuenta_cf');
...
//                    $objcu=  FUNCIONES::objeto_bd_sql("select * from con_cuenta where cue_codigo='$fila->cue_codigo' and cue_ges_id='$ges_id'");
                    ...
                    if(!array_key_exists($fila->cue_codigo, $arr_cuentas_post)){
                        $aprobado=0;
                        $obs[]='No existe una cuenta con el mismo codigo en la Gestion actual';
                    } else {
                        $cue_id_ges_post = $arr_cuentas_post[$fila->cue_codigo];
                    }
                    ...
                    if($fila->cde_can_id>0){
                       ...
                       if(!in_array($can_codigo, $arr_cuentas_ca_post)){
                            $aprobado=0;
                            $obs[] = 'No existe una cuenta analitica con el mismo codigo en la Gestion actual(1)(<b>'.$can_codigo.'</b>)';
                        }
                    }
                    $cfl_codigo='';
                    $fila->cde_cfl_id = 0;
...
//                    }
                    ...
                    $lista_saldos[]=(object)array(
                        ...
                        'cue_id_ges_post' => $cue_id_ges_post,
                    );
                }
            }
            $equ2=$formula->{'2'};
...
//                    $objcu=  FUNCIONES::objeto_bd_sql("select * from con_cuenta where cue_codigo='$fila->cue_codigo' and cue_ges_id='$ges_id'");
                    ...
                    if($fila->cde_can_id>0){
                       ...
                       if(!$objcan){
                            $aprobado=0;
                            $obs[] = 'No existe una cuenta analitica con el mismo codigo en la Gestion actual(2)(<b>'.$can_codigo.'</b>)';
                        }
                    }
            ...
            $lista_saldos_cartera = $this->listado_saldos_cartera($gestion_id);            
            $lista_saldos_res = array_merge($lista_saldos, $lista_saldos_cartera);
            
            return $lista_saldos_res;
...
<!--                        <div id="ContenedorDiv">
                            <div id="CajaInput">
                                <br>                                                                    
                                    <a href="<?php // echo "gestor.php?mod=$this->modulo&tarea=ACCEDER&acc=gestionar_cmps_saldos_cartera";?>" class="boton">Generar/Actualizar Comprobantes Saldos Cartera</a>                                                                
                                <br>
                                <br>
                            </div>
                        </div>-->
...
//		}
	...
	?>
        ...
        function get_cuentas_cartera($ges_id) {
            $conec = new ADO();

//            $sql_cuentas = "select urb_reserva,urb_cta_por_cobrar_bs,urb_cta_por_cobrar_usd,
//                urb_cue_descuento_capital_bs,urb_cue_descuento_capital_usd,urb_costo,urb_costo_diferido,
//                urb_inv_terrenos,urb_ing_por_venta_bs,urb_ing_por_venta_usd,urb_ing_diferido,
//                urb_cue_costo_producto,urb_cue_costo_diferido_producto,urb_cue_inv_producto,
//                urb_cue_por_cobrar_producto,urb_cue_ing_diferido_producto
//                from urbanizacion where urb_id not in(5, 6)";
            
            $sql_cuentas = "select urb_cta_por_cobrar_bs,urb_cta_por_cobrar_usd,
                urb_costo_diferido,urb_inv_terrenos,urb_ing_diferido,
                urb_cue_costo_diferido_producto,urb_cue_inv_producto,
                urb_cue_por_cobrar_producto,urb_cue_ing_diferido_producto
                from urbanizacion where urb_id not in(5, 6)";                        

            $urbs = FUNCIONES::objetos_bd_sql($sql_cuentas);
            $arr_urb = $urbs->get_datos();
            // FUNCIONES::print_pre($arr_urb);

            $arr_codigos = array();
            for ($i = 0; $i < count($arr_urb); $i++) {
                $urb = $arr_urb[$i];

                for ($j = 0; $j < (count($urb) / 2); $j++) {
                    $cod_cuenta = $urb[$j];

                    if ($cod_cuenta != '' && $cod_cuenta != null) {
                        $arr_codigos[] = $cod_cuenta;
                    }
                }
            }

            // FUNCIONES::print_pre($arr_codigos);

            $s_codigos = implode("','", $arr_codigos);
            $s_codigos = "'$s_codigos'";

            $sql_max = "SET SESSION group_concat_max_len = 1000000;";
            $conec->ejecutar($sql_max);

            $sql_cue_ids = "select convert(group_concat(cue_id) using utf8) as campo from con_cuenta 
                where cue_codigo in($s_codigos) and cue_ges_id='$ges_id'";

            // echo "<p>$sql_cue_ids</p>";

            $s_cue_ids = FUNCIONES::atributo_bd_sql($sql_cue_ids);

            return $s_cue_ids;
        }
        
    function arr_cod_cuentas_por_gestion_old_version($ges_id, $tipo = 'con_cuenta') {
        ...
        $arr_campos = array(
            'con_cuenta' => array(
                ...
                'codigo' => 'cue_codigo', 
                'ges_id' => 'cue_ges_id', 
                'tipo_cuenta' => 'cue_tipo', 
                'tabla_eliminado' => 'cue_eliminado'
            ),            
            'con_cuenta_ca' => array(
                'tabla_id' => 'can_id', 
                'codigo' => 'can_codigo', 
                'ges_id' => 'can_ges_id', 
                'tipo_cuenta' => 'can_tipo', 
                'tabla_eliminado' => 'can_eliminado'
            ),            
            'con_cuenta_cf' => array(
                'tabla_id' => 'cfl_id', 
                'codigo' => 'cfl_codigo', 
                'ges_id' => 'cfl_ges_id', 
                'tipo_cuenta' => 'cfl_tipo', 
                'tabla_eliminado' => 'cfl_eliminado'
            ),            
        );
        
        $campo_codigo = $arr_campos[$tipo][codigo];
        $campo_gestion = $arr_campos[$tipo][ges_id];
        $campo_eliminado = $arr_campos[$tipo][tabla_eliminado];
        $campo_tipo = $arr_campos[$tipo][tipo_cuenta];
        
        $sql_can_cods_post = "select $campo_codigo
		from $tipo
		where $campo_gestion='$ges_id' and $campo_eliminado='No'
                and $campo_tipo='Movimiento'
		group by $campo_codigo
		order by $campo_codigo asc";
		
	$can_cods_post = FUNCIONES::lista_bd_sql($sql_can_cods_post);
	$arr_can_cods_post = array();
	foreach ($can_cods_post as $can_cod) {
            $valor = $can_cod->$campo_codigo;
//            $arr_can_cods_post[] = "'$valor'";
            $arr_can_cods_post[] = "$valor";
	}
        
        return $arr_can_cods_post;
    }
    
    function arr_cod_cuentas_por_gestion($ges_id, $tipo = 'con_cuenta') {
        ...
        $campo_id = $arr_campos[$tipo][tabla_id];
        $campo_codigo = $arr_campos[$tipo][codigo];
        $campo_gestion = $arr_campos[$tipo][ges_id];
        $campo_eliminado = $arr_campos[$tipo][tabla_eliminado];
        $campo_tipo = $arr_campos[$tipo][tipo_cuenta];
        
        $sql_can_cods_post = "select $campo_id,$campo_codigo
		from $tipo
		where $campo_gestion='$ges_id' and $campo_eliminado='No'
                and $campo_tipo='Movimiento'
		group by $campo_codigo
	...
	foreach ($can_cods_post as $can_cod) {
            $valor = $can_cod->$campo_codigo;
//            $arr_can_cods_post[] = "$valor";
            $arr_can_cods_post[$valor] = $can_cod->$campo_id;
	}
        
        return $arr_can_cods_post;
    }
    
    function listado_saldos_cartera($gestion_id) {

        $lista_saldos = array();
        $ges_id = $_SESSION[ges_id];
        
        $arr_cuentas_post = $this->arr_cod_cuentas_por_gestion($ges_id, 'con_cuenta');
        $arr_cuentas_ca_post = $this->arr_cod_cuentas_por_gestion($ges_id, 'con_cuenta_ca');
        $arr_cuentas_cf_post = $this->arr_cod_cuentas_por_gestion($ges_id, 'con_cuenta_cf');
        
//        echo "<p>ARRAY DE C. ANALITICAS";
//        FUNCIONES::print_pre($arr_cuentas_ca_post);
//        echo "</p>";

        $sql = "select can_id,cue_id,can_codigo,cue_codigo,can_descripcion,cfl_codigo,
            cue_descripcion,sum(cde_valor)as saldo,cde_can_id,cde_cfl_id,cde_int_id,cde_cue_id,cde_cmp_id,cde_mon_id,cde_secuencia
            from con_comprobante_detalle 
            inner join con_comprobante on(cde_cmp_id=cmp_id) 
            inner join con_cuenta on(cde_cue_id=cue_id)
            left join con_cuenta_ca on(cde_can_id=can_id)             
            left join con_cuenta_cf on(cde_cfl_id=cfl_id)
            where can_ges_id=cmp_ges_id 
            and cue_ges_id=cmp_ges_id 
            and cde_mon_id='1'
            and cmp_ges_id='$gestion_id' 
            group by can_id,cue_id
            order by can_codigo asc";

        $detalles = FUNCIONES::objetos_bd_sql($sql);

        for ($i = 0; $i < $detalles->get_num_registros(); $i++) {
            $fila = $detalles->get_objeto();
            
            $s_rastro = "$fila->cde_cmp_id-$fila->cde_mon_id-$fila->cde_secuencia";
            ...
            if ($fila->cde_can_id > 0) {
                ...
                $can_codigo = $fila->can_codigo;                
                if (!array_key_exists($can_codigo, $arr_cuentas_ca_post)) {
                    $aprobado = 0;
                    $obs[] = 'No existe una cuenta analitica con el mismo codigo en la Gestion actual(3-cmp_id:'.$s_rastro.')(<b>'.$can_codigo.'</b>)';
                } else {
                    $can_id_ges_post = $arr_cuentas_ca_post[$can_codigo];
                }
            }
            ...
            if ($fila->cde_cfl_id > 0) {
                ...
                $cfl_codigo = $fila->cfl_codigo;                
                if (!array_key_exists($cfl_codigo, $arr_cuentas_cf_post)) {
                    $aprobado = 0;
                    $obs[] = 'No existe una cuenta de flujo con el mismo codigo en la Gestion actual';
                } else {
                    $cfl_id_ges_post = $arr_cuentas_ca_post[$cfl_codigo];
                }
            }
            ...
            $lista_saldos[] = (object) array(
                ...
                'observacion' => implode('<br>', $obs),
                'cue_id_ges_post' => $cue_id_ges_post,
                'can_id_ges_post' => $can_id_ges_post,
                'cfl_id_ges_post' => $cfl_id_ges_post,
            );
