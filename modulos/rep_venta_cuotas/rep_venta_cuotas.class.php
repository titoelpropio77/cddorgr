...
class REP_VENTA_CUOTAS extends BUSQUEDA {

    var $formulario;
    var $mensaje;

    function REP_VENTA_CUOTAS() {
        //permisos
        $this->ele_id = 151;
        $this->busqueda();
        //fin permisos
        ...
        $this->modulo = 'rep_venta_cuotas';
        $this->formulario = new FORMULARIO();
        $this->formulario->set_titulo('VENTA CUOTAS');
    ...
    function obtener_resultados() {
        ...
        if ($_POST['fecha'] <> "") {
            $filtro.=" and ind_fecha_programada >= '" . FUNCIONES::get_fecha_mysql($_POST['fecha']) . "' ";
            $info[]=array('label'=>'Fecha','valor'=> $_POST['fecha']);
            $_fecha=FUNCIONES::get_fecha_mysql($_POST['fecha']) ;
        }else{
            $fecha_now=  date('Y-m-d');
            $filtro.=" and ind_fecha_programada >= '$fecha_now' ";
            $info[]=array('label'=>'Fecha','valor'=> date('d/m/Y'));
            $_fecha=$fecha_now;
        }

        $num_cuotas=$_POST[nro_cuotas]?$_POST[nro_cuotas]:'0';
        $op_rel=$_POST[op_rel]?$_POST[op_rel]:'=';
        $having=" having(count(*) $op_rel $num_cuotas)";
        $info[]=array('label'=>'Cuotas','valor'=> "$op_rel $num_cuotas");
...
//        }
        ...
        $sql = "SELECT 
                urb_id,ven_id,ven_fecha,ven_superficie,ven_monto,ven_moneda,
                    ...
                    int_telefono,int_celular,int_direccion,ven_ubicacion,int_ci,int_ci_exp,
                    count(*) as num_cuotas,ind_fecha_programada
                FROM 
                venta
                ...
                group by ven_id
                $having
                order by ven_fecha asc, ind_fecha_programada asc";
//        echo "$sql;<br>";
...
//        $estados_carteras=$_POST[estados_carteras];
        $head=array('#','Nro Venta','Cliente','C.I.','Telef.','Direccion','Ubicacion','Proyecto','Moneda','UV','MZ','Lote','Estado','Fecha',
                            ...
                            ,'Saldo','U. Fecha Pag.','U. Fecha Valor','Est. Cartera','Num. Cuotas');
        for ($i = 0; $i < $num; $i++) {
...
//            }
            ...
            $num_cuotas_mora=  FUNCIONES::atributo_bd_sql("select count(*) as campo from interno_deuda where ind_tabla='venta' and ind_tabla_id=$obj->ven_id and ind_estado='Pendiente' and ind_fecha_programada<'$_fecha'");
            if($num_cuotas_mora!=0){
//                echo "DIF $obj->ven_id $num_cuotas_mora<br>";
                $conec->siguiente();
                continue;
            }
//            echo "CERO $obj->ven_id $num_cuotas_mora<br>";
...
//            $pagado=  FUNCIONES::total_pagado_venta($obj->ven_id);
            ...
            if(!$obj->ven_lot_ids){
                $result[]=array(
                    ...
                    $saldo_capital,$ufecha_pago,$ufecha_valor,$est_cartera,$obj->num_cuotas
                );
                $nro++;
            }else{
