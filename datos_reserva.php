...
if ($_POST[tarea] == 'concretar') {
    se_puede_concretar($_POST[res_id]);
}

function se_puede_concretar($res_id) {
    $interno = FUNCIONES::objeto_bd_sql("select * from interno 
        inner join reserva_terreno on(int_id=res_int_id)
        where res_id=$res_id");
    $resp = new stdClass();

    if ($interno) {
        if ($interno->int_completo == 0) {
            $resp->resultado = "no";
            $resp->mensaje .= FUNCIONES::html_encode("<br/>No ha completado los datos de Referencia Personal y Documentacion Presentada del Cliente.");

            $resp->mensaje .= "<br/><a href='javascript:completar_datos($interno->res_int_id);'>Completar Datos</a>";
        } else {
            $resp->resultado = "si";
            $resp->mensaje = "";
        }
