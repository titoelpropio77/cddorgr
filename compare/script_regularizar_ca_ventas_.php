...
function gestionar_descuentos_incrementos() {
    $sql = "select * from venta_pago where vpag_estado='Activo'
    ...
    for ($i = 0; $i < $pagos->get_num_registros(); $i++) {
        ...
        if ($cmp) {
            agregar_detalles_desc_inc($pag, $cmp);
        } else {
            crear_cmp_descuento_incremento($pag);
        }
...
function agregar_detalles_desc_inc($pag, $cmp) {
    ...
    if ($pag->vpag_capital_inc > 0) {
        $detalles[] = array(
            "cuen" => FUNCIONES::get_cuenta($ges_id, $cue_por_c),
            "debe" => 0,
            "haber" => $capital_producto,
            "glosa" => $glosa,
            "ca" => 0,
            "cf" => '0',
            "cc" => 0,
...
function crear_cmp_descuento_incremento($pag) {
