...
$f_ven_id = "";
if (($_GET[venta] * 1) > 0) {
    $f_ven_id = " and venta.ven_id='$_GET[venta]'";
}
...
cargar_tmp_ventas_con_pagos_anteriores($fecha_limite);
...
AUX_REPORTE::cargar_historia_ventas($arr_ventas_ids, $fecha_limite, $token_user, $_SESSION[id], TRUE);

$join_aux = " inner join aux_reporte_saldos on(venta.ven_id=ars_ven_id and ars_token_user='$token_user')";
...
$sql_ventas_demazu = "SELECT 
     urbanizacion.*,venta.*,interno.*,lot_nro,uv_nombre,man_nro,
     $campos_aux
    ifnull(tmp.cap_inval,0)as capital_invalido,
 ...
 FROM 
     venta
     left join venta_pago on (venta.ven_id=vpag_ven_id and vpag_estado='Activo' and vpag_fecha_pago<='$fecha_limite')
     inner join interno on (ven_int_id=int_id)
     inner join urbanizacion on (ven_urb_id=urb_id)
     inner join lote on(ven_lot_id=lot_id)
     inner join manzano on(lot_man_id=man_id)
     inner join uv on(lot_uv_id=uv_id)
     left join tmp_res_vtas_pags_ant tmp on (venta.ven_id=tmp.ven_id)
     $join_aux         
     where 1 and ven_urb_id in (7,8,9,10)
	  and ven_numero is not null 
	  and ven_estado != 'Anulado' $f_ven_id
     group by venta.ven_id";

$ventas = funciones::objetos_bd_sql($sql_ventas_demazu);

$arr_ventas = array();
for ($i = 0; $i < $ventas->get_num_registros(); $i++) {
    $s_rastro .= "=>1";
    $venta = $ventas->get_objeto();
    $venta->capital_pagado -= $venta->capital_invalido;
    ...
    <tbody>
        ...
        foreach ($arr_ventas as $reg) {
            ...
            <tr>
                ...
                <td><?php echo $reg->uv; ?></td>                                
                <td><?php echo $reg->manzano; ?></td>                                
...
function capital_invalido($venta, $fecha_limite) {
    $sql_pag = "select ifnull(sum(vpag_capital), 0) as campo from venta_pago 
	inner join con_comprobante on(cmp_tabla='venta_pago' and cmp_tabla_id=vpag_id)
	where vpag_ven_id='$venta->ven_id' 
	and vpag_estado='Activo' 
	and cmp_eliminado='No'
	and vpag_fecha_pago<='$fecha_limite'";

    $capital_invalido = FUNCIONES::atributo_bd_sql($sql_pag) * 1;
    return $capital_invalido;
}

function cargar_tmp_ventas_con_pagos_anteriores($fecha_limite) {

    $conec = new ADO();

    $sql_ventas = "create temporary table tmp_res_vtas_pags_ant as 
	select ven_id,ifnull(sum(vpag_capital), 0)as cap_inval from venta 
	inner join venta_pago on(ven_id=vpag_ven_id)
	inner join con_comprobante on(cmp_tabla='venta_pago' and cmp_tabla_id=vpag_id)
	where ven_urb_id in (7,8,9,10)
        and ven_numero is not null 
    ...
    $conec->ejecutar($sql_ventas, false);
}
?>