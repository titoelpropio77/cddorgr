...
$sql_ventas = "SELECT 
 ...
 FROM 
     ...
     where 1 and ven_urb_id in (2,3,4) and ven_fecha<='$fecha_limite' 
...
$sql_ventas_cdd = "SELECT 
     venta.*,urbanizacion.*,interno.*,
	  lot_nro,
	  uv_nombre,
	  man_nro, $campos_aux
...
$ventas = funciones::objetos_bd_sql($sql_ventas_cdd);
...
function cargar_tmp_ventas_con_pagos_anteriores($fecha_limite) {
	...
	inner join con_comprobante on(cmp_tabla='venta_pago' and cmp_tabla_id=vpag_id)
	where ven_urb_id in (2,3,4) and ven_fecha<='$fecha_limite' 
        and ven_numero is not null and ven_numero != '' and ven_numero not like 'NET-%'
