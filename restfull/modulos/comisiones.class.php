...
class COMISIONES {
    ...
    function historial($params) {
        ...
        $par_mostrar_comisiones = FUNCIONES::atributo_bd_sql("select par_mostrar_comisiones as campo
            from ad_parametro limit 0,1");
        
        $arr_estados_perm = array('Cerrado');
                
        if ($par_mostrar_comisiones == 'Si') {
            $arr_estados_perm[] = 'Abierto';
        }
        ...
        for ($i = $params->pdo_ini * 1; $i <= $params->pdo_fin * 1; $i++) {
            ...
            $periodo = FUNCIONES::objeto_bd_sql("select * from comision_periodo 
            where pdo_id='$i' and pdo_ver_comisiones='Si'");
            
            if (in_array($periodo->pdo_estado, $arr_estados_perm) === FALSE) {
                continue;
            }
...
//        for ($i = $params->pdo_ini * 1; $i <= $params->pdo_fin * 1; $i++) {
            // echo "entrando al bucle";
            $com_per = new stdClass();
            $com_per->BIR = $this->comisiones_generadas_detalle($i, "BIR", $params->vdo_id);
            $com_per->BVI = $this->comisiones_generadas_detalle($i, "BVI", $params->vdo_id);
            $com_per->BRA = $this->comisiones_generadas_detalle($i, "BRA", $params->vdo_id);
            $elem = new stdClass();
            $elem->comisiones = $com_per;
            $elem->periodo = $this->descripcion_periodo($i);
            $coms_per[] = $elem;
...
//        echo $sql;
        ...
        if ($num > 0) {
            ...
            $des = $periodo->pdo_descripcion . " - " . $periodo->ges_descripcion;
        }
        return $des;
    }

    private function comisiones_generadas_detalle($pdo_id, $tipo_comision, $vdo_id){
        $sql = "select com.*,vdo.*,pat.*,ven.*,
        concat(afil.int_nombre,' ',afil.int_apellido)as afiliado from comision com
        ...
        and com.com_tipo='$tipo_comision'
        and com.com_pdo_id=$pdo_id
        and com.com_vdo_id=$vdo_id";
        
        $comisiones = FUNCIONES::objetos_bd_sql($sql);
        
        $arr_coms = array();
        for ($i = 0; $i < $comisiones->get_num_registros(); $i++) {
            $com = $comisiones->get_objeto();
            $elem = new stdClass();
            $elem->com_descripcion = html_encode($com->com_observacion);
            $elem->venta = $com->com_ven_id;
            $elem->afiliado = html_encode($com->afiliado);
            $elem->importe = ($tipo_comision == 'BRA') ? $com->ven_bono_bra : $com->ven_bono_inicial;
            $elem->porcentaje = $com->com_porcentaje;
            $elem->monto = $com->com_monto;
            $arr_coms[] = $elem;
...
//                        and com_fecha_cre <= '$periodo->pdo_fecha_fin'";
            ...
            $sql_com = "select sum(com_monto)as total from comision 
                inner join comision_periodo compdo on (com_pdo_id=compdo.pdo_id)
                where com_tipo='$tipo_comision'
                        and com_estado in ('Pendiente','Pagado')
                        and com_vdo_id='$vdo_id'
...
//        $sql = "select * from con_periodo where pdo_estado='Abierto' and pdo_eliminado='No' and pdo_ges_id='$ges_id'";
        ...
        $s_mostrar = "and compdo.pdo_estado='Cerrado' and compdo.pdo_ver_comisiones='Si'";
//        $s_mostrar = "and compdo.pdo_estado='Cerrado'";
//        if ($par_mostrar_comisiones == 'Si') {
//            $s_mostrar = "and compdo.pdo_estado in ('Abierto','Cerrado')";
//        }
        
        $sql = "select * from con_periodo pdo
        inner join comision_periodo compdo on (pdo.pdo_id=compdo.pdo_id)
        where 1=1 $s_mostrar and pdo.pdo_eliminado='No' and pdo.pdo_ges_id='$ges_id'";
