...
//        echo $sql;
        ...
        for ($i = 0; $i < count($hijos); $i++) {
            ...
            $ele = FUNCIONES::objeto_bd_sql("select convert(concat(int_nombre,' ',int_apellido)using utf8)as nombre,
                ...
                int_fecha_nacimiento as fecha_nac,int_direccion as direccion,
            ...
            $desc_lote = FUNCIONES::objeto_bd_sql("select upper(concat('URB:',urb_nombre,' - Mz:',man_nro,' - Lt:',lot_nro))as terreno,
                ven_fecha_firma
            from venta inner join lote on (ven_lot_id=lot_id)
            inner join manzano on (lot_man_id=man_id)
            inner join urbanizacion on (man_urb_id=urb_id)
            where ven_id='$ele->codigo_venta'");
            ...
            $elem->direccion = html_encode($ele->direccion);
            $elem->terreno = html_encode($desc_lote->terreno);
            $elem->firmado = ($desc_lote->ven_fecha_firma == '0000-00-00')?'No':'Si';
