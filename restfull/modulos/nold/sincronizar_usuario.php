...
function urbanizacion($int_usu_id, $usuDatos) { 
    $arr = array();
    if ($int_usu_id != '') {  
        $sql = "SELECT urb_id,urb_nombre,urb_maxplazo,urb_interes_anual,urb_val_form FROM urbanizacion order by urb_nombre asc";
        $result = mysql_query($sql);
        $num = mysql_num_rows($result);
        if ($num > 0) {
            for ($i = 0; $i < $num; $i++) {
...
function cliente($int_usu_id, $usuDatos) {
    ...
    if ($int_usu_id != '') {
        ...
        $sql = "SELECT int_id, int_nombre,int_apellido,int_email,int_telefono,int_celular,int_fecha_nacimiento,int_direccion,int_usu_id,int_ci,int_estado_civil,int_ci_exp FROM interno WHERE int_usu_id='".$usuDatos[0]."'  or int_id='".$usuDatos[2]."' order by int_id desc LIMIT 0, 500";
		$result = mysql_query($sql); 
        $num = mysql_num_rows($result);
        if ($num > 0) {
            for ($i = 0; $i < $num; $i++) {
