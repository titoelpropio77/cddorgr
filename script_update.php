...
//listado_observados();
//actualizar_sucursal();
//actualizar_ufecha_prog();
//actualizar_costo();
//actualizar_costo();
//actualizar_propietario();
//echo FUNCIONES::get_concepto(1885);
//delete_log();
//actualizar_saldos_netzen();

if (isset($_GET[venta]) && $_GET[venta]*1 > 0) {
    resetear_venta('8787');
}
...
function delete_log() {
    $conec = new ADO();
    $sql_del = "delete from ad_logs where log_accion like'%insert%into%con_comprobante_detalle%'";
    $conec->ejecutar($sql_del);
}

function actualizar_propietario() {
    $conec = new ADO();
    $ventas = FUNCIONES::objetos_bd_sql("select * from venta ");
//    $ventas = FUNCIONES::objetos_bd_sql("select * from venta limit 40,100");
    $num_up = 0;
    for ($i = 0; $i < $ventas->get_num_registros(); $i++) {
        $venta = $ventas->get_objeto();
        $adj = _db::objeto_sql("select * from adjudicatario  where PY=$venta->ven_urb_id and Numero_Adj=$venta->ven_numero ");
        if ($adj) {
            $interno = FUNCIONES::objeto_bd_sql("select * from interno where int_id=$venta->ven_int_id");
            if ($interno->int_codigo != $adj->Codigo_C) {
                $ninterno = FUNCIONES::objeto_bd_sql("select * from interno where int_codigo=$adj->Codigo_C");
                $sql_update = "update venta set ven_int_id='$ninterno->int_id' where ven_id=$venta->ven_id";
                $conec->ejecutar($sql_update, false);
//                echo " $sql_update;<br>";
            }
        }
        $num_up++;
...
//    $limit="0,40";1-40
//    $limit="40,60"; // 41-100
//    $and_filtro=" and ven_id=4198 ";
//    $and_filtro=" and ven_id>=1 and ven_id<=10";
//    $and_filtro=" and ven_id>=11 and ven_id<=100";
//    $and_filtro=" and ven_id>=101 and ven_id<=500";
...
//    $and_filtro=" and ven_id>=6001 and ven_id<=7000";
    ...
    $ventas = FUNCIONES::objetos_bd_sql("    select * from venta where ven_costo_up=0 $and_filtro order by ven_id asc;");
//    $ventas = FUNCIONES::objetos_bd_sql("select * from venta limit 40,100");
    $num_up = 0;
    for ($i = 0; $i < $ventas->get_num_registros(); $i++) {
        $venta = $ventas->get_objeto();
        $urb = FUNCIONES::objeto_bd_sql("select * from urbanizacion where urb_id=$venta->ven_urb_id");
        $costom2 = $urb->urb_val_costo; //FUNCIONES::atributo_bd_sql("select urb_val_costo as campo from urbanizacion where urb_id=$venta->ven_urb_id");
//        $costo=$venta->ven_superficie* $costom2;
        $cambio_usd = 1;
        if ($venta->ven_moneda == '1') {
            $cambio_usd = 6.96; // FUNCIONES::atributo_bd_sql("select tca_valor as campo from con_tipo_cambio where tca_fecha='$venta->ven_fecha' and tca_mon_id=2");
        }
//        $costo=$superficie*($urb->urb_val_costo*$cambio_usd);
        $ven_costo = $venta->ven_superficie * ($costom2 * $cambio_usd);
        $ven_costo_cub = $venta->ven_res_anticipo;
        $ven_costo_pag = 0;
        if ($ven_costo_cub > $ven_costo) {
            $ven_costo_cub = $ven_costo;
        }

        $pagos = FUNCIONES::lista_bd_sql("select * from venta_pago where vpag_ven_id=$venta->ven_id and vpag_estado='Activo'");

//        $saldo_costo=$venta->ven_costo-$venta->ven_cuota_inicial-$venta->ven_costo_cub;
        $saldo_costo = $ven_costo - $venta->ven_res_anticipo - $venta->ven_monto_intercambio - $venta->ven_cuota_inicial;
        $sum_costo_pag = 0;
        if ($saldo_costo > 0) {
            $saldo_venta = $venta->ven_monto_efectivo - $venta->ven_cuota_inicial;
            $sum_costo_pag = 0;
            foreach ($pagos as $pago) {
                if ($pago->vpag_capital > 0) {
                    $capital_ids = explode(',', $pago->vpag_capital_ids);
                    $capital_montos = explode(',', $pago->vpag_capital_montos);
                    $costo = 0;
                    $costo_ids = array();
                    $costo_montos = array();
//                    echo "$pago->vpag_capital_montos<br>";
                    for ($j = 0; $j < count($capital_ids); $j++) {
                        $ind_id = $capital_ids[$j];
                        $cu = FUNCIONES::objeto_bd_sql("select * from interno_deuda where ind_id='$ind_id'");
                        $capital_pagado = $capital_montos[$j] * 1;
                        if ($capital_pagado > 0) {
                            if ($pago->vpag_saldo_final <= 0) {
                            ...
                            if ($costo_pagado > 0) {
                                $sql_ins_idp = "insert into interno_deuda_pago(
                                                    ...
                                                    '$ind_id','$cu->ind_num_correlativo','$pago->vpag_id','$$pago->vpag_codigo','costo',
                                                    '$pago->vpag_fecha_pago','$costo_pagado','Activo','$fecha_cre','$pago->vpag_usu_cre',
                                                    '$pago->vpag_cob_usu','','$pago->vpag_cob_aut'
                                                )
                                    ";
                                $conec->ejecutar($sql_ins_idp, false);
                    ...
                    $sql_up_pag_costo = "update venta_pago set vpag_costo='$costo', vpag_costo_ids='$txt_costo_ids', vpag_costo_montos='$txt_costo_montos' where vpag_id=$pago->vpag_id";
                    $conec->ejecutar($sql_up_pag_costo, false);
                    $cmp = FUNCIONES::objeto_bd_sql("select * from con_comprobante where cmp_tabla='venta_pago' and cmp_tabla_id='$pago->vpag_id'");
                    if ($cmp) {
                        $cue_costo = $urb->urb_costo;
                        $cue_costo_diferido = $urb->urb_costo_diferido;
                        $cue_costo_id = FUNCIONES::get_cuenta($cmp->cmp_ges_id, $cue_costo);
                        $cue_costo_diferido_id = FUNCIONES::get_cuenta($cmp->cmp_ges_id, $cue_costo_diferido);

                        if ($pago->vpag_moneda == 1) {
                            $bcosto = $costo;
                        } else {
                            $bcosto = $costo * 6.96;
                        }

                        $tcambios = FUNCIONES::lista_bd_sql("select * from con_tipo_cambio where tca_fecha='$pago->vpag_fecha_pago'");
                        foreach ($tcambios as $tca) {
                            $scosto = $bcosto / $tca->tca_valor;
                            $sql_up_cmp = "update con_comprobante_detalle set cde_valor='$scosto' where cde_cmp_id='$cmp->cmp_id' and cde_mon_id='$tca->tca_mon_id' and cde_cue_id='$cue_costo_id'";
                            $conec->ejecutar($sql_up_cmp, false);
                            $sql_up_cmp = "update con_comprobante_detalle set cde_valor='-$scosto' where cde_cmp_id='$cmp->cmp_id' and cde_mon_id='$tca->tca_mon_id' and cde_cue_id='$cue_costo_diferido_id'";
                            $conec->ejecutar($sql_up_cmp, false);
                        }
                    }
...
//            $sql_up_ven_costo = "update venta set  ven_costo_pag=$sum_costo_pag  where ven_id=$venta->ven_id";
//            $conec->ejecutar($sql_up_ven_costo);
        } else {
//            echo "$saldo_costo<br>";
        }
        $sql_up = "update venta set ven_costo_up='1', ven_costo='$ven_costo', ven_costo_cub='$ven_costo_cub',ven_costo_pag='$sum_costo_pag' where ven_id=$venta->ven_id";
//        echo "$sql_up;<br>";
        $conec->ejecutar($sql_up);
        $num_up++;
        $ventas->siguiente();
...
function actualizar_ufecha_prog() {
    $conec = new ADO();
    $ventas = FUNCIONES::objetos_bd_sql("select * from venta ");
    $num_up = 0;
    for ($i = 0; $i < $ventas->get_num_registros(); $i++) {
        $venta = $ventas->get_objeto();
        $ucuota = FUNCIONES::objeto_bd_sql("select * from interno_deuda where ind_tabla_id=$venta->ven_id order by ind_id desc limit 1;");
        if ($ucuota) {
            $fecha_ven = $ucuota->ind_fecha_programada;
        } else {
            $fecha_ven = $venta->ven_fecha;
        }

        $sql_up = "update venta set ven_ufecha_prog='$fecha_ven' where ven_id=$venta->ven_id";
        $conec->ejecutar($sql_up);
        $num_up++;
        $ventas->siguiente();
    }
...
function actualizar_sucursal() {
    ...
    for ($i = 0; $i < $ventas->get_num_registros(); $i++) {
        ...
        $adj = _db::objeto_sql("select * from adjudicatario where py=$venta->ven_urb_id and Numero_adj=$venta->ven_numero ");
        if ($adj) {
            $res_adj = $adj = _db::objeto_sql("select * from reservas_adjudicados where py=$venta->ven_urb_id and Numero_Reserva=$adj->Numero_Reserva ");
//            echo "select * from reservas_adjudicados where py=$venta->ven_urb_id and Numero_Reserva=$adj->Numero_Reserva ;<br>";
            if ($res_adj) {

                $sql_up = "update venta set ven_suc_id=$res_adj->Cod_Sucursal where ven_id=$venta->ven_id";
                $conec->ejecutar($sql_up);
                $num_up++;
//                echo "$sql_up;<br>";
            }
...
//    $and_filtro = " and ven_id >=6001 and ven_id <=7000";
//    $and_filtro = " and ven_id >=7001 and ven_id <=8000";
//    $and_filtro = " and ven_id >=1 and ven_id <=100";
//    $and_filtro = " and ven_id >=101 and ven_id <=800";
//    $and_filtro = " and ven_id >=801 and ven_id <=1800"; 
//    $and_filtro = " and ven_id >=1801 and ven_id <=3000"; 
//    $and_filtro = " and ven_id >=3001 and ven_id <=5000"; 
//    $and_filtro = " and ven_id >=5001 and ven_id <=7000"; 
//    $and_filtro = " and ven_id >=7001 and ven_id <=7313"; 
//    $and_filtro = " and ven_id >=7103 and ven_id <=7313"; 
    $and_filtro = " and ven_id >=7314 and ven_id <=8000";
//    
...
//        echo "select * from pagos where  fecha<='$fecha_limite' and PY=$venta->ven_urb_id and Numero_Adj=$venta->ven_numero and sw=0 and tipo in ('0','2') order by fecha asc,recibo asc,Secuencia asc; <br>";
        $tpagos = _db::objetos_sql("select * from pagos where  fecha>='2015-08-12' and PY=$venta->ven_urb_id and Numero_Adj=$venta->ven_numero and sw=0 and tipo in ('0','2') order by fecha asc,recibo asc,Secuencia asc");
        $pagado = total_pagado($venta->ven_id);
        $nsaldo = $venta->ven_monto_efectivo - $pagado->capital - $pagado->descuento + $pagado->incremento;

        if (count($tpagos)) {
            if ($venta->ven_id <= 7313) {
                $pha = $tpagos[0]->Numero_Adj_His;
            } else {
                $pha = 0;
            }
...
//    $and_filtro = " and PY='2'  and Numero_Adj=1 ";
    $urbs = _urbs;
//    $fecha_inicio='2015-08-12';
//    $and_filtro = " and fecha <='$fecha_limite'";
    $and_filtro = "and fecha>='2015-08-12'";
...
//    return;
    ...
    $insert = "insert into venta(
                    ...
                    ven_tipo_intercambio,ven_estado,ven_usu_id,ven_tipo,ven_val_interes,ven_cuota_inicial,ven_tipo_plan,ven_plazo,ven_cuota,ven_frecuencia,
                    ven_observacion,ven_codigo,ven_vdo_pri,ven_comision_pri,ven_concepto,
...
//            }
            ...
            //                    ven_metro,ven_superficie,ven_valor,ven_decuento,ven_monto,ven_res_anticipo,ven_monto_intercambio,ven_monto_efectivo,
            //                    ven_tipo_intercambio,ven_estado,ven_usu_id,ven_tipo,ven_val_interes,ven_cuota_inicial,ven_tipo_plan,ven_plazo,ven_cuota,ven_frecuencia
            //                    ven_observacion,ven_codigo,ven_vdo_pri,ven_comision_pri,ven_vdo_sec,ven_comision_sec,ven_concepto,
            //                    ven_tipo_pago,ven_fecha_cre,ven_monto_pagar,ven_form,ven_intercambio_ids,ven_intercambio_montos,
            //                    ven_costo,ven_costo_cub,ven_costo_pag,ven_numero,ven_promotor
            //                ) values";
...
function internos() {
    $tclientes = _db::query("select * from clientes where Codigo_C>=10524");
...
function actualizar_superficie() {
    $conec = new ADO();
    $ventas = FUNCIONES::objetos_bd_sql("select * from venta where ven_id in (1376,1391,1397,1415,1529,1581,1817,1818,1826,1827,1828,2702,2740,3033,3451)");
    $num_up = 0;
    for ($i = 0; $i < $ventas->get_num_registros(); $i++) {
        $venta = $ventas->get_objeto();
        $adj = _db::objeto_sql("select * from adjudicatario where py=$venta->ven_urb_id and Numero_adj=$venta->ven_numero ");
...
//            echo "select * from reservas_adjudicados where py=$venta->ven_urb_id and Numero_Reserva=$adj->Numero_Reserva ;<br>";
            if ($res_adj) {
                $lote = _db::objeto_sql("select * from lotes where PY=$venta->ven_urb_id and UV=$res_adj->UV and MZA=$res_adj->MZA and Lote=$res_adj->Lote");
                if ($lote) {
                    $sql_up = "update venta set ven_superficie=$lote->Superficie where ven_id=$venta->ven_id";
                    echo "$sql_up;<br>";
                }
            }
        }
