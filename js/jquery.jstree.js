...
function select_input(rel,moneda){
//    alert(rel);
    $('#cue_tipo option').remove();
    var txt1='';
    var txt2='';
    if(rel==='Titulo'){        
        txt1='<option value="Titulo" selected="selected">Titulo</option>';
        txt2='<option value="Movimiento" >Movimiento</option>';        
    }else if (rel==='Movimiento'){
        txt1='<option value="Titulo" >Titulo</option>';
        txt2='<option value="Movimiento" selected="selected">Movimiento</option>';
    }
    $('#cue_tipo').append(txt1);
    $('#cue_tipo').append(txt2);
    $("#cue_tipo").trigger('change');
    var monedas=$('#cue_mon_id option');
    var mon=new Array();
    for (var i=0;i<monedas.size();i++){
        mon[i]={valor:$(monedas[i]).val(),text:$(monedas[i]).text()}
    }
    $('#cue_mon_id option').remove();
    
    for (var j=0;j<mon.length;j++){
        var txt='';
        if(mon[j].valor==moneda){
            txt='<option value="'+mon[j].valor+'" selected="selected">'+mon[j].text+'</option>';
        }else{
            txt='<option value="'+mon[j].valor+'">'+mon[j].text+'</option>';
        }
        $('#cue_mon_id').append(txt);
    }
//    alert($('#cue_tipo option:selected').val());
        ...
        $.jstree.plugin("core", {
            ...
            _fn: {
                ...
                create_node: function(obj, position, js, callback, is_loaded) {
                    ...
                    if (!js.data) {
                        var cue_codigo1 = $("#cue_codigo").val();
                        var cue_descripcion1 = $("#cue_descripcion").val();
                        js.data = this._get_string(cue_codigo1 + " | " + cue_descripcion1);
                    }
                ...
                select_node: function(obj, check, e) {
                    ...
                    if (proceed && !is_selected) {
                        ...
                        if (s.selected_parent_open) {
                            ...
                            // mostra texto en los input
                            treeActivo = obj.attr("id").replace("node_", "");
                            treeTipo = obj.attr('rel');

                            if (treePrimeraVez)
                            {
                                if (obj.attr('rel') !== 'Root')
                                {
                                    //alert(obj.find("a:first").text().toString()); //first()
//                                    alert(obj.attr('rel'));
                                    var cue_descripcion = obj.find("a:first").text().toString().split("|");
                                    $("#cue_codigo").val(treeTrim(cue_descripcion[0]));
                                    $("#cue_descripcion").val(treeTrim(cue_descripcion[1]));
                                    $('#cue_tipo option').removeAttr('selected');
                                    $('#cue_mon_id option').removeAttr('selected');
                                    var rel=obj.attr('rel');
                                    var moneda=obj.attr('data-moneda');
                                    select_input(rel, moneda);
//                                    setTimeout('select_input("'+rel+'","'+moneda+'")',100);
                                }
                            } else {
                                treePrimeraVez = true;
                            }
