<?php
  ...
  {     
   ...
   // $ss = obtener_frases($id);
	...
	$nodoGrupo->caption="MODULOS";

	$nodoGrupo->id = "'0'";

	if (count($vector_aux)>0)
    {
		$nodoGrupo->isChecked= 1;
	}
...
//	$nodoGrupo->addEvento_onClickCaption("ColocarDatos($nodoGrupo->id);");	
  ...
  function obtener_tabla($id,&$conec, $tabla, $campo)

  {

      $vector=array();

      $consulta = "select elp_ele_id from $tabla where $campo='".$id."'";	
...

  {
      ...
      $consulta = "select ele_id,ele_titulo,ele_padre from ad_elemento";	
	  ...
	  for($i=0;$i<$numero;$i++)
		{
			...
			$vector[$i]['id'] = $objeto->ele_id;

		    $vector[$i]['nombre'] = $objeto->ele_titulo;

		    $vector[$i]['id_padre'] = $objeto->ele_padre;	
