...
//                        if ($monto_comision <= $saldo_red) {
    ...
    public static function provisionar_comision($datos) {
        ...
        $params = array(
            ...
            'tipo_bono' => $datos->tipo_bono,
        );
        include_once 'clases/registrar_comprobantes.class.php';
        include_once 'clases/modelo_comprobantes.class.php';
        $comprobante = MODELO_COMPROBANTE::comision_multinivel($params);
        COMPROBANTES::registrar_comprobante($comprobante);
    }

    public static function provision_masiva($pdo_id) {
        $sql = "select * from comision where com_pdo_id='$pdo_id' 
        and com_tipo in ('BIR','BVI','BRA','FED')
        and com_estado in ('Pendiente')";

        $bonos = FUNCIONES::lista_bd_sql($sql);
//exit();
        foreach ($bonos as $b) {

            $venta = FUNCIONES::objeto_bd_sql("select * from venta where ven_id='$b->com_ven_id'");

            $urb = FUNCIONES::objeto_bd_sql("select urb.* from urbanizacion urb
            inner join venta v on (urb.urb_id=v.ven_urb_id)        
            where v.ven_id='$b->com_ven_id'");

            $datos = array(
                'venta' => $venta,
                'vendedor' => $b->com_vdo_id,
                'urb' => $urb,
                'moneda' => $b->com_moneda,
                'fecha' => $b->com_fecha_cre,
                'com_id' => $b->com_id,
                'com_monto' => $b->com_monto,
                'glosa' => "Provision de $b->com_observacion",
                'tipo_bono' => $b->com_tipo
            );
            COMISION::provisionar_comision($datos);
        }
    }

    public static function provision_masiva_vendedor() {
