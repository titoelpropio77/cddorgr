...
class CUPON {
    ...
    public static function generar_cupones($vpag_id, $sorteo) {
        ...
        $obj_sorteo = FUNCIONES::objeto_bd_sql("select * from sorteo where sor_id='$sorteo'");        
        $pago = FUNCIONES::objeto_bd_sql("select * from venta_pago inner join 
            venta on (vpag_ven_id=ven_id)
            where vpag_id='$vpag_id'");
        
        $sucursales = explode(',', $obj_sorteo->sor_sucursales);
        
        if (in_array($pago->ven_suc_id, $sucursales) === FALSE){
            $resp->ok = FALSE;
            $resp->mensaje = "La sucursal de la venta indicada no genera cupones.";
            return $resp;
        }
