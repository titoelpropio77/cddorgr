...
class FUNCION {
    ...
    public static function lista_bd($tabla, $campo = '*', $condicion = '1') {
        ...
        if ($conec->num_registros() > 0) {
            $lista = array();
            for ($i = 0; $i < $conec->num_registros(); $i++) {
                $fila = clone $conec->valores_fila_objeto();
                $lista[] = $fila;
    ...
    public static function objeto_bd($tabla, $col, $valor) {
        ...
        $sql = "select * from $tabla where $col='$valor'";
        $conec->consulta($sql);
        return $conec->valores_fila_objeto();
    }
    ...
    public static function atributo_bd($tabla, $condicion, $campo) {
        ...
        $conec->consulta($sql);
        if ($conec->num_registros() > 0) {
            $objeto = $conec->valores_fila_objeto();
            return $objeto->campo;
        } else {
            return '';
        }
...
//        echo $sql.'<br>';
    ...
    public static function get_cuenta($ges_id, $codigo) {
        $cuenta = FUNCION::atributo_bd("con_cuenta", "cue_ges_id=$ges_id and cue_codigo='$codigo'", "cue_id");
        return $cuenta;
    }

    public static function get_cuenta_ca($ges_id, $codigo) {
        $can_id = FUNCION::atributo_bd("con_cuenta_ca", "can_ges_id=$ges_id and can_codigo='$codigo'", "can_id");
        return $can_id;
    }

    public static function get_cuenta_cf($ges_id, $codigo) {
        $cfl_id = FUNCION::atributo_bd("con_cuenta_cf", "cfl_ges_id=$ges_id and cfl_codigo='$codigo'", "cfl_id");
        return $cfl_id;
    }

    public static function get_cuenta_cc($ges_id, $codigo) {
        $coc_id = FUNCION::atributo_bd("con_cuenta_cc", "cco_ges_id=$ges_id and cco_codigo='$codigo'", "cco_id");
        return $coc_id;
    }

    public static function obtener_periodo($fecha) {
        $periodo = FUNCION::atributo_bd("con_periodo", "pdo_fecha_inicio<='$fecha' and pdo_fecha_fin>='$fecha'", 'pdo_id');
        return $periodo;
    }
    ...
    public static function registrar_comprobante($comprobante) {
        ...
        $cmp_revisado = FUNCION::atributo_bd("con_configuracion", "conf_nombre='revisado'", "conf_valor");
        $cmp_aprobado = FUNCION::atributo_bd("con_configuracion", "conf_nombre='aprobado'", "conf_valor");
        $cmp_estado = 'Activo';
        ...
        $cmp_nro = FUNCION::obtener_per_tc_nro($cmp_peri_id, $cmp_tco_id) + 1;
...
//        echo $insert.';<br>';
        ...
        FUNCION::actualizar_per_tc_nro($cmp_peri_id, $cmp_tco_id, $cmp_nro);

        /** INSERTAR LOS DETALLES* */
        $cmp_detalles = $comprobante->detalles;

        $cambios = FUNCION::objetos_bd("con_tipo_cambio", "tca_fecha='$cmp_fecha'");

        if ($cambios->num_registros() == 0) {
            $cambios = FUNCION::objetos_bd_sql("select * from con_tipo_cambio where tca_fecha in (select max(tca_fecha) as campo from con_tipo_cambio)");
        }
        ...
        $tc = 1;
        for ($j = 0; $j < $cambios->num_registros(); $j++) {
            $cambio = $cambios->valores_fila_objeto();
...
//            echo $sql.";<br>";
            $conec->consulta($sql);
            $cambios->reset();
            for ($i = 0; $i < $cambios->num_registros(); $i++) {
                $cambio = $cambios->valores_fila_objeto();
...
//        echo $sql;
        $periodo = FUNCION::objeto_bd_sql($sql);
//        _PRINT::pre($periodo);
        if ($periodo != null) {
