...
//actualizar_cuota_plazo();
//actualizar_usuarios();
actualizar_copropietarios();

function actualizar_copropietarios() { 
//    $and_filtro = " and PY='2'  and Numero_Adj=1 ";
    $urbs=_urbs;
//    $fecha_limite='2015-08-11';
//    $and_filtro = " and fecha <='$fecha_limite'";
    $and_filtro = "";
    $adjudicados = _db::query("select * from venta where ven_estado in ('Pendiente','Pagado')");
...
//    $lotes_vendidos=array();
    ...
    foreach ($adjudicados as $tven) {
        ...
        if($tven->ven_co_propietario){
            
            $co_propietario = FUNCIONES::objeto_bd_sql("select * from interno where int_id_ant='$tven->ven_co_propietario'");
            $ven_numero=$num_ini+($tven->ven_id*1);
            $venta=FUNCIONES::objeto_bd_sql("select * from venta where ven_numero='$ven_numero'");
            $sql_up="update venta set ven_co_propietario='$co_propietario->int_id' where ven_id='$venta->ven_id'";
            echo "$sql_up;<br>";
...
//    $and_filtro = " and ven_id >=10 and ven_id <=30";
...
function insertar_pagos() { 
    ...
    $ventas = FUNCIONES::objetos_bd_sql("select * from venta where ven_importado='1' and ven_urb_id in (7,8,9,10) and ven_estado in ('Pendiente','Pagado') $and_filtro ");
    echo '-- ' . count($ventas) . '<br>';
//    return;
    
    $insert = "insert into venta_pago(
                    vpag_ven_id,vpag_codigo,vpag_fecha_pago,vpag_fecha_valor,
                    vpag_int_id,vpag_moneda,vpag_saldo_inicial,vpag_dias_interes,
    ...
    for($i=0;$i<$ventas->get_num_registros();$i++){
        $venta=$ventas->get_objeto();
        $ven_id=$venta->ven_id;
        $pagos=FUNCIONES::lista_bd_sql("select * from interno_deuda where ind_estado in ('Pagado') and ind_tabla='venta' and ind_tabla_id='$ven_id' order by ind_id asc");
        $num = 0;
        ...
        $sum_capital_pag=0;
        foreach ($pagos as $indpago) {
            if($indpago->ind_num_correlativo!=0 ){
                $fecha_cre=date('Y-m-d H:i:s');
                $codigo=  FUNCIONES::fecha_codigo();
                if($indpago->ind_num_correlativo==-1){
                    $indpago->ind_observacion=$indpago->ind_concepto;
                }
                if($indpago->ind_estado=='Pagado'){
                    $indpago->ind_interes=$indpago->ind_interes_pagado;
                    $indpago->ind_capital=$indpago->ind_capital_pagado;
                    $indpago->ind_monto=$indpago->ind_monto_pagado;
                    $indpago->ind_saldo=$indpago->ind_saldo_final;
                    
                }
                $saldo_inicial=$indpago->ind_saldo_final-$indpago->ind_capital_pagado;
                $nro_recibo=  FUNCIONES::nro_recibo($indpago->ind_fecha_pago);
//                vpag_ven_id,vpag_codigo,vpag_fecha_pago,vpag_fecha_valor,
//                vpag_int_id,vpag_moneda,vpag_saldo_inicial,vpag_dias_interes,
//                vpag_interes,vpag_capital,vpag_mora,vpag_form,vpag_envio,vpag_monto,vpag_saldo_final,
//                vpag_interes_ids,vpag_capital_ids,vpag_form_ids,vpag_envio_ids,vpag_mora_ids,vpag_mora_con_ids,vpag_mora_gen_ids,
...
//                vpag_val_monto
                $sql_insert=$insert."(
                                '$venta->ven_id','$codigo','$indpago->ind_fecha_pago','$indpago->ind_fecha_programada',
                                $venta->ven_int_id,$venta->ven_moneda,$saldo_inicial,30,
                                $indpago->ind_interes_pagado,$indpago->ind_capital_pagado,0,0,0,$indpago->ind_monto_pagado,$indpago->ind_saldo_final,
                                '$indpago->ind_id','$indpago->ind_id','','','','','',
                                '$indpago->ind_interes_pagado','$indpago->ind_capital_pagado','','','','','','',
                                '$fecha_cre','$indpago->ind_usu_pago','Activo','$indpago->ind_usu_pago','$codigo',0,'$nro_recibo',1,0,
                                0
                                )";
                $conec->ejecutar($sql_insert,false,true);
                $vpag_id=ADO::$insert_id;
                
                if($indpago->ind_capital_pagado>0){
                    $sql_ins_pago="insert into interno_deuda_pago(
                                        idp_ind_id,idp_ind_correlativo,idp_vpag_id,idp_vpag_recibo,idp_vpag_codigo,idp_tipo,
                                        idp_fecha,idp_monto,idp_estado,idp_fecha_cre,idp_usu_cre,idp_cob_usu,idp_cob_codigo,idp_cob_aut
                                    )values(
                                        '$indpago->ind_id','$indpago->ind_num_correlativo','$vpag_id','$nro_recibo','$codigo','capital',
                                        '$indpago->ind_fecha_pago','$indpago->ind_capital_pagado','Activo','$fecha_cre','$indpago->ind_usu_pago','$indpago->ind_usu_pago','$codigo',0
                                    )";
                    $conec->ejecutar($sql_ins_pago,false,false);
                    $sum_capital_pag+=$indpago->ind_capital_pagado;
                }
                if($indpago->ind_interes_pagado>0){
                    $sql_ins_pago="insert into interno_deuda_pago(
                                        idp_ind_id,idp_ind_correlativo,idp_vpag_id,idp_vpag_recibo,idp_vpag_codigo,idp_tipo,
                                        idp_fecha,idp_monto,idp_estado,idp_fecha_cre,idp_usu_cre,idp_cob_usu,idp_cob_codigo,idp_cob_aut
                                    )values(
                                        '$indpago->ind_id','$indpago->ind_num_correlativo','$vpag_id','$nro_recibo','$codigo','interes',
                                        '$indpago->ind_fecha_pago','$indpago->ind_interes_pagado','Activo','$fecha_cre','$indpago->ind_usu_pago','$indpago->ind_usu_pago','$codigo',0
                                    )";
                    $conec->ejecutar($sql_ins_pago,false,false);
                }
                $ufecha_pago=$indpago->ind_fecha_pago;
                $ufecha_valor=$indpago->ind_fecha_programada;
                $cuota_pag=$indpago->ind_num_correlativo;

                $usaldo=$indpago->ind_saldo_final;
        ...
        $sql_up="update venta set 
                        ven_ufecha_pago='$ufecha_pago',
                        ven_ufecha_valor='$ufecha_valor',
                        ven_cuota_pag='$cuota_pag',
                        ven_capital_pag='$sum_capital_pag',
                        ven_capital_desc=0,
                        ven_capital_inc=0,
                        ven_usaldo='$usaldo',
                        ven_sfecha_prog='$sfecha_prog',
                        ven_importado='2'
                    where ven_id='$venta->ven_id'
            ";
        $conec->ejecutar($sql_up, false, false);
...
//    $and_filtro=" and ven_id=7720";
    ...
    $ventas = FUNCIONES::objetos_bd_sql("select * from venta where ven_importado='0' and ven_urb_id in (7,8,9,10) and ven_estado in ('Pendiente','Pagado') $and_filtro ");
...
//    return;
    ...
    for($i=0;$i<$ventas->get_num_registros();$i++){
        $venta=$ventas->get_objeto();
        $ven_id=$venta->ven_numero-$num_ini;
        $tplan_pagos=  _db::objetos_sql("select * from interno_deuda where ind_estado in ('Pendiente','Pagado') and ind_tabla='venta' and ind_tabla_id='$ven_id' order by ind_id asc");
        $num = 0;
        ...
        foreach ($tplan_pagos as $tplan) {
            if($tplan->ind_num_correlativo!=0 || $tplan->ind_estado=='Pendiente' || $tplan->ind_tipo=='pcontado'){
                ...
                //                ind_tipo,ind_fecha_cre,ind_estado_parcial,ind_monto_parcial,
                $fecha_cre=date('Y-m-d');
                if($tplan->ind_num_correlativo==-1){
                    $tplan->ind_observacion=$tplan->ind_concepto;
                }
                ...
                $sql_insert.="(
                                'venta',$venta->ven_id,'$nro',$venta->ven_int_id,'$tplan->fecha','$tplan->ind_usu_id',$tplan->ind_moneda,$tplan->ind_interes,$tplan->ind_capital,$tplan->ind_monto,
                                $tplan->ind_saldo,'$tplan->ind_fecha_programada','$tplan->ind_estado','Cuota Nro $nro, $venta->ven_concepto','$tplan->ind_observacion',0,0,
                                $tplan->ind_interes_pagado,$tplan->ind_capital_pagado,$tplan->ind_monto_pagado,$tplan->ind_saldo_final,'$tplan->ind_fecha_pago',30,
                                'pcuota','$fecha_cre','$tplan->ind_estado_parcial',$tplan->ind_monto_parcial
                                )";
...
//            echo "$sql_insert<br>";
        ...
        $interno = FUNCIONES::objeto_bd_sql("select * from interno where int_id_ant='$tven->ven_int_id'");
        if($tven->ven_co_propietario){
            $co_propietario = FUNCIONES::objeto_bd_sql("select * from interno where int_id_ant='$tven->ven_co_propietario'");
        }
        $sql_lote = "select * from lote where lot_id='$tven->ven_lot_id'";
...
//            }
            ...
            $res_anticipo=0;
            if($tven->ven_res_anticipo>0){
                $res_anticipo+=$tven->ven_res_anticipo;
            }
            $cuota_inicial= _db::objeto_sql("select * from interno_deuda where ind_num_correlativo='0' and ind_estado='Pagado' and ind_tabla='venta' and ind_tabla_id='$tven->ven_id'");
            if($cuota_inicial){
                $res_anticipo+=$cuota_inicial->ind_capital;
            }
            $saldo_efectivo=$tven->ven_monto-$res_anticipo;
            $ven_numero=$num_ini+$tven->ven_id;
//                $insert="insert into venta(
//                            ven_urb_id,ven_numero,ven_lot_id,ven_lot_ids,ven_int_id,ven_co_propietario,ven_fecha,ven_moneda,
//                            ven_res_id,ven_superficie,ven_metro,ven_valor,ven_decuento,ven_incremento,ven_monto,ven_res_anticipo,ven_monto_intercambio,ven_monto_efectivo,
//                            ven_estado,ven_usu_id,ven_tipo,ven_val_interes,ven_cuota_inicial,ven_tipo_plan,ven_plazo,ven_cuota,ven_rango,ven_frecuencia,
//                            ven_observacion,ven_codigo,ven_vdo_id,ven_comision,ven_concepto,ven_tipo_pago,ven_fecha_firma,ven_fecha_cre,ven_monto_pagar,
//                            ven_form,ven_costo,ven_costo_cub,ven_costo_pag,ven_promotor,ven_importado,
//                            ven_lug_id,ven_ubicacion,ven_suc_id,
//                            ven_ufecha_prog,ven_ufecha_pago,ven_ufecha_valor,
//                            ven_cuota_pag,ven_capital_pag,ven_capital_desc,ven_capital_inc,ven_usaldo,ven_sfecha_prog,ven_costo_up
//                        ) values";
            $sql_insert.="(
                            '$tven->ven_urb_id','$ven_numero','$tven->ven_lot_id','','$interno->int_id','$co_propietario->int_id','$tven->ven_fecha','$tven->ven_moneda',
                            '0','$tven->ven_superficie','$tven->ven_metro','$tven->ven_valor','$tven->ven_decuento','0','$tven->ven_monto','$res_anticipo',0,'$saldo_efectivo',
                            '$tven->ven_estado','$tven->ven_usu_id','$tven->ven_tipo','$tven->ven_val_interes','0','plazo',0,0,1,'dia_mes',
                            '$tven->ven_observacion','$tven->ven_codigo',0,0,'$tven->ven_concepto','Normal','0000-00-00','$tven->ven_fecha_creacion',0,
                            7,0,0,0,'$txt_promotor',0,
                            1,'Bolivia,Santa Cruz,Santa Cruz de la Sierra',1,
                            '0000-00-00','0000-00-00','0000-00-00',0,0,0,0,0,'0000-00-00',0
                            )";
            $num++;
            $reg++;
...
//            echo "$res->res_id -- $_mon<br>";
            ...
            $sql_insert.="('$monto','2','$res_fecha','$res_hora',
...
function internos() {
    $clientes=  _db::query("select * from interno where
                int_id in (select ven_int_id from venta where ven_estado in ('Pendiente','Pagado')) or 
                int_id in (select res_int_id from reserva_terreno where res_estado in ('Habilitado','Pendiente')) or
                int_id in (select bloq_int_id from bloquear_terreno where bloq_estado in ('Habilitado')) or 
                int_id in (select ven_co_propietario from venta where ven_estado in ('Pendiente','Pagado'))
                ;");
    
    $insert = "insert into interno(int_codigo,int_id_ant,int_nombre,int_apellido,int_email,int_foto,int_telefono,int_celular,int_direccion,int_ci,int_fecha_nacimiento,int_fecha_ingreso,int_usu_id) values";
    $num = 0;
    $sql_insert = $insert;
    $fecha_cre = date('Y-m-d');
    $reg = 0;
...
//            echo "$sql_insert<br>";
        ...
        $interno=  FUNCIONES::objeto_bd_sql("select * from interno where int_id_ant='$cliente->int_id'");
        ...
        if(!$interno){
            $sql_insert.="('','$cliente->int_id','$cliente->int_nombre','$cliente->int_apellido','$cliente->int_email','$cliente->int_foto','$cliente->int_telefono','$cliente->int_celular','$cliente->int_direccion','$cliente->int_ci','$cliente->int_fecha_nacimiento','$cliente->int_fecha_ingreso','$cliente->int_usu_id')";
            $num++;
            $reg++;
        }else{
